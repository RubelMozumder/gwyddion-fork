/*
 *  gwybatch.c
 *  $Id: gwybatch.c 25497 2023-06-27 09:37:52Z yeti-dn $
...
/*
 * If the file is in a complex multichannel format it is necessary to find something to operate on.  This function
 * finds and returns the channel with the lowest id, which might or might not be the right thing.
 *
 * See get_channel0() for a simple alternative.
 *
 * This requires Gwyddion at least 2.45 for using gwy_app_data_browser_get_data_ids() on data containers not managed
 * by the data browser.
 */
static GwyDataField*
get_first_channel(GwyContainer *data, gint *pid)
{
    ...
    /* Obtain the list of channel numbers and check whether there are any. If we deal with a complex format, the ids
     * can be arbitrary numbers, we cannot just count from 0.  */
...
/*
 * If the file is in a simple format that can contain only one image, this is sufficient to obtain it.
 *
...
/*
 * A function that does something with the data field.  Not necessarily a good grain marking method...
 */
static GwyDataField*
     ...
     char *argv[])
{
    ...
    if (argc == 1) {
        ...
        g_print("Version: $Id: gwybatch.c 25497 2023-06-27 09:37:52Z yeti-dn $.\n");
        return 0;
    }

    /* Initialize Gwyddion and lower level libraries.  Pass TRUE for the "enable-threads" option to enable
     * multi-thread processing within individual library function (if available).
     *
     * This functions requires Gwyddion 2.55 but it makes the initialisation very simple. */
    gwy_app_init_nongui("enable-threads", FALSE, NULL);
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (mfield) {
            ...
            gwy_container_set_object(newdata, gwy_app_get_data_key_for_id(0), dfield);
            gwy_container_set_object(newdata, gwy_app_get_mask_key_for_id(0), mfield);
            /* Be nice and also copy title, metadata and other properties if the original file has them. */
            ...
            if (!gwy_file_save(newdata, newfilename, GWY_RUN_NONINTERACTIVE, &err)) {
