/*
 *  $Id: gwymodule-file.c 26073 2024-01-02 10:41:41Z yeti-dn $
 ...
 * @detect: Detection function.  It may be %NULL, files of such a type can can be then loaded and saved only on
 ...
 *
 * At least one of @load, @save, and @export_ must be non-%NULL.  See #GwyFileOperationType for differences between
 * save and export.
 *
                         ...
                         FileDetectData *ddata)
{
    ...
    score = func_info->detect(ddata->fileinfo, ddata->only_name, func_info->name);
                        ...
                        GError **error)
{
    ...
    if (ddata.winner) {
        ...
        gwy_file_detect_free_info(&fileinfo);
        if (gwy_file_func_run_save(ddata.winner, data, filename, mode, error))
            return ddata.mode;
        return 0;
    }
    ...
    if (ddata.winner) {
        ...
        if (gwy_file_func_run_export(ddata.winner, data, filename, mode, error))
            return ddata.mode;
        return 0;
    }
 ...
 * @data: A #GwyContainer.
 * @name: Location to store file type (that is file function name) of @data, or %NULL.  The returned string is owned
 *        by module system.
 * @filename_sys: Location to store file name of @data (in GLib encoding), or %NULL.  The returned string is owned by
 *                module system and is valid only until the container is destroyed or saved again.
 *
 ...
 *
 * Returns: File name of @data (in GLib encoding), or %NULL.  The returned string is owned by module system and is
                       ...
                       const gchar *filename_sys)
{
    ...
    gwy_assign_string(&fti->filename_sys, filename_sys);
}
 ...
 * @name: Function name from as registered with gwy_file_func_register() (single-function modules can safely ignore
 *        this argument).
 *
...
/**
 * GwyModuleFileError:
 * @GWY_MODULE_FILE_ERROR_CANCELED: Interactive operation was cancelled by user.  (Since 2.45)
