/*
 *  $Id: log.c 25641 2023-09-07 12:26:23Z yeti-dn $
...
static GQuark         curve_map_log_key            (gint id);
 ...
 * then can also create secondary outputs.  Note you still need to pass a second %NULL argument as the option
 * terminator.
 *
 * Since: 2.35
 **/
 ...
 *
 * This simplified variant of gwy_app_channel_log_add() takes the currently running data processing function name and
 * constructs the qualified function name from that.
 *
 * Since: 2.38
 **/
void
 ...
 *
 * This simplified variant of gwy_app_volume_log_add() takes the currently running function volume data processing
 * name and constructs the qualified function name from that.
 *
 * Since: 2.38
 **/
void
 ...
 * @function: Quailified name of the function applied as shown by the module browser.  For instance "xyz::xyz_invert".
 * @...: Logging options as a %NULL-terminated list of pairs name, value.
 *
 ...
 *
 * This simplified variant of gwy_app_xyz_log_add() takes the currently running function XYZ data processing name and
 * constructs the qualified function name from that.
 *
 * Since: 2.45
 **/
 ...
 * @data: A data container.
 * @previd: Identifier of the previous (source) #GwyLawn curve map in the container.
 *          Pass -1 for a no-source (or unclear source) operation.
 * @newid: Identifier of the new (target) #GwyLawn curve map in the container.
 * @function: Quailified name of the function applied as shown by the module browser.  For instance "cmap::cmap_crop".
 * @...: Logging options as a %NULL-terminated list of pairs name, value.
 *
                          ...
                          ...)
{
    va_list ap;
    va_start(ap, function);
    data_log_add_valist(data, curve_map_log_key, previd, newid, function, ap);
    va_end(ap);
}
 ...
 *
 * This simplified variant of gwy_app_curve_map_log_add() takes the currently running function #GwyLawn curve map
 * processing name and constructs the qualified function name from that.
 *
 * Since: 2.60
 **/
void
                                    ...
                                    gint newid)
{
    ...
    funcname = current_function_name_by_type("cmap");
    g_return_if_fail(funcname);
    qname = g_strconcat("cmap::", funcname, NULL);
    gwy_app_curve_map_log_add(data, previd, newid, qname, NULL);
    g_free(qname);
}
                    ...
                    va_list ap)
{
    ...
    if (!targetlog) {
        ...
        gwy_container_pass_object(data, newquark, targetlog);
    }
             ...
             gboolean create)
{
    ...
    gwy_container_pass_object(data, quark, slog);
 ...
 *
 * If the log browser is already shown for this channel it is just raised and given focus.  Otherwise, a new window is
 * created.
 *
 * Returns: The log browser (owned by the library).  Usually, you can ignore the return value.
 *
 * Since: 2.35
 **/
GtkWidget*
 ...
 *
 * If the log browser is already shown for this volume data it is just raised and given focus.  Otherwise, a new
 * window is created.
 *
 * Returns: The log browser (owned by the library).  Usually, you can ignore the return value.
 *
 ...
 *
 * If the log browser is already shown for this XYZ data it is just raised and given focus.  Otherwise, a new window
 * is created.
 *
 * Returns: The log browser (owned by the library).  Usually, you can ignore the return value.
 *
 ...
 * @data: A data container.
 * @id: Id of #GwyLawn curve map in @data to show log for.
 *
 * Shows a simple log browser for #GwyLawn curve map.
 *
 * If the log browser is already shown for this #GwyLawn curve map it is just raised and given focus.  Otherwise,
...
gwy_app_log_browser_for_curve_map(GwyContainer *data,
                                  gint id)
{
    return get_log_browser(data, BROWSER_DATA_CURVE_MAP, id);
}
                ...
                gint id)
{
    ...
    if (type == BROWSER_DATA_CHANNEL)
        quark = channel_log_key(id);
    else if (type == BROWSER_DATA_VOLUME)
        quark = volume_log_key(id);
    else if (type == BROWSER_DATA_XYZ)
        quark = xyz_log_key(id);
    else if (type == BROWSER_DATA_CURVE_MAP)
        quark = curve_map_log_key(id);
    else {
        g_return_val_if_reached(NULL);
    }

    if (gwy_container_gis_object(data, quark, &slog)) {
    ...
    browser = log_browser_new(data, type, id, quark);
                ...
                GQuark quark)
{
    ...
    gwy_help_add_to_window(GTK_WINDOW(browser->window), "logging", NULL, GWY_HELP_DEFAULT);
    ...
    if (type == BROWSER_DATA_CHANNEL)
        ...
        dataname = gwy_app_get_lawn_title(data, id);
    else {
        g_return_val_if_reached(browser);
    }

    title = g_strdup_printf(_("Log of %s (%s)"), dataname, g_get_application_name());
    ...
    g_object_weak_ref(G_OBJECT(slog), (GWeakNotify)&gwy_log_data_finalized, browser);

    browser->changed_id = g_signal_connect(slog, "value-changed", G_CALLBACK(log_changed), browser);
    ...
    gtk_window_set_default_size(GTK_WINDOW(browser->window),
                                ...
                                MIN(request.height + 32, 2*gdk_screen_height()/3));
    ...
    browser->destroy_id
        = g_signal_connect_swapped(browser->window, "destroy", G_CALLBACK(gwy_log_destroy), browser);
    ...
    g_signal_connect_swapped(browser->save, "clicked", G_CALLBACK(gwy_log_export), browser);
    ...
    g_signal_connect_swapped(browser->clear, "clicked", G_CALLBACK(gwy_log_clear), browser);
    ...
    g_signal_connect_swapped(browser->close, "clicked", G_CALLBACK(gtk_widget_destroy), browser->window);
...
log_browser_construct(LogBrowser *browser)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(columns); i++) {
        ...
        column = gtk_tree_view_column_new_with_attributes(_(columns[i].title), renderer, NULL);
        gtk_tree_view_column_set_cell_data_func(column, renderer, log_cell_renderer, browser, NULL);
...
gwy_log_export(LogBrowser *browser)
{
    ...
    gwy_save_auxiliary_data(_("Export Log"), GTK_WINDOW(browser->window), -1, str_to_save);
...
gwy_log_destroy(LogBrowser *browser)
{
    ...
    g_object_weak_unref(G_OBJECT(browser->log), (GWeakNotify)&gwy_log_data_finalized, browser);
 ...
 *
 * If logging is disabled logging functions such as gwy_app_channel_log_add() become no-op.  It is possible to run the
 * log viewer with gwy_app_log_browser_for_channel() to see log entries created when logging was enabled.
 *
 * Since: 2.35
 **/
void
...
current_function_name_by_type(const gchar *type)
{
    ...
    else if (gwy_strequal(type, "cmap"))
 ...
 *
 * Logging functions such as gwy_app_channel_log_add() take settings values corresponding to the function name and
