/*
 *  $Id: filelist.c 25970 2023-10-27 14:58:24Z yeti-dn $
...
static void           gwy_app_recent_file_list_close              (Controls *controls);
...
gwy_app_recent_file_list_new(void)
{
    ...
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(gwy_app_recent_file_list_close), &gcontrols);
...
gwy_app_recent_file_list_update_sensitivity(Controls *controls)
{
    ...
    if (!controls->window || !controls->filter)
        return;
...
static void
gwy_app_recent_file_list_close(Controls *controls)
{
    controls->filter = NULL;
...
gwy_app_recent_file_list_free(void)
{
    ...
    model = GTK_TREE_MODEL(gcontrols.store);
    if (gtk_tree_model_get_iter_first(model, &iter)) {
        do {
            gtk_tree_model_get(model, &iter, FILELIST_RAW, &rf, -1);
            gwy_app_recent_file_free(rf);
...
gwy_app_recent_file_list_update_menu(Controls *controls)
{
    ...
    if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(controls->store), &iter)) {
        ...
        do {
            ...
            i++;
        } while (i < nrecent && gtk_tree_model_iter_next(GTK_TREE_MODEL(controls->store), &iter));
    }
...
gwy_recent_file_thumbnail_name(const gchar *uri)
{
    GChecksum *checksum;
    gchar filename[37];
    const gchar *hexdigest;
    guint len;

    checksum = g_checksum_new(G_CHECKSUM_MD5);
    g_checksum_update(checksum, uri, strlen(uri));
    hexdigest = g_checksum_get_string(checksum);
    len = strlen(hexdigest);
    if (len != 32) {
        g_critical("Unexpected MD5 hexadecimal digest length %u.", len);
        len = MIN(len, 32);
    }
    memcpy(filename, hexdigest, len);
    memcpy(filename + len, ".png", 5);
    g_checksum_free(checksum);

    return g_build_filename(gwy_recent_file_thumbnail_dir(), "normal", filename, NULL);
}
