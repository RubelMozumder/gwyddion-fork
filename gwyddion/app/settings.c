/*
 *  $Id: settings.c 25204 2023-01-25 15:03:59Z yeti-dn $
 ...
 * storing persistent module data manually you should use <literal>"/module/YOUR_MODULE_NAME/"</literal> prefix.
                      ...
                      GError **error)
{
    ...
    new_settings = gwy_container_deserialize_from_text(buffer + strlen(magic_header));
    g_free(buffer);
    if (!new_settings) {
        g_set_error(error, GWY_APP_SETTINGS_ERROR, GWY_APP_SETTINGS_ERROR_CORRUPT,
    ...
    gwy_container_remove_by_prefix(gwy_settings, "/module/logistic/thetas");
...
gwy_app_settings_merge_string(const gchar *s,
                              ...
                              GError **error)
{
    GwyContainer *settings, *new_settings;
    ...
    new_settings = gwy_container_deserialize_from_text(s_to_parse);
    g_free(buffer);
    if (!new_settings) {
        g_set_error(error, GWY_APP_SETTINGS_ERROR, GWY_APP_SETTINGS_ERROR_CORRUPT,
                    _("File is corrupted, deserialization failed."));
    ...
    gwy_container_transfer(new_settings, settings, "/", "/", overwrite);
...
gwy_app_settings_set_defaults(GwyContainer *settings)
{
    ...
    if (!gwy_container_contains_by_name(settings, "/mask/alpha")) {
        gwy_container_set_double_by_name(settings, "/mask/red", default_mask_color.r);
        gwy_container_set_double_by_name(settings, "/mask/green", default_mask_color.g);
        gwy_container_set_double_by_name(settings, "/mask/blue", default_mask_color.b);
        gwy_container_set_double_by_name(settings, "/mask/alpha", default_mask_color.a);
    }
    if (!gwy_container_contains_by_name(settings, "/app/gradients/preferred"))
        gwy_container_set_const_string_by_name(settings, "/app/gradients/preferred", default_preferred_gradients);
    if (!gwy_container_contains_by_name(settings, "/app/glmaterials/preferred"))
        gwy_container_set_const_string_by_name(settings, "/app/glmaterials/preferred", default_preferred_gl_materials);
    if (!gwy_container_contains_by_name(settings, "/app/toolbox/visible/graph"))
        gwy_container_set_boolean_by_name(settings, "/app/toolbox/visible/graph", TRUE);
    if (!gwy_container_contains_by_name(settings, "/app/toolbox/visible/proc"))
        gwy_container_set_boolean_by_name(settings, "/app/toolbox/visible/proc", TRUE);
    if (!gwy_container_contains_by_name(settings, "/app/toolbox/visible/tool"))
        gwy_container_set_boolean_by_name(settings, "/app/toolbox/visible/tool", TRUE);
    if (!gwy_container_contains_by_name(settings, "/app/toolbox/visible/zoom"))
        gwy_container_set_boolean_by_name(settings, "/app/toolbox/visible/zoom", TRUE);
}
...
gwy_app_settings_gather(GwyContainer *settings)
{
    ...
    if (name)
        gwy_container_set_string_by_name(settings, "/app/gradients/default", g_strdup(name));
    name = gwy_inventory_get_default_item_name(gwy_gl_materials());
    if (name)
        gwy_container_set_string_by_name(settings, "/app/glmaterials/default", g_strdup(name));
}
...
gwy_app_settings_apply(GwyContainer *settings)
{
    ...
    /* Preferred resources */
    if (gwy_container_gis_string_by_name(settings, "/app/gradients/preferred", &s)) {
    ...
    if (gwy_container_gis_string_by_name(settings, "/app/glmaterials/preferred", &s)) {
    ...
    /* Default resources */
    if (gwy_container_gis_string_by_name(settings, "/app/gradients/default", &s))
        gwy_inventory_set_default_item_name(gwy_gradients(), s);
    if (gwy_container_gis_string_by_name(settings, "/app/glmaterials/default", &s))
        gwy_inventory_set_default_item_name(gwy_gl_materials(), s);

    /* Globally disabled 3D view axes */
    if (gwy_container_gis_boolean_by_name(settings, "/app/3d/axes/disable", &disabled) && disabled)
        gwy_3d_view_class_disable_axis_drawing(disabled);

    /* Globally disabled logging */
    if (gwy_container_gis_boolean_by_name(settings, "/app/log/disable", &disabled) && disabled)
...
gwy_app_settings_get_module_dirs(void)
{
    const gchar *module_types[] = {
        "cmap", "layer", "file", "process", "graph", "tool", "volume", "xyz",
    };
    gchar **module_dirs;
    gchar *p;
    const gchar *q;
 ...
 * gtk_gl_init_check() and gwy_widgets_gl_init() (if the former succeeeds).
 *
 ...
 *
 * Returns: The return value is the same as the return value of gwy_app_gl_init() which needs to be called prior to
 ...
 *                                 its creation failed.
 * @GWY_APP_SETTINGS_ERROR_EMPTY: Settings file is empty (Since 2.45).  This was previously reported as
 *                                %GWY_APP_SETTINGS_ERROR_CORRUPT but empty settings files can be overwritten without
 *                                losing anything.
 *
