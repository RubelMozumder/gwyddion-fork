/*
 *  $Id: validate.c 26098 2024-01-04 14:02:30Z yeti-dn $
           ...
           GSList **errors)
{
    ...
    *errors = g_slist_prepend(*errors,
                              FAIL(GWY_DATA_ERROR_NON_UTF8_STRING, key, _("byte at position %d"), end - string));
    return FALSE;
}
...
fail:
    *errors = g_slist_prepend(*errors,
                              FAIL(GWY_DATA_ERROR_KEY_CHARACTERS, key, _("byte at position %d"), end - strkey));
    return FALSE;
}
           ...
           GSList **errors)
{
    ...
    if (G_UNLIKELY(!G_VALUE_HOLDS(gvalue, G_TYPE_OBJECT))) {
        *errors = g_slist_prepend(*errors,
                                  FAIL(GWY_DATA_ERROR_ITEM_TYPE, key, _("%s instead of %s"),
                                       g_type_name(G_VALUE_TYPE(gvalue)), g_type_name(type)));
        return FALSE;
    }
                    ...
                    gpointer user_data)
{
    ...
    if (strkey[0] != GWY_CONTAINER_PATHSEP)
        *errors = g_slist_prepend(*errors, FAIL(GWY_DATA_ERROR_KEY_FORMAT, key, NULL));

    id = _gwy_app_analyse_data_key(strkey, &type, &len);
    if (type == KEY_IS_NONE) {
        if (info->flags & GWY_DATA_VALIDATE_UNKNOWN)
            *errors = g_slist_prepend(*errors, FAIL(GWY_DATA_ERROR_KEY_UNKNOWN, key, NULL));
        return;
    }
    ...
    /* Items that must have data id.  While ids above 2^20 are technically valid, they indicate a bug or malice. */
    if (id < 0 || id >= (1 << 20))
        *errors = g_slist_prepend(*errors, FAIL(GWY_DATA_ERROR_KEY_ID, key, NULL));
    ...
    switch (type) {
        ...
        case KEY_IS_LAWN:
        if (check_type(gvalue, GWY_TYPE_LAWN, key, errors))
            g_array_append_val(info->curve_maps, id);
        break;
        ...
        default:
        g_warning("Key type %u of %s not handled in validate_item()", type, strkey);
        break;
    }
}
                    ...
                    gpointer user_data)
{
    ...
    switch (type) {
        ...
        if (!in_array(info->channels, id)) {
            *errors = g_slist_prepend(*errors,
                                      FAIL(GWY_DATA_ERROR_STRAY_SECONDARY_DATA, key, _("no channel %d exists for %s"),
                                           id, strkey));
        }
        break;
        ...
        if (!in_array(info->graphs, id)) {
            *errors = g_slist_prepend(*errors,
                                      FAIL(GWY_DATA_ERROR_STRAY_SECONDARY_DATA, key, _("no graph %d exists for %s"),
                                           id, strkey));
        }
        break;
        ...
        if (!in_array(info->spectra, id)) {
            *errors = g_slist_prepend(*errors,
                                      FAIL(GWY_DATA_ERROR_STRAY_SECONDARY_DATA, key, _("no spectra %d exists for %s"),
                                           id, strkey));
        }
        break;
        ...
        if (!in_array(info->volumes, id)) {
            *errors = g_slist_prepend(*errors,
                                      FAIL(GWY_DATA_ERROR_STRAY_SECONDARY_DATA, key, _("no brick %d exists for %s"),
                                           id, strkey));
        }
        break;
        ...
        if (!in_array(info->xyzs, id)) {
            *errors = g_slist_prepend(*errors,
                                      FAIL(GWY_DATA_ERROR_STRAY_SECONDARY_DATA, key, _("no surface %d exists for %s"),
                                           id, strkey));
        }
        break;
        ...
        case KEY_IS_LAWN_LOG:
        case KEY_IS_LAWN_VIEW_SCALE:
        if (!in_array(info->curve_maps, id)) {
            *errors = g_slist_prepend(*errors,
                                      FAIL(GWY_DATA_ERROR_STRAY_SECONDARY_DATA, key, _("no lawn %d exists for %s"),
                                           id, strkey));
        }
        break;
                ...
                GSList **errors)
{
    ...
    *errors = g_slist_prepend(*errors,
                              FAIL(GWY_DATA_ERROR_REF_COUNT, key, _("ref_count is %d for %s"), object->ref_count, s));
    g_free(s);
                    ...
                    gpointer user_data)
{
    ...
    if (GWY_IS_CONTAINER(object))
        gwy_container_foreach((GwyContainer*)object, NULL, &validate_item_pass3, info);
    ...
    else if (GWY_IS_LAWN(object)) {
        GwyLawn *lawn = (GwyLawn*)object;

        CRFC(lawn->si_unit_xy, "si-unit-xy");
    }
    POP(info->stack);
}
...
/**
 * gwy_data_validate:
 * @data: Data container.  It should not be managed by the data browser (yet) if flags contain
 *        %GWY_DATA_VALIDATE_CORRECT (because things can in principle break during the correction) or
 *        %GWY_DATA_VALIDATE_REF_COUNT (because the application took some references).
 * @flags: Validation flags.  Some influence what is checked, some determine what to do when problems are found.
 *
 * Checks the contents of a data file.
 *
 * If %GWY_DATA_VALIDATE_CORRECT is given in @flags, correctable problems are corrected.  At present correctable
 * problems are those that can be fixed by removal of the offending data.
 *
 * Returns: List of errors found, free with gwy_data_validation_failure_list_free(). The list is independent on
 *          whether %GWY_DATA_VALIDATE_CORRECT is given in flags, even though the offending items may be no longer
 *          exist in the container after correction.  If %GWY_DATA_VALIDATE_NO_REPORT is present in flags, %NULL is
 *          always returned.
 *
 * Since: 2.9
 **/
                  ...
                  GwyDataValidateFlags flags)
{
    ...
    if ((flags & GWY_DATA_VALIDATE_NO_REPORT) && !(flags & GWY_DATA_VALIDATE_CORRECT)) {
    ...
    info.curve_maps = g_array_new(FALSE, FALSE, sizeof(gint));
    ...
    g_array_free(info.curve_maps, TRUE);
 ...
 *
 * Returns: Error description as an untranslated string owned by the library.
 *
 * Since: 2.9
 **/
const gchar*
 ...
 *
 * A #GwyContainer can be used to represent all sorts of data.  However, Gwyddion, the application, follows certain
 * conventions in data organization. Function gwy_data_validate() checks whether the data actually follows them. This
 ...
 * @details: Error details, may be %NULL for some types of @error.  This is a dynamically allocated string owned by
 *           the caller, however, he normally frees the complete errors lists with
 *           gwy_data_validation_failure_list_free() which frees these fields too.
 *
...
/**
 * GwyDataError:
 * @GWY_DATA_ERROR_KEY_FORMAT: Key format is invalid (e.g. does not start with %GWY_CONTAINER_PATHSEP).
 * @GWY_DATA_ERROR_KEY_CHARACTERS: Key contains unprintable characters or characters not representable in ASCII.
 * @GWY_DATA_ERROR_KEY_UNKNOWN: Key does not correspond to any data item known to this version of Gwyddion.
 * @GWY_DATA_ERROR_KEY_ID: Key corresponds to a data item with bogus id number.
 * @GWY_DATA_ERROR_ITEM_TYPE: Wrong item type (for instance an integer at key <literal>"/0/data"</literal>).
 * @GWY_DATA_ERROR_NON_UTF8_STRING: String value is not valid UTF-8.
 * @GWY_DATA_ERROR_REF_COUNT: Reference count is higher than 1.
 * @GWY_DATA_ERROR_STRAY_SECONDARY_DATA: Secondary data item (e.g. mask, selection or visibility) without
 *                                       a corresponding valid primary data item.
 *
 ...
 * @GWY_DATA_VALIDATE_UNKNOWN: Report all unknown keys as %GWY_DATA_ERROR_KEY_UNKNOWN errors.  Note while a data item
 *                             unknown to the current version of Gwyddion can come from a newer version therefore it
 *                             can be in certain sense valid.
 * @GWY_DATA_VALIDATE_REF_COUNT: Report all object items with reference count higher than 1 as
 *                               %GWY_DATA_ERROR_REF_COUNT errors.  Obviously this makes sense only with `fresh' data
 *                               containers.
