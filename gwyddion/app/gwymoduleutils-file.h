/*
 *  $Id: gwymoduleutils-file.h 25239 2023-02-23 12:59:48Z yeti-dn $
...
gwy_get_pascal_real_le(const guchar **ppv)
{
    ...
    x = 1.0 + (((((*ppv)[1]/256.0 + (*ppv)[2])/256.0 + (*ppv)[3])/256.0 + (*ppv)[4])/256.0 + ((*ppv)[5] & 0x7f))/128.0;
    if ((*ppv)[5] & 0x80)
        x = -x;
...
gwy_get_pascal_real_be(const guchar **ppv)
{
    ...
    x = 1.0 + (((((*ppv)[4]/256.0 + (*ppv)[3])/256.0 + (*ppv)[2])/256.0 + (*ppv)[1])/256.0 + ((*ppv)[0] & 0x7f))/128.0;
    if ((*ppv)[0] & 0x80)
        x = -x;
