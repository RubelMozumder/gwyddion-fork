/*
 *  $Id: app.c 26046 2023-12-01 09:52:36Z yeti-dn $
...
gwy_app_quit(void)
{
    ...
    gwy_app_get_graph_thumbnail(NULL, -1, 0, 0);
                       ...
                       Gwy3DWindow *gwy3dwindow)
{
    ...
    pixbuf = g_object_get_data(G_OBJECT(dialog), "pixbuf-to-save");
    ...
    if (!gdk_pixbuf_save(pixbuf, filename_sys, filetype ? filetype : "png", &err, NULL)) {
...
static GdkPixbuf*
gwy_app_3d_window_get_pixbuf(Gwy3DWindow *gwy3dwindow,
                             gboolean autocrop, gboolean transparent)
{
...
gwy_app_3d_window_export(Gwy3DWindow *gwy3dwindow)
{
    ...
    pixbuf = gwy_app_3d_window_get_pixbuf(gwy3dwindow, autocrop, transparent);
    ...
    g_object_set_data(G_OBJECT(dialog), "pixbuf-to-save", pixbuf);
...
brick_preview_filter(GwyContainer *data,
                     ...
                     gpointer user_data)
{
    BrickPreviewGUI *gui = (BrickPreviewGUI*)user_data;
    GwyDataField *field = gwy_container_get_object(data, gwy_app_get_data_key_for_id(id));

    if (gwy_data_field_check_compatibility_with_brick_xy(field, gui->brick,
...
static GwyParamDef*
define_brick_preview_parameters(GwyBrick *brick)
{
    static const GwyEnum types[] = {
        { N_("Mean"),          BRICK_PREVIEW_MEAN,    },
        { N_("Minimum"),       BRICK_PREVIEW_MINIMUM, },
        { N_("Maximum"),       BRICK_PREVIEW_MAXIMUM, },
        { N_("Min. position"), BRICK_PREVIEW_MINPOS,  },
        { N_("Max. position"), BRICK_PREVIEW_MAXPOS,  },
        { N_("RMS"),           BRICK_PREVIEW_RMS,     },
        { N_("Image:"),        BRICK_PREVIEW_IMAGE,   },
        { N_("Section:"),      BRICK_PREVIEW_SECTION, },
    };
    GwyParamDef *pardef = gwy_param_def_new();

    gwy_param_def_add_gwyenum(pardef, PARAM_PREVIEW_TYPE, NULL, _("Preview quantity"),
                              types, G_N_ELEMENTS(types), BRICK_PREVIEW_MEAN);
    gwy_param_def_add_int(pardef, PARAM_PREVIEW_LEVEL, NULL, NULL, 0, gwy_brick_get_zres(brick)-1, 0);
...
static void
recalculate_brick_preview(BrickPreviewGUI *gui)
{
    GwyBrick *brick = gui->brick;
    BrickPreviewType type = gwy_params_get_enum(gui->params, PARAM_PREVIEW_TYPE);
    GwyDataField *preview = gui->preview;
    ...
    if (type == BRICK_PREVIEW_MEAN)
        ...
        gwy_data_field_assign(preview, gwy_params_get_image(gui->params, PARAM_PREVIEW_IMAGE));
    else if (type == BRICK_PREVIEW_SECTION)
        gwy_brick_extract_xy_plane(brick, preview, gwy_params_get_int(gui->params, PARAM_PREVIEW_LEVEL));
    else {
        g_assert_not_reached();
    }
    gwy_container_set_object(gui->data, gui->quark, gui->preview);
    gwy_data_field_data_changed(gui->preview);
}

static void
brick_preview_param_changed(BrickPreviewGUI *gui, gint id, GwyParamTable *table)
{
    gboolean has_image = !!gwy_params_get_image(gui->params, PARAM_PREVIEW_IMAGE);
    BrickPreviewType type = gwy_params_get_enum(gui->params, PARAM_PREVIEW_TYPE);

    if (id < 0 && !has_image) {
        if (type == BRICK_PREVIEW_IMAGE)
            gwy_param_table_set_enum(table, PARAM_PREVIEW_TYPE, (type = BRICK_PREVIEW_MEAN));
        gwy_param_table_radio_set_sensitive(table, PARAM_PREVIEW_TYPE, BRICK_PREVIEW_IMAGE, FALSE);
    }
    if (id < 0 || id == PARAM_PREVIEW_TYPE) {
        gwy_param_table_set_sensitive(table, PARAM_PREVIEW_LEVEL, type == BRICK_PREVIEW_SECTION);
        gwy_param_table_set_sensitive(table, PARAM_PREVIEW_IMAGE, type == BRICK_PREVIEW_IMAGE);
    }
    gwy_dialog_invalidate(GWY_DIALOG(gui->dialog));
}

static void
brick_preview_preview(gpointer user_data)
{
    BrickPreviewGUI *gui = (BrickPreviewGUI*)user_data;
    ...
    if (gui->initial_preview) {
        gui->initial_preview = FALSE;
        return;
    }
    recalculate_brick_preview(gui);
...
change_brick_preview(GwyDataWindow *data_window)
{
    static const guint simple_types[] = {
        BRICK_PREVIEW_MEAN, BRICK_PREVIEW_MINIMUM, BRICK_PREVIEW_MAXIMUM,
        BRICK_PREVIEW_MINPOS, BRICK_PREVIEW_MAXPOS,
        BRICK_PREVIEW_RMS,
    };
    BrickPreviewGUI gui;
    GwyDialog *dialog;
    ...
    gui.data = data;
    gui.id = id;
    gui.quark = gwy_app_get_brick_preview_key_for_id(id);
    gui.preview = gwy_container_get_object(data, gui.quark);
    gui.old_preview = gwy_data_field_duplicate(gui.preview);

    pardef = define_brick_preview_parameters(brick);
    params = gui.params = gwy_params_new();
    gwy_params_set_def(params, pardef);

    /* These two are safe because zeros (from NULL) are good defaults. */
    gwy_params_set_enum(params, PARAM_PREVIEW_TYPE,
                        GPOINTER_TO_UINT(g_object_get_data(G_OBJECT(brick), "gwy-preview-type")));
    gwy_params_set_int(params, PARAM_PREVIEW_LEVEL,
                       GPOINTER_TO_UINT(g_object_get_data(G_OBJECT(brick), "gwy-preview-level")));
    if (gwy_container_gis_boolean_by_name(gwy_app_settings_get(), "/app/brick-preview/update", &update))
        gwy_params_set_boolean(params, PARAM_PREVIEW_UPDATE, update);
    gui.initial_preview = !update;

    gui.dialog = gwy_dialog_new(_("Change Volume Data Preview"));
    dialog = GWY_DIALOG(gui.dialog);
    gtk_window_set_transient_for(GTK_WINDOW(dialog), GTK_WINDOW(data_window));
    gwy_dialog_add_buttons(dialog, GTK_RESPONSE_CANCEL, GTK_RESPONSE_OK, 0);

    table = gwy_param_table_new(params);
    gwy_param_table_append_radio_header(table, PARAM_PREVIEW_TYPE);
    for (i = 0; i < G_N_ELEMENTS(simple_types); i++)
        gwy_param_table_append_radio_item(table, PARAM_PREVIEW_TYPE, simple_types[i]);
    gwy_param_table_append_radio_item(table, PARAM_PREVIEW_TYPE, BRICK_PREVIEW_IMAGE);
    gwy_param_table_append_image_id(table, PARAM_PREVIEW_IMAGE);
    gwy_param_table_data_id_set_filter(table, PARAM_PREVIEW_IMAGE, brick_preview_filter, &gui, NULL);
    gwy_param_table_append_radio_item(table, PARAM_PREVIEW_TYPE, BRICK_PREVIEW_SECTION);
    gwy_param_table_append_slider(table, PARAM_PREVIEW_LEVEL);
    ...
    gwy_dialog_set_preview_func(dialog, GWY_PREVIEW_IMMEDIATE, brick_preview_preview, &gui, NULL);
    g_signal_connect_swapped(table, "param-changed", G_CALLBACK(brick_preview_param_changed), &gui);

    outcome = gwy_dialog_run(dialog);
    if (outcome == GWY_DIALOG_CANCEL) {
        /* Make sure we get back the old preview, regardless what the user did in the dialog. */
        gwy_data_field_assign(gui.preview, gui.old_preview);
        gwy_data_field_data_changed(gui.preview);
        goto end;
    }
    ...
    g_object_set_data(G_OBJECT(brick), "gwy-preview-type",
                      GUINT_TO_POINTER(gwy_params_get_enum(params, PARAM_PREVIEW_TYPE)));
    g_object_set_data(G_OBJECT(brick), "gwy-preview-level",
                      GUINT_TO_POINTER(gwy_params_get_int(params, PARAM_PREVIEW_LEVEL)));
    gwy_container_set_boolean_by_name(gwy_app_settings_get(),
                                      "/app/brick-preview/update",
                                      gwy_params_get_boolean(params, PARAM_PREVIEW_UPDATE));

end:
    g_object_unref(gui.old_preview);
...
change_lawn_preview(GwyDataWindow *data_window)
{
    ...
    if (!gwy_container_gis_object(data, quark, (GObject**)&preview)) {
        ...
        gwy_container_pass_object(data, quark, preview);
    }
