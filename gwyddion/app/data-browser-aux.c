/*
 *  $Id: data-browser-aux.c 25870 2023-10-16 08:11:39Z yeti-dn $
    ...
    for (i = 0; i < nfuncs; i++) {
        if (keyfuncs[i].item == what) {
            ...
            if (!type) {
                ...
                gwy_container_pass_object(dest, qto, obj);
            }
            else {
    ...
    switch (what) {
        ...
        for (i = 0; i < G_N_ELEMENTS(cal_keys); i++) {
            ...
            if (gwy_container_gis_object_by_name(source, key_from, &obj)) {
                ...
                gwy_container_pass_object_by_name(dest, key_to, obj);
            }
            else if (delete_too)
 ...
 *
 * All items except %GWY_DATA_ITEM_PREVIEW are valid for image data.
 **/
void
                                  ...
                                  GwyContainer *dest)
{
    ...
    if (gwy_container_gis_object(source, key, &dfield)) {
        ...
        gwy_container_pass_object(dest, key, dfield);
    }
    ...
    if (gwy_container_gis_object_by_name(source, buf, &slog)
        ...
        gwy_container_pass_object_by_name(dest, buf, slog);
