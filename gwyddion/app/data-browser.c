/*
 *  $Id: data-browser.c 26368 2024-05-30 13:40:15Z yeti-dn $
 *  Copyright (C) 2006-2024 David Necas (Yeti), Petr Klapetek, Chris Anderson
...
typedef struct {
    GtkListStore *store;
    gint last;  /* The id of last object, if no object is present, it is equal to the smallest possible id minus 1 */
    gint active;
    gint visible_count;
} GwyAppDataList;
...
struct _GwyAppDataBrowser {
    ...
    GwyAppDataId switch_to_data;
    GwyAppPage switch_to_page;
...
static void                   desperately_switch_to_something        (void);
...
gwy_app_data_proxy_finalize(gpointer user_data)
{
    ...
    if (browser == proxy->parent) {
        /* This is crude and we try to improve it below. */
...
#if 0
    desperately_switch_to_something();
#endif
...
set_up_data_list_signals(GtkTreeView *treeview, GwyAppDataBrowser *browser)
{
    ...
    g_signal_connect(treeview, "drag-begin", G_CALLBACK(gwy_app_data_browser_block_switching), NULL);
    g_signal_connect(treeview, "drag-end", G_CALLBACK(gwy_app_data_browser_unblock_switching), NULL);
}
                                  ...
                                  G_GNUC_UNUSED gpointer userdata)
{
    ...
    pixbuf = gwy_app_get_graph_thumbnail(container, id, 500*THUMB_SIZE/433, 433*THUMB_SIZE/500);
    pbuf_timestamp = g_new(gdouble, 1);
    *pbuf_timestamp = gwy_get_timestamp();
    g_object_set_data_full(G_OBJECT(pixbuf), "timestamp", pbuf_timestamp, g_free);
    gtk_list_store_set(GTK_LIST_STORE(model), iter, MODEL_THUMBNAIL, pixbuf, -1);
                                  ...
                                  gint id)
{
    ...
    /* XXX: Strictly, this might not be necessary. But just block switching during all DnD for consistency. */
    g_signal_connect(curves, "drag-begin", G_CALLBACK(gwy_app_data_browser_block_switching), NULL);
    g_signal_connect(curves, "drag-end", G_CALLBACK(gwy_app_data_browser_unblock_switching), NULL);
...
ensure_brick_previews(GwyAppDataProxy *proxy)
{
    ...
    do {
        ...
        if (!gwy_container_gis_object(proxy->container, quark, (GObject**)&preview) || !GWY_IS_DATA_FIELD(preview)) {
            ...
            gwy_container_pass_object(proxy->container, quark, preview);
        }
        g_object_unref(brick);
    } while (gtk_tree_model_iter_next(model, &iter));
}
                                ...
                                gint id)
{
    ...
    if (!gwy_container_gis_object(proxy->container, gwy_app_get_surface_preview_key_for_id(id), &raster)
        ...
        gwy_container_pass_object(proxy->container, gwy_app_get_surface_preview_key_for_id(id), raster);
    }
...
static void
switch_to_widget(GtkWidget *widget, GwyAppPage page)
{
    if (!widget)
        return;

    if (page == GWY_PAGE_CHANNELS)
        gwy_app_data_browser_select_data_view(GWY_DATA_VIEW(widget));
    else if (page == GWY_PAGE_VOLUMES)
        gwy_app_data_browser_select_volume(GWY_DATA_VIEW(widget));
    else if (page == GWY_PAGE_XYZS)
        gwy_app_data_browser_select_xyz(GWY_DATA_VIEW(widget));
    else if (page == GWY_PAGE_CURVE_MAPS)
        gwy_app_data_browser_select_curve_map(GWY_DATA_VIEW(widget));
    else if (page == GWY_PAGE_GRAPHS)
        gwy_app_data_browser_select_graph(GWY_GRAPH(widget));
}

G_GNUC_UNUSED
static void
desperately_switch_to_something(void)
{
    GwyAppDataBrowser *browser = gwy_app_get_data_browser();
    GwyAppDataProxy *proxy;
    GwyAppDataList *list;
    ...
    /* The data proxy list is natively sorted by last access, so we just start from the head. */
    for (l = browser->proxy_list; l; l = g_list_next(l)) {
        proxy = (GwyAppDataProxy*)l->data;
        for (page = 0; page < GWY_NPAGES; page++) {
            if (page == GWY_PAGE_SPECTRA)
                continue;
            list = proxy->lists + page;
            if (list->visible_count)
                break;
        }
        if (page < GWY_NPAGES)
            break;
    ...
    gwy_debug("page = %d, list->active = %d", page, list->active);
    if (!gwy_app_data_proxy_find_object(list->store, list->active, &iter))
        return;

    gtk_tree_model_get(GTK_TREE_MODEL(list->store), &iter, MODEL_WIDGET, &widget, -1);
    gwy_debug("found widget %p", widget);
    switch_to_widget(widget, page);
    GWY_OBJECT_UNREF(widget);
}
...
gwy_app_data_browser_select_data_view(GwyDataView *data_view)
{
    ...
    if (browser->block_switching) {
        browser->switch_to_page = GWY_PAGE_CHANNELS;
        browser->switch_to_data.datano = proxy->data_no;
        browser->switch_to_data.id = i;
        return;
    }
...
/**
 * gwy_app_data_browser_block_switching:
 *
 ...
 * the DnD, so at the drop time the tree view content no longer matches the dragged row.
 *
 * Use gwy_app_data_browser_unblock_switching() to restore data switching. It will also switch data to the window that
...
gwy_app_data_browser_block_switching(void)
{
    ...
    if (!browser->block_switching) {
        browser->switch_to_data.datano = 0;
        browser->switch_to_data.id = -1;
        browser->switch_to_page = GWY_PAGE_NOPAGE;
    }
 ...
 *
 * The blocking is started by gwy_app_data_browser_block_switching(). See its description for details.
 *
 * Since: 2.67
 **/
void
gwy_app_data_browser_unblock_switching(void)
{
    GwyAppDataBrowser *browser = gwy_app_get_data_browser();
    GwyContainer *container;
    GwyAppDataProxy *proxy;
    ...
    page = browser->switch_to_page;
    dataid = browser->switch_to_data;
    if (page == GWY_PAGE_NOPAGE || dataid.datano <= 0 || dataid.id < 0)
        return;

    if (!(container = gwy_app_data_browser_get(dataid.datano))
        || !(proxy = gwy_app_data_browser_get_proxy(browser, container)))
        return;

    list = proxy->lists + page;
    if (!gwy_app_data_proxy_find_object(list->store, dataid.id, &iter))
        return;
                                 ...
                                 gboolean showit)
{
    ...
    gwy_container_pass_object(proxy->container, quark, raster);
 ...
 * @GWY_DATA_ITEM_PREVIEW: Volume, XYZ or curve map data preview.  (Since 2.51)
 *
 ...
 * gwy_app_sync_data_items() or gwy_app_sync_volume_items() for specification which items can be used with them.
 **/
