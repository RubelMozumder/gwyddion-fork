/*
 *  $Id: gwyappfilechooser.c 25787 2023-10-06 13:44:50Z yeti-dn $
...
gwy_app_file_chooser_add_type_list(GwyAppFileChooser *chooser)
{
    ...
    gtk_tree_selection_select_iter(selection, &iter);  /* This relies on iter being set above. */
    ...
    if (action == GTK_FILE_CHOOSER_ACTION_OPEN) {
         ...
         * an explicit no_filter object that does nothing (2) since we cannot combine filters we need a single
...
ensure_brick_preview(GwyContainer *container, gint id)
{
    ...
    gwy_container_pass_object(container, pquark, preview);
}
