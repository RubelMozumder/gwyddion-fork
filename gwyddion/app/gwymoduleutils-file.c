/*
 *  $Id: gwymoduleutils-file.c 25239 2023-02-23 12:59:48Z yeti-dn $
 ...
 *
 * The threshold for highly non-square is somewhat arbitrary. Fortunately, most files encoutered in practice have the
 * measure ratio either very close to 1, larger or equal than 2.
 *
 * Returns: %TRUE if the channel was found to have highly non-square pixels and `realsquare' was set (otherwise it was
 *          unset).
 *
 * Since: 2.3
 **/
 ...
 *
 * The guess is very simple, but probably better than `Unknown channel' in most cases.  If there already is a title it
 * is left intact, making use of this function as a fall-back easier.
 *
 ...
 * @dfield: A data field.  The values of bad data points are ignored and might be even left uninitialized.
 * @mfield: A mask field containing 1.0 in place of good data points, 0.0 in place of bad points.  It will be inverted
 *          to become the mask of bad points.
 *
 * Replaces bad data points with some neutral values.
 *
 * Since Gwyddion has no concept of bad data points, they are usually marked with a mask and replaced with some
 * neutral values upon import, leaving the user to decide how to proceed further.  This helper function performs such
 * replacement.
 *
 * Returns: The number of bad data points replaced.  If zero is returned, all points are good and there is no need for
 *          masking.
 *
 * Since: 2.14
 **/
 ...
 *
 * NaNs and infinities are not normally allowed in Gwyddion so this function should be used upon import if the source
 * file format can represent such values.
 *
 * The invalid values can be automatically removed using the same method as gwy_app_channel_remove_bad_data().
 *
 * Returns: A newly created data field with 1.0 in pixels corresponding to a NaN or infinity in @dfield.  If no such
 *          pixels are present, %NULL is returned.
 *
 * Since: 2.38
 **/
GwyDataField*
...
{
    if (!context->parser->error || !context->parser->error(context, context->error, context->user_data))
 ...
 * @header: Text header to parse.  It must be %NULL-terminated and writable. Its contents will be modified to directly
 *          embed the hash keys and/or values.  It must not be freed while the returned hash table is in use.
 ...
 *
 * Returns: A newly created hash table with values indexed by they keys found in the header.
 *
 * Since: 2.18
 **/
GHashTable*
                      ...
                      GError **error)
{
    ...
    const gchar *section_prefix = NULL, *section_suffix = NULL, *endsect_prefix = NULL, *endsect_suffix = NULL,
                **comments = NULL;
    guint section_prefix_len = 0, section_suffix_len = 0, endsect_prefix_len = 0, endsect_suffix_len = 0,
    ...
    g_return_val_if_fail(parser->section_template || !parser->endsection_template, NULL);
    g_return_val_if_fail(!parser->section_template == !parser->section_accessor, NULL);
    ...
    if (parser->comment_prefix) {
        ...
        if (strchr(parser->comment_prefix, '\n')) {
            ...
            for (j = 0, p = parser->comment_prefix-1; p; p = strchr(p+1, '\n'), j++)
                ;
            ncomments = j;
    ...
    context.hash = g_hash_table_new_full(g_str_hash, g_str_equal,
                                         parser->destroy_key ? parser->destroy_key : (free_keys ? g_free : NULL),
                                         parser->destroy_value ? parser->destroy_value : NULL);
    for (line = gwy_str_next_line(&header);
        ...
        /* Section ends -- before section starts beause they might look similar. */
        if (endsect_prefix
            ...
            line[len] = '\0';
            endsection = strip(line + endsect_prefix_len, len - endsect_prefix_len);

            if (!context.section) {
                make_noise(&context, GWY_TEXT_HEADER_ERROR_SECTION_END,
                           _("Section %s ended at line %u but it has never started."), endsection, context.lineno);
                continue;
            }
            ...
            if (parser->endsection && !parser->endsection(&context, context.section, user_data, &context.error))
                check_fatal_error(&context);

            context.section = NULL;
            continue;
        }
        else if (parser->endsection_template && gwy_strequal(line, parser->endsection_template)) {
            if (parser->endsection && !parser->endsection(&context, context.section, user_data, &context.error))
                check_fatal_error(&context);
            context.section = NULL;
            continue;
        ...
        if (section_prefix
            ...
            line[len] = '\0';
            newsection = strip(line + section_prefix_len, len - section_prefix_len);
            ...
            if (!*newsection) {
                make_noise(&context, GWY_TEXT_HEADER_ERROR_SECTION_NAME,
                           _("Empty section name at header line %u."), context.lineno);
                continue;
            }

            context.section = newsection;
            if (parser->section && !parser->section(&context, context.section, user_data, &context.error))
                check_fatal_error(&context);
            continue;
        }
        /* Comments */
        if (comments) {
            for (j = 0; j < ncomments; j++) {
                if (strncmp(line, comments[j], comments[j+1] - comments[j] - 1) == 0)
                    break;
            }
            if (j < ncomments)
                continue;
        }
        else if (parser->comment_prefix && strncmp(line, parser->comment_prefix, comment_prefix_len) == 0)
            continue;
        ...
        if (parser->line_prefix) {
            if (strncmp(line, parser->line_prefix, line_prefix_len) == 0) {
                ...
                make_noise(&context, GWY_TEXT_HEADER_ERROR_PREFIX,
                           _("Header line %u lacks prefix %s."), context.lineno, parser->line_prefix);
                continue;
            }
        }
        ...
        if (!value) {
            make_noise(&context, GWY_TEXT_HEADER_ERROR_GARBAGE,
                       _("Header line %u lacks key-value separator."), context.lineno);
            continue;
        }
        ...
        *(value++) = '\0';
        ...
        if (parser->section_template) {
            if (context.section)
                key = g_strconcat(context.section, parser->section_accessor, key, NULL);
            else
                key = g_strdup(key);
        }

        if (parser->item) {
            if (!parser->item(&context, context.hash, key, value, context.user_data, &context.error))
                check_fatal_error(&context);
        }
        else
    ...
    if (parser->terminator && !line && !context.error) {
        make_noise(&context, GWY_TEXT_HEADER_ERROR_TERMINATOR,
                   _("Header suddenly ended at line %u; end of header marker is missing"), context.lineno);
    }
 ...
 * @data: A data container.
 * @id: Data channel id.
 * @filetype: File type, i.e. the name of the function importing the data (without any "file::" prefix).  Since 2.38
 ...
 *
 * This is a convenience wrapper for gwy_app_channel_log_add().  The source id will be set to -1.  The file name will
 ...
 *
 * This is a convenience wrapper for gwy_app_volume_log_add().  The source id will be set to -1.  The file name will
 * be added to function arguments.
 *
 * Since: 2.35
 **/
 ...
 * @data: A data container.
 * @id: XYZ surface data id.
 * @filetype: File type, i.e. the name of the function importing the data (without any "file::" prefix).  It is
 *            possible to pass %NULL to fill the name of the currently running file type function automatically.
 * @filename: Name of the imported file.  If it is not valid UTF-8, it will be converted to UTF-8 using
 *            g_filename_to_utf8().  Failing even that, non-ASCII characters will be escaped.
 *
 * Logs the import of xyz data from third-party file.
 *
 * This is a convenience wrapper for gwy_app_xyz_log_add().  The source id will be set to -1.  The file name will be
 ...
 * Logs the import of volume data from third-party file.
 *
 * This is a convenience wrapper for gwy_app_curve_map_log_add().  The source id will be set to -1.  The file name
 ...
 * @include: app/gwymoduleutils-file.h
 *
 * Functions gwy_app_channel_check_nonsquare() and gwy_app_channel_title_fall_back() perform common tasks improving
 ...
 *
 * The group of functions gwy_get_gint16_le(), gwy_get_gint16_be(), etc. is intended to portably read packed binary
 ...
 *
 * As no buffer size is passed, obviously no buffer size checking is performed. The caller has to ensure the buffer is
 * large enough -- it is expected the caller checks the total buffer size before starting to parse it.
 *
 ...
 *
 * The data types used in @header do not matter (provided they are large enough to hold the values), the exact types
 * are determined by the functions used.  Therefore the reading would work identically if @header was defined using
 * common types:
 * <informalexample><programlisting>
 * struct {
 *     gint xres;
 ...
 *
 * Error domain for text header parsing. Errors in this domain will be from the #GwyTextHeaderError enumeration. See
...
/**
 * GwyTextHeaderError:
 * @GWY_TEXT_HEADER_ERROR_SECTION_NAME: Section name is invalid.  It is raised by the parser only for an empty section
 *                                      name.
 * @GWY_TEXT_HEADER_ERROR_SECTION_END: Section ended when a different section or no section was open.  Note that
 *                                     gwy_text_header_context_get_section() returns the section being closed at the
 *                                     time this error is raised.
 * @GWY_TEXT_HEADER_ERROR_SECTION_START: Section started before the previous ended.  This is raised only if
 ...
 * @GWY_TEXT_HEADER_ERROR_VALUE: Value is invalid.  This is never raised by the parser.
 * @GWY_TEXT_HEADER_ERROR_TERMINATOR: The text header has ended without being terminated by specified terminator.
 *
 * Error codes returned by text header parsing.
 *
 * Some errors, in particular %GWY_TEXT_HEADER_ERROR_KEY and %GWY_TEXT_HEADER_ERROR_VALUE are expected to be raised by
 ...
 *
 * #GwyTextHeaderContext represents the parsing state.  It is an opaque data structure and should be only manipulated
...
/**
 * GwyTextHeaderParser:
 * @comment_prefix: Prefix denoting comment lines.  It is possible to specify multiple prefixes by separating them
 *                  with newline ("\n").
 * @section_template: Section start template.  It must contain the character "\x1a" in the place where the section
 *                    name apprears. For example, "[Section \x1a]".
 * @endsection_template: Section end template.  It may or may not contain the substitute character "\x1a" depending on
 *                       whether the section end markers contain the section name.  It is invalid to set
 *                       @endsection_template without setting @section_template.  Example: "[Section \x1a End]".
 * @section_accessor: Glue to put between the section name and key when constructing hash table keys.  It is invalid
 *                    to set @section_accessor without setting @section_template. Typically, "::" is used.
 * @line_prefix: Mandatory prefix of each line.  Note this applies only to key-value lines.  The templates, such
 *               comments, sections and terminators, must still include @line_prefix if they start with it.
 * @key_value_separator: The string separating keys and values on each line. Typically, "=" or ":" is used.  When left
 *                       %NULL, whitespace plays the role of the separator.  Of course, keys cannot contain whitespace
 *                       then.
 * @terminator: Line that marks end of the header.  It is mandatory if specified, @GWY_TEXT_HEADER_ERROR_TERMINATOR is
 *              raised when the header ends sooner than @terminator is found.
 * @item: Callback called when a key-value pair is found.  If set it is responsible for inserting the pair to the hash
 *        table with g_hash_table_replace().  It is free to insert a different pair or nothing.  It must return %FALSE
 *        if it raises an error.
 * @section: Callback called when a section starts.  It must return %FALSE if it raises an error.
 * @endsection: Callback called when a section end.  It must return %FALSE if it raises an error.
 * @end: Callback called when the parsing finishes successfully, @length contains the length of parsed text in bytes.
 * @error: Callback called when an error occurs, including errors raised by other user callbacks.  If it returns %TRUE
 *         the error is considered fatal and the parsing terminates immediately.  If it is left unset no errors are
 *         fatal hence no errors are reported to the caller.
 * @destroy_key: Function to destroy keys, passed to g_hash_table_new_full(). It is invalid to set @destroy_key if
 *               @item callback is not set.
 * @destroy_value: Function to destroy values, passed to g_hash_table_new_full().  It is invalid to set @destroy_value
 *                 if @item callback is not set.
 *
 * Text header parser specification.
 *
 * Memory considerations: In general, the parser attempts to reuse the contents of @header directly for the hash keys
 * and values.  There are two cases when it cannot: sectioning implies that keys must be constructed dynamically and
 * the use of @item callback means the parser has no control on what is inserted into the hash table.
 *
 * This means that the @item callback must free @key if sectioning is used and it is not going to actually use it as
 * the hash table key.  And, of course, suitable @destroy_key and @destroy_value functions must be set.
 *
 * Since: 2.18
 **/
