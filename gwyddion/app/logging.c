/*
 *  $Id: logging.c 25432 2023-06-14 13:30:43Z yeti-dn $
...
enum {
    ALERT_LEVELS = G_LOG_LEVEL_ERROR | G_LOG_LEVEL_CRITICAL | G_LOG_LEVEL_WARNING,
    SEVERE_LEVELS = G_LOG_LEVEL_WARNING | G_LOG_LEVEL_CRITICAL,
    IGNORABLE_LEVELS = G_LOG_LEVEL_DEBUG | G_LOG_LEVEL_MESSAGE | G_LOG_LEVEL_INFO,
    STDERR_LEVELS = G_LOG_LEVEL_ERROR | G_LOG_LEVEL_CRITICAL | G_LOG_LEVEL_WARNING | G_LOG_LEVEL_MESSAGE,
};
 ...
 *
 * The log handler sends the messages to a log file or console, as Gwyddion usually does.  This function may not be
 * useful in Gwyddion-based programs unless they try to emulate Gwyddion behaviour closely.
 *
 * Since: 2.45
 **/
void
gwy_app_setup_logging(GwyAppLoggingFlags flags)
{
    static const gchar *our_domains[] = {
        "Gwyddion", "GwyProcess", "GwyDraw", "Gwydgets", "GwyModule", "GwyApp", "Module", "Gwyddion-Program", NULL
    };
    ...
    log_setup.message_history = g_array_new(FALSE, FALSE, sizeof(GwyAppLogMessage));
    log_setup.capturing_from = G_MAXUINT;
    /* NB: We must not initialise the text buffer here because Gtk+ may not be initialised yet.  Only do that on
     * demand. */

    for (i = 0; i < G_N_ELEMENTS(our_domains); i++)
        g_log_set_handler(our_domains[i], SEVERE_LEVELS | IGNORABLE_LEVELS, logger, &log_setup);
    for (i = 0; i < G_N_ELEMENTS(other_domains); i++)
        g_log_set_handler(other_domains[i], SEVERE_LEVELS, logger, &log_setup);
    /* Send messages at levels lower than WARNING to a black hole because the developers of the libraries think
     * printing DEBUG messages in production code is a good idea and hence they cannot be trusted.  We could make an
     * effort to pass some specifically to the default handler, but I do not care. */
    for (i = 0; i < G_N_ELEMENTS(other_domains); i++)
        g_log_set_handler(other_domains[i], IGNORABLE_LEVELS, black_hole, NULL);
}
...
_gwy_app_log_get_captured_messages(guint *nmesg)
{
    ...
    for (i = 0; i < n; i++) {
        messages[i] = g_array_index(message_history, GwyAppLogMessage, capturing_from + i);
...
flush_last_message(LoggingSetup *setup)
{
    ...
    g_string_printf(setup->last, "Last message repeated %u times", setup->last_count);
    format_log_message(setup->message, setup->last_domain, just_log_level, setup->last->str);
...
static void
logger_impl(const gchar *log_domain_str,
            GLogLevelFlags log_level,
            const gchar *message,
            gpointer user_data)
{
    ...
    GQuark log_domain = (log_domain_str ? g_quark_from_string(log_domain_str) : 0);
...
#pragma omp critical
#endif
    logger_impl(log_domain_str, log_level, message, user_data);
}
                 ...
                 GQuark log_domain, GLogLevelFlags log_level)
{
    ...
    if (setup->to_console) {
        if (!skip_annoying_message(message))
            emit_message_to_file((log_level & STDERR_LEVELS) ? stderr : stdout, log_level, message, TRUE);
    }
...
append_escaped_message(GString *str, const gchar *message)
{
    ...
    for (pos = good_from = 0; (c = (guchar)message[pos]); pos++) {
        /* First find out if this character breaks the sequence of characters we just copy as-is. */
        escape_this = FALSE;
        ...
        /* If it does not just increment the counter.  If we have to escape this one then first copy as-is the entire
         * good segment, and only after that add the bad character. */
        if (escape_this) {
            if (pos > good_from)
...
/*
 * Similar to GLib's function but we do not handle recursive and fatal errors so we do not have to handle the
 * difficult cases and can use GLib functions.
 */
static void
...
skip_annoying_message(const gchar *message)
{
    static const gchar *skip_message_prefixes[] = {
        "GtkSourceView-WARNING: Cannot create a regex for all the transitions, the syntax highlighting process "
 ...
 * messages occurring between gwy_app_setup_logging() and this function call.  New messages will be appended to the
...
{
    if (!log_setup.message_history) {
        g_warning("Obtaining program log text buffer requires gwy_app_setup_logging() being called first.");
    ...
    if (!log_setup.textbuf) {
        ...
        for (i = 0; i < message_history->len; i++) {
            GwyAppLogMessage *message = &g_array_index(message_history, GwyAppLogMessage, i);
            _gwy_app_log_add_message_to_textbuf(log_setup.textbuf, message->message, message->log_level);
        }
    }
                                    ...
                                    GLogLevelFlags log_level)
{
    ...
    if (tagname)
        gtk_text_buffer_insert_with_tags_by_name(textbuf, &iter, message, -1, tagname, NULL);
    else
...
/**
 * GwyAppLoggingFlags:
 * @GWY_APP_LOGGING_TO_FILE: Messages go to a log file, either gwyddion.log or given by environment variable
 *                           GWYDDION_LOGFILE.
 * @GWY_APP_LOGGING_TO_CONSOLE: Messages go to standard output and standard error depending on message type (emulating
