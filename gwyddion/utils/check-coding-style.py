...
def check_file(filename, lines):
    ...
    token_checks = [
        ...
        check_brace_padding,
        check_misaligned_parenthesisation,
...
def check_long_lines(lines, warnings):
    ...
    for i, l in enumerate(lines):
        if len(l.decode('utf-8')) > 120 and not l.startswith('/* vim: ') and not l.startswith('#define' ):
            w = 'Line longer than 120 characters: %s'
...
def check_brace_padding(tokens, lines, warnings):
    "Check for empty lines on the inner side of { }"
    for t in tokens:
        if t.typ != Token.punct:
            continue
        if t.string == '{' and t.succ.line > t.line+1 and not lines[t.line+1]:
            w = 'Empty line after opening brace (col %d)'
            warnings.append((t.line, w % t.col))
        if t.string == '}' and t.prec.line < t.line-1 and not lines[t.line-1]:
            w = 'Empty line before closing brace (col %d)'
            warnings.append((t.line, w % t.col))

def check_misaligned_parenthesisation(tokens, lines, warnings):
    "Check for parentheses content not aligned with the opening parenthesis"
    for i, t in enumerate(tokens):
        if t.typ != Token.punct or t.string != '(':
            continue
        if t.parenlevel > 0 or t.matching.line == t.line:
            continue
        stack = [t]
        j = i+1
        while stack:
            tt = tokens[j]
            if tt.typ == Token.punct and tt.string == '(':
                stack.append(tt)
            elif tt.typ == Token.punct and tt.string == ')':
                del stack[-1]
            elif tt.prec.line < tt.line:
                if tt.col != stack[-1].col + 1:
                    w = 'Misaling parentheses content (col %d)'
                    warnings.append((tt.line, w % tt.col))
            j += 1
    ...
    for t in tokens:
        ...
        if t.string in ('for', 'while', 'if'):
            ...
            if succ.line > m.line:
                continue
            w = 'Statement for `%s\' on the same line (col %d): %s'
            warnings.append((succ.line, w % (t.string, succ.col, lines[succ.line])))
            continue
...
def check_eol_operators(tokens, lines, warnings):
    ...
    # There is a reasonable use case: multiline strings where we end the variable declaration line with =.
    oplist = '&&', '||', '+', '-', '*', '/', '%', '|', '&', '^', '==', '!=', '<', '>', '<=', '>=', '?', '='
    operators = dict([(x, 1) for x in oplist])
    for t in tokens:
        if t.bracelevel == 0:
...
def check_return_case_parentheses(tokens, lines, warnings):
    ...
    for i, t in enumerate(tokens):
        ...
        if t.prec.string not in keywords:
            continue
        if t.matching.line != t.line:
            continue
        if t.matching.succ.typ != Token.punct or t.matching.succ.string != ';':
            continue
        contains_conditional_expr = False
        for tt in tokens[i+1:]:
            if tt.typ == Token.punct and tt.string == '?':
                contains_conditional_expr = True
                break
            if tt == t.matching:
                break
        if contains_conditional_expr:
            continue
        w = 'Extra %s parentheses (col %d)'
        warnings.append((t.line, w % (t.prec.string, t.col)))

def check_conditional_expr_parentheses(tokens, lines, warnings):
    # XXX: Too crude.  Disabled.
    for t in tokens:
        if t.typ != Token.punct or t.string != '?':
            continue
        if t.parenlevel:
            continue
        w = 'Conditional expression should be parenthesised for clarity (col %d)'
        warnings.append((t.line, w % (t.col)))
...
def check_boolean_comparisons(tokens, lines, warnings):
    ...
    for t in tokens:
        ...
        if prec.typ != Token.punct or (prec.string != '!='
                                       and prec.string != '=='):
            continue
        w = 'Comparison to TRUE or FALSE (col %d); use if (x), if (!x)'
        warnings.append((t.line, w % (t.col)))
...
def tokenize(lines, warnings):
    ...
    # permit identifiers starting @ for Objective-C stuff in gwyddion/
    ...
    re_grp = re.compile(r'&&|<<|>>|->|::|\.\*|->\*|\|\||\+\+|--|\*\*+|\.\.\.')
    re_sin = re.compile(r'[][(){};:?,.+~!%^&*|/^<>]|-')
    re_tokens = re_com, re_str, re_chr, re_id, re_dbl, re_int, re_eq, re_grp, re_sin
    token_ids = {
...
# vim: set columns=120 tw=118 et ts=4 sw=4
