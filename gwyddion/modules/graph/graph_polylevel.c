/*
 *  $Id: graph_polylevel.c 26117 2024-01-07 17:44:59Z yeti-dn $
...
static void             graph_polylevel     (GwyGraph *graph);
...
static void
graph_polylevel(GwyGraph *graph)
{
...
execute(ModuleArgs *args, GwyResults *results)
{
    ...
    param = gwy_math_fit_polynom(n, xdata + fitstart, ydata + fitstart, degree, param);
        ...
        if (output_type == OUTPUT_DATA_FIT) {
           for (j = 0; j < ndata; j++) {
            ...
            for (j = 0; j < ndata; j++) {
                nxdata[j] = xdata[j];
                x = nxdata[j];
                nydata[j] = ydata[j] - (param[0] + param[1]*x + param[2]*x*x + param[3]*x*x*x
    ...
    if (output_type == OUTPUT_DATA_FIT) {
        ...
        for (j = 0; j < ndata; j++) {
           ...
           x = nxdata[j];
           nydata[j] = param[0] + param[1]*x + param[2]*x*x + param[3]*x*x*x
                                              + param[4]*x*x*x*x + param[5]*x*x*x*x*x;
        }
...
create_results(GwySIUnit *xunit, GwySIUnit *yunit)
{
    ...
    gwy_results_add_value(results, "p2", N_("P2"), "power-x", -2, "power-z", 2, NULL);
    gwy_results_add_value(results, "p3", N_("P3"), "power-x", -3, "power-z", 3, NULL);
    gwy_results_add_value(results, "p4", N_("P4"), "power-x", -4, "power-z", 4, NULL);
    gwy_results_add_value(results, "p5", N_("P5"), "power-x", -5, "power-z", 5, NULL);
