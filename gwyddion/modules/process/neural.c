/*
 *  $Id: neural.c 25638 2023-09-07 12:23:28Z yeti-dn $
                ...
                NeuralTrainArgs *args)
{
    ...
    gwy_app_sync_data_items(modeldata, mydata, args->tmodel.id, 0, FALSE,
    ...
    gwy_app_sync_data_items(signaldata, mydata, args->tsignal.id, 1, FALSE,
                            GWY_DATA_ITEM_REAL_SQUARE,
                            GWY_DATA_ITEM_GRADIENT,
                            GWY_DATA_ITEM_RANGE_TYPE,
                            GWY_DATA_ITEM_RANGE,
    ...
    gwy_container_pass_object_by_name(mydata, "/2/data", result);
    gwy_app_sync_data_items(signaldata, mydata, args->tsignal.id, 2, FALSE,
                            GWY_DATA_ITEM_REAL_SQUARE,
                            GWY_DATA_ITEM_GRADIENT,
                            GWY_DATA_ITEM_RANGE_TYPE,
                            GWY_DATA_ITEM_RANGE,
    ...
    gwy_container_pass_object_by_name(mydata, "/3/data", diff);
    gwy_app_sync_data_items(signaldata, mydata, args->tsignal.id, 3, FALSE,
                            GWY_DATA_ITEM_REAL_SQUARE,
                            GWY_DATA_ITEM_GRADIENT,
                            0);
}
...
neural_train_dialog(NeuralTrainArgs *args)
{
    ...
    controls.view = gwy_create_preview(mydata, 0, PREVIEW_SMALL_SIZE, FALSE);
    ...
    filtermodel = gwy_create_inventory_model_without_default(controls.store);
                   ...
                   GwyDataChooser *chooser)
{
    ...
    gwy_set_data_preview_size(GWY_DATA_VIEW(controls->view), PREVIEW_SMALL_SIZE);
    gtk_widget_set_sensitive(gwy_radio_buttons_find(group, PREVIEW_RESULT), FALSE);
    gtk_widget_set_sensitive(gwy_radio_buttons_find(group, PREVIEW_DIFFERENCE), FALSE);
...
outunits_changed(NeuralTrainControls *controls, GtkEntry *entry)
{
    ...
    if (controls->in_update)
        return;
    gwy_assign_string(&nndata->outunits, gtk_entry_get_text(entry));
    GWY_RESOURCE(controls->args->nn)->is_modified = TRUE;
}
...
train_network(NeuralTrainControls *controls)
{
    ...
    tmodel = GWY_DATA_FIELD(gwy_container_get_object_by_name(controls->mydata, "/0/data"));
    tsignal = GWY_DATA_FIELD(gwy_container_get_object_by_name(controls->mydata, "/1/data"));
    result = GWY_DATA_FIELD(gwy_container_get_object_by_name(controls->mydata, "/2/data"));
    diff = GWY_DATA_FIELD(gwy_container_get_object_by_name(controls->mydata, "/3/data"));
    ...
    ok = (train_do(nn, errors, gcmodel, tmodel, tsignal, mask, args->masking, sfactor, sshift, args->trainsteps)
    ...
    if (!ok) {
        gtk_label_set_text(GTK_LABEL(controls->message), _("Training was canceled."));
        return;
    }

    gwy_data_field_subtract_fields(diff, result, tsignal);
...
network_store(NeuralTrainControls *controls)
{
    ...
    gwy_select_in_filtered_inventory_treeeview(GTK_TREE_VIEW(controls->networklist), name);
}
...
network_rename(NeuralTrainControls *controls)
{
    ...
    gwy_select_in_filtered_inventory_treeeview(GTK_TREE_VIEW(controls->networklist), newname);
}
...
neural_train_sanitize_args(NeuralTrainArgs *args)
{
    ...
    args->masking = gwy_enum_sanitize_value(args->masking, GWY_TYPE_MASKING_TYPE);
                       ...
                       NeuralTrainArgs *args)
{
    ...
    gwy_container_gis_int32_by_name(settings, trainsteps_key, &args->trainsteps);
                       ...
                       NeuralTrainArgs *args)
{
    ...
    gwy_container_set_int32_by_name(settings, trainsteps_key, args->trainsteps);
...
neural_apply_sanitize_args(NeuralApplyArgs *args)
{
    if (!gwy_inventory_get_item(gwy_neural_networks(), args->name)) {
        gwy_assign_string(&args->name, GWY_NEURAL_NETWORK_UNTITLED);
    }
                       ...
                       NeuralApplyArgs *args)
{
    *args = neural_apply_defaults;
    ...
    gwy_container_gis_string_by_name(settings, name_key, (const guchar**)&args->name);
    gwy_container_gis_boolean_by_name(settings, scale_output_key, &args->scale_output);
...
neural_apply_save_args(GwyContainer *settings,
                       NeuralApplyArgs *args)
{
    gwy_container_set_string_by_name(settings, name_key, g_strdup(args->name));
    gwy_container_set_boolean_by_name(settings, scale_output_key, args->scale_output);
}
