/*
 *  $Id: mfm_current.c 25126 2022-10-23 08:45:21Z klapetek $
...
static void
mfm_current(GwyContainer *data, GwyRunType runtype)
{
...
execute(ModuleArgs *args)
{
    ...
    gdouble height = gwy_params_get_double(params, PARAM_HEIGHT)*1e-9,
            ...
            position = gwy_params_get_double(params, PARAM_POSITION)*gwy_data_field_get_xreal(result)/100;
    ...
    gwy_data_field_mfm_current_line(result, height, width, position, current, component);
    ...
    if (out == GWY_MFM_CURRENT_OUTPUT_FORCE
        ...
        gwy_data_field_mfm_perpendicular_medium_force(tmp, result, probe, mtip, bx, by, length);
        g_object_unref(tmp);
    }
}
