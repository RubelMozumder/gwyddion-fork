/*
 *  $Id: flatten_base.c 26234 2024-03-01 13:38:24Z yeti-dn $
...
find_base_peak(GwyDataField *dfield, gdouble *mean, gdouble *rms)
{
    ...
    fitter = gwy_nlfit_preset_fit(gaussian, NULL, ndata, xdata, ydata, param, NULL, NULL);
                    ...
                    gdouble mean, gdouble rms)
{
    ...
    gwy_debug("min %g, max %g, threshold %g => threshval %g", min, max, threshold, threshval);
...
flatten_base(GwyContainer *data, GwyRunType run)
{
    ...
    gwy_app_wait_start(gwy_app_find_window_for_channel(data, id), _("Facet-leveling..."));

    dfield = gwy_data_field_duplicate(origfield);
    found_peak = find_base_peak(dfield, &mean, &sigma);
    gwy_debug("initial peak: %s (mean=%g, rms=%g)", found_peak ? "OK" : "NOT FOUND", mean, sigma);
    ...
    for (i = 0; i < 5; i++) {
        ...
        found_peak = find_base_peak(dfield, &mean, &sigma);
        gwy_debug("facet[%d] peak: %s (mean=%g, rms=%g)", i, found_peak ? "OK" : "NOT FOUND", mean, sigma);
        if (!found_peak)
            break;
    ...
    for (i = 2; i <= 5; i++) {
        ...
        found_peak = find_base_peak(dfield, &mean, &sigma);
        gwy_debug("poly[%d] peak: %s (mean=%g, rms=%g)", i, found_peak ? "OK" : "NOT FOUND", mean, sigma);
        if (!found_peak)
            break;
