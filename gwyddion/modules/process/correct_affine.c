/*
 *  $Id: correct_affine.c 26039 2023-11-24 12:06:54Z yeti-dn $
...
correct_affine(GwyContainer *data, GwyRunType run)
{
    ...
    for (i = 0; all_channels[i] != -1; i++) {
        ...
        gwy_container_pass_object_by_name(data, s, selection);
        g_free(s);
                        ...
                        GwyDataField *dfield)
{
    ...
    gwy_container_pass_object_by_name(controls->mydata, "/1/data/full", acf);
    ...
    gwy_container_pass_object_by_name(controls->mydata, "/1/hacf", hacf);
    ...
    gwy_container_pass_object_by_name(controls->mydata, "/1/hacf-fixed", hacf);
    g_object_unref(mask);
    g_object_unref(mid);
...
do_correction(AffcorControls *controls)
{
    ...
    corrected = create_corrected_dfield(dfield, a1a2, a1a2_corr,
                                        args->interp, args->scaling);
    gwy_container_pass_object_by_name(controls->mydata, "/2/data", corrected);
    gwy_set_data_preview_size(GWY_DATA_VIEW(controls->view), PREVIEW_SIZE);
