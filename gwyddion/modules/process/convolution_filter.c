/*
 *  $Id: convolution_filter.c 25638 2023-09-07 12:23:28Z yeti-dn $
...
convolution_filter_create_filter_tab(ConvolutionControls *controls)
{
    ...
    table = gtk_table_new(1 + nsizes, 1, FALSE);
...
{
    gwy_resource_delete(GWY_RESOURCE(controls->args->preset));
}
                                      ...
                                      const gchar *text)
{
    ...
    if (gwy_resource_rename(resource, text)) {
        gwy_inventory_store_get_iter(controls->presets, text, &iter);
        gtk_tree_selection_select_iter(controls->selection, &iter);
    }
}
...
convolution_filter_preview(ConvolutionControls *controls)
{
    ...
    if (!gwy_container_gis_object_by_name(controls->mydata, "/1/data",
        ...
        gwy_container_pass_object_by_name(controls->mydata, "/0/data", preview);
    }
    else {
