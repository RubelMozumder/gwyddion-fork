/*
 *  $Id: polydistort.c 25638 2023-09-07 12:23:28Z yeti-dn $
                   ...
                   gint id)
{
    ...
    if (mfield) {
        ...
        args->interp = interp;
        gwy_container_pass_object(data, gwy_app_get_mask_key_for_id(newid), result);
    }

    if (sfield) {
        result = gwy_data_field_duplicate(sfield);
        distort_do(args, sfield, result);
        gwy_container_pass_object(data, gwy_app_get_show_key_for_id(newid), result);
    }
}
