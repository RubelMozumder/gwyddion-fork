/*
 *  $Id: grainmeasure.c 24920 2022-08-23 15:40:11Z yeti-dn $
...
gwy_tool_grain_measure_finalize(GObject *object)
{
    ...
    GWY_SI_VALUE_FORMAT_FREE(tool->vf);
             ...
             gpointer user_data)
{
    ...
    if (tool->gno <= 0 || !gvalue) {
        ...
        GWY_OBJECT_UNREF(gvalue);
        return;
    }

    g_object_unref(gvalue);
    if (!tool->same_units && (gwy_grain_value_get_flags(gvalue) & GWY_GRAIN_VALUE_SAME_UNITS)) {
    ...
    if (gwy_grain_value_get_flags(gvalue) & GWY_GRAIN_VALUE_IS_ANGLE) {
        ...
        field = GWY_PLAIN_TOOL(tool)->data_field;
        siunitxy = gwy_data_field_get_si_unit_xy(field);
        siunitz = gwy_data_field_get_si_unit_z(field);
        tool->siunit = gwy_si_unit_power_multiply(siunitxy, gwy_grain_value_get_power_xy(gvalue),
                                                  siunitz, gwy_grain_value_get_power_z(gvalue),
                                                  tool->siunit);
        tool->vf = gwy_si_unit_get_format_with_digits(tool->siunit, style, value, 3, tool->vf);
...
recalculate(GwyToolGrainMeasure *tool)
{
    ...
    if (!tool->grains) {
        tool->grains = g_new0(gint, gwy_data_field_get_xres(field) *gwy_data_field_get_yres(field));
    ...
    gwy_grain_values_calculate(n, gvalues, (gdouble**)tool->values->pdata, field, tool->ngrains, tool->grains);
    g_free(gvalues);
}
