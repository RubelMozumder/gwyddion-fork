/*
 *  $Id: rprofile.c 25221 2023-02-21 09:38:06Z yeti-dn $
...
static void         gwy_tool_rprofile_response         (GwyTool *gwytool,
                                                        gint response_id);
...
static void         param_changed                      (GwyToolRprofile *tool,
                                                        gint id);
static void         update_symm_sensitivty             (GwyToolRprofile *tool);
static void         update_curve                       (GwyToolRprofile *tool,
                                                        gint i);
static void         update_all_curves                  (GwyToolRprofile *tool);
static void         improve_all                        (GwyToolRprofile *tool);
static void         improve                            (GwyToolRprofile *tool);
static void         symmetrize_profile                 (GwyToolRprofile *tool,
                                                        gint id);
...
static void         render_color                       (GtkCellLayout *layout,
                                                        ...
                                                        GtkTreeIter *iter,
                                                        gpointer user_data);
static gdouble      angular_average_mismatch           (GwyDataField *data_field,
                                                        GwyDataField *mask,
...
define_module_params(void)
{
    ...
    gwy_param_def_set_function_name(paramdef, "rprofile");
...
{
    GwyToolRprofile *tool = GWY_TOOL_RPROFILE(object);
...
gwy_tool_rprofile_init_dialog(GwyToolRprofile *tool)
{
    ...
    g_object_set(tool->gmodel, "title", _("Radial profiles"), "label-visible", FALSE, NULL);
    ...
    gwy_param_table_append_button(table, BUTTON_IMPROVE, -1, RESPONSE_IMPROVE, _("S_ymmetrize"));
    gwy_param_table_append_button(table, BUTTON_IMPROVE_ALL, BUTTON_IMPROVE,
                                  RESPONSE_IMPROVE_ALL, _("Symmetrize _All"));
...
gwy_tool_rprofile_data_switched(GwyTool *gwytool, GwyDataView *data_view)
{
    ...
    GwyToolRprofile *tool = GWY_TOOL_RPROFILE(gwytool);
    gboolean ignore = (data_view == plain_tool->data_view);

    GWY_TOOL_CLASS(gwy_tool_rprofile_parent_class)->data_switched(gwytool, data_view);
...
static void
gwy_tool_rprofile_response(GwyTool *gwytool, gint response_id)
{
    GwyToolRprofile *tool = GWY_TOOL_RPROFILE(gwytool);

    GWY_TOOL_CLASS(gwy_tool_rprofile_parent_class)->response(gwytool, response_id);

    if (response_id == GTK_RESPONSE_APPLY)
        gwy_tool_rprofile_apply(tool);
...
{
    update_all_curves(GWY_TOOL_RPROFILE(plain_tool));
    gwy_param_table_data_id_refilter(GWY_TOOL_RPROFILE(plain_tool)->table, PARAM_TARGET_GRAPH);
}
...
static void
param_changed(GwyToolRprofile *tool, gint id)
{
    GwyPlainTool *plain_tool = GWY_PLAIN_TOOL(tool);
...
static void
update_symm_sensitivty(GwyToolRprofile *tool)
{
...
{
    GwyDataField *dfield = GWY_PLAIN_TOOL(tool)->data_field;
    gint xl1, xl2, yl1, yl2, lineres;

    if (gwy_params_get_boolean(tool->params, PARAM_FIXRES))
        return gwy_params_get_int(tool->params, PARAM_RESOLUTION);
    ...
    lineres = MAX(lineres, 4);

    return lineres;
}

static void
update_curve(GwyToolRprofile *tool, gint i)
{
    GwyPlainTool *plain_tool = GWY_PLAIN_TOOL(tool);
    ...
    r = fmax(r, hypot(gwy_data_field_get_dx(data_field), gwy_data_field_get_dy(data_field)));
    gwy_data_field_angular_average(data_field, tool->line, mask, masking, xc, yc, r, lineres);
...
improve(GwyToolRprofile *tool)
{
    ...
    symmetrize_profile(tool, indices[0]);
    gwy_app_wait_cursor_finish(GTK_WINDOW(GWY_TOOL(tool)->dialog));
    gtk_tree_path_free(path);
}

static void
improve_all(GwyToolRprofile *tool)
{
    GwyPlainTool *plain_tool = GWY_PLAIN_TOOL(tool);
    gint n, i;
    ...
    gwy_app_wait_cursor_start(GTK_WINDOW(GWY_TOOL(tool)->dialog));
    for (i = 0; i < n; i++)
        symmetrize_profile(tool, i);
    gwy_app_wait_cursor_finish(GTK_WINDOW(GWY_TOOL(tool)->dialog));
}

static void
update_all_curves(GwyToolRprofile *tool)
{
    GwyPlainTool *plain_tool = GWY_PLAIN_TOOL(tool);
    gint n, i;
...
#pragma omp parallel if(gwy_threads_are_enabled()) default(none) \
...
#endif
    {
        ...
        for (ij = ijfrom; ij < ijto; ij++) {
            ...
            allvar[ij] = calculate_angular_mismatch(dfield, mask, masking, offline, lineres);
        }
    }
...
static void
symmetrize_profile(GwyToolRprofile *tool, gint id)
{
    GwyPlainTool *plain_tool = GWY_PLAIN_TOOL(tool);
    GwyDataField *dfield = GWY_PLAIN_TOOL(tool)->data_field;
    GwyDataField *mask = plain_tool->mask_field;
    ...
    g_return_if_fail(plain_tool->selection);
    if (!gwy_selection_get_object(plain_tool->selection, id, line_fine))
        g_return_if_reached();
    ...
    if (hypot((line_fine[2] - line_fine[0])/dx, (line_fine[3] - line_fine[1])/dy) < 4.0)
        return;
    h = hypot(line_fine[2] - line_fine[0], line_fine[3] - line_fine[1]);

    r = 0.07*h;
    while (!optimize_profile_at_scale(dfield, mask, masking, r, line_coarse, lineres, &mismatch_coarse))
        r *= 0.25;

    r = 0.015*h;
    while (!optimize_profile_at_scale(dfield, mask, masking, r, line_fine, lineres, &mismatch_fine))
        r *= 0.25;
