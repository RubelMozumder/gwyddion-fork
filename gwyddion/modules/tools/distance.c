/*
 *  $Id: distance.c 26100 2024-01-04 14:09:03Z yeti-dn $
...
gwy_tool_distance_init_dialog(GwyToolDistance *tool)
{
    ...
    gtk_box_pack_start(GTK_BOX(dialog->vbox), gwy_param_table_widget(table), FALSE, TRUE, 0);
...
gwy_tool_distance_data_switched(GwyTool *gwytool, GwyDataView *data_view)
{
    ...
    if (data_view) {
        ...
        tool->has_calibration = (gwy_container_gis_object_by_name(plain_tool->container, xukey, &tool->xunc)
...
static gdouble
get_uncertainty(GwyDataField *uncfield, const gdouble *coords)
{
    gdouble unc;

    if (!uncfield)
        return 0.0;
    unc = gwy_data_field_get_dval_real(uncfield, coords[0], coords[1], GWY_INTERPOLATION_BILINEAR);
    unc *= unc;
    unc += (gwy_data_field_get_dval_real(uncfield, coords[2], coords[3], GWY_INTERPOLATION_BILINEAR)
            * gwy_data_field_get_dval_real(uncfield, coords[2], coords[3], GWY_INTERPOLATION_BILINEAR));
    return sqrt(unc);
}
            ...
            gpointer user_data)
{
    ...
    if (id == COLUMN_DX) {
        ...
        val = line[2] - line[0];
        unc = get_uncertainty(tool->xunc, line);
    }
    else if (id == COLUMN_DY) {
        vf = plain_tool->coord_format;
        val = line[3] - line[1];
        unc = get_uncertainty(tool->yunc, line);
    }
    else if (id == COLUMN_R) {
        vf = plain_tool->coord_format;
        ...
        unc = get_uncertainty(tool->zunc, line);
    }
    else {
        g_return_if_reached();
    }
...
format_report(gpointer user_data)
{
    ...
    if (!(report_style & GWY_RESULTS_REPORT_MACHINE)) {
        ...
        vf_dz = gwy_si_unit_get_format(zunit, style, fmax(fabs(dx), fabs(dy))/120.0, NULL);
