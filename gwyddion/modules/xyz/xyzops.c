/*
 *  $Id: xyzops.c 25938 2023-10-24 13:27:32Z yeti-dn $
...
#define XYZAVERAGE_RUN_MODES (GWY_RUN_IMMEDIATE)
...
static void     xyzmerge          (GwyContainer *data,
                                   GwyRunType run);
static void     xyzaverage        (GwyContainer *data,
                                   GwyRunType run);
...
static void     xyzaverage_do     (GwySurface *surface,
                                   GwyContainer *data,
                                   const XYZAverageArgs *args);
...
module_register(void)
{
    ...
    gwy_xyz_func_register("xyz_average",
                          (GwyXYZFunc)&xyzaverage,
                          N_("/_Average Identical Positions"),
                          NULL,
                          XYZAVERAGE_RUN_MODES,
                          GWY_MENU_FLAG_XYZ,
                          N_("Average points with identical XY positions"));
...
static void
xyzaverage(GwyContainer *data, GwyRunType run)
{
    XYZAverageArgs args;
    GwySurface *surface = NULL;

    g_return_if_fail(run & XYZAVERAGE_RUN_MODES);
    ...
    xyzaverage_do(surface, data, &args);
}
            ...
            const XYZMergeArgs *args)
{
    ...
    if (gwy_container_gis_string(data, gwy_app_get_surface_palette_key_for_id(args->op1.id), &gradient))
...
static void
xyzaverage_do(GwySurface *surface, GwyContainer *data,
              const XYZAverageArgs *args)
{
    GwySurface *out;
    const GwyXYZ *xyz1;
    GwyXYZ *xyz;
    guint n, i, pos, bstart, len;
    gint newid;
    const guchar *gradient;
    ...
    newid = gwy_app_data_browser_add_surface(out, data, TRUE);
    gwy_app_set_surface_title(data, newid, _("Points averaged"));
