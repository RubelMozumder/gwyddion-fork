/*
 *  $Id: xyz_drift.c 26118 2024-01-08 10:07:11Z yeti-dn $
...
xyzdrift(GwyContainer *data, GwyRunType run)
{
    ...
    g_free(ids);
    if (!tsfound) { //create timestamp channel here
       rdata.timesurface = timesurface = gwy_surface_duplicate(surface);
       rdata.timepoints = rdata.timesurface->data;
       rdata.ntimepoints = rdata.timesurface->n;
       for (i=0; i<rdata.ntimepoints; i++) {
           rdata.timepoints[i].z = i;
       }
    }
              ...
              gint id)
{
    ...
    gwy_container_pass_object_by_name(controls.mydata, "/0/data", dfield);
                  ...
                  gint row)
{
    ...
    vf = gwy_surface_get_value_format_xy(surface, GWY_SI_UNIT_FORMAT_VFMARKUP, NULL);
