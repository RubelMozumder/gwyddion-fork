/*
 *  $Id: volume_arithmetic.c 26301 2024-05-04 15:51:03Z yeti-dn $
...
module_register(void)
{
    gwy_volume_func_register("volume_arithmetic",
                             ...
                             N_("/_Basic Operations/_Arithmetic..."),
...
define_module_params(void)
{
    ...
    for (i = 0; i < NARGS; i++) {
        gchar *s = g_strdup_printf("volume%u", i);
        gwy_param_def_add_volume_id(paramdef, PARAM_VOLUME + i, s, NULL);
    }
    return paramdef;
}
        ...
        ModuleArgs *args, EvaluationData *evdata)
{
    ...
    gui.dialog = gwy_dialog_new(_("Arithmetic"));
    ...
    for (i = 0; i < NARGS; i++) {
        ...
        dataid = gwy_params_get_data_id(args->params, PARAM_VOLUME + i);
...
data_chosen(ModuleGUI *gui, GwyDataChooser *chooser)
{
    ...
    gwy_params_set_volume_id(args->params, PARAM_VOLUME + i, dataid);
    gwy_dialog_invalidate(GWY_DIALOG(gui->dialog));
    if (!(gui->evdata->err & ARITHMETIC_EXPR))
        show_state(gui, NULL);
}
...
expr_changed(ModuleGUI *gui, GtkWidget *entry)
{
    ...
    if (gwy_expr_compile(evdata->expr, expr, &error)) {
        ...
        if (nvars == 1) {
            ...
            evdata->err = ARITHMETIC_NUMERIC;
        }
        else {
            if (gwy_expr_resolve_variables(evdata->expr,
                                           ARITHMETIC_NARGS, (const gchar*const*)evdata->name, evdata->pos)) {
                evdata->err = ARITHMETIC_EXPR;
...
show_state(ModuleGUI *gui, const gchar *message)
{
    ...
    if (!message && evdata->err != ARITHMETIC_NUMERIC) {
...
check_bricks(ModuleArgs *args, EvaluationData *evdata)
{
    ...
    if (evdata->err & (ARITHMETIC_EXPR | ARITHMETIC_NUMERIC))
        return NULL;
    ...
    if (i == NARGS) {
        /* no variables */
        evdata->err &= ~ARITHMETIC_DATA;
        return NULL;
    }

    /* each window must match with first, this is transitive */
    bfirst = gwy_params_get_volume(args->params, PARAM_VOLUME + first);
    for (i = first+1; i < NARGS; i++) {
        if (!nd[i])
            continue;

        brick = gwy_params_get_volume(args->params, PARAM_VOLUME + i);
...
preview(gpointer user_data)
{
    ...
    if (gui->evdata->err != ARITHMETIC_OK)
        return;
...
execute(ModuleArgs *args, EvaluationData *evdata)
{
    ...
    g_return_if_fail(evdata->err == ARITHMETIC_OK);
    ...
    for (i = 0; i < NARGS; i++) {
        ...
        brick = data_bricks[i] = gwy_params_get_volume(args->params, PARAM_VOLUME + i);
...
sanitise_params(ModuleArgs *args)
{
    ...
    dataid = gwy_params_get_data_id(params, PARAM_VOLUME);
    for (i = 1; i < NARGS; i++) {
        /* Replace lost volumes with d1. */
        if (!gwy_params_get_volume(params, PARAM_VOLUME + i))
            gwy_params_set_volume_id(params, PARAM_VOLUME + i, dataid);
    }
