/*
 *  $Id: volume_fdfit.c 25241 2023-02-23 16:00:42Z klapetek $
...
volfit_do(VolfitControls *controls)
{
    ...
    gboolean *fixed;
    gint xres, yres, newid, i, j, k, m, nparams, nfree = 0;
    ...
    for (i = 0; i < nfree; i++)
        result[i] = gwy_data_field_new_alike(args->dfield, TRUE);

    eresult = g_new(GwyDataField*, nfree);
    for (i = 0; i < nfree; i++)
        eresult[i] = gwy_data_field_new_alike(args->dfield, TRUE);
    ...
    xres = gwy_brick_get_xres(args->brick);
    yres = gwy_brick_get_yres(args->brick);
    ...
    for (i = 0; i < xres; i++) {
        for (j = 0; j < yres; j++) {
            ...
            if (errorknown) {
                gwy_data_field_set_val(chresult, i, j,
...
fail:
    ...
    g_free(result);
    for (i = 0; i < nfree; i++)
        g_object_unref(eresult[i]);
    g_free(eresult);

    g_object_unref(cresult);
    g_object_unref(chresult);
}
