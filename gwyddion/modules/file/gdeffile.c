/*
 *  $Id: gdeffile.c 25675 2023-09-18 05:41:59Z yeti-dn $
 *  Copyright (C) 2010-2020 David Necas (Yeti).
...
typedef struct {
    ...
    guint version;
    guchar alignment[2];
    /* On-disk this is ‘32bit time_t’.  We assume it can be safely assigned to a larger integer when actual time_t is
     * larger. */
                 ...
                 GError **error)
{
    ...
    get_CHARARRAY(header->alignment, p);
    header->creation_time = (time_t)gwy_get_guint32_le(p);
                        ...
                        GError **error)
{
    if (size < CONTROL_BLOCK_SIZE) {
        err_TRUNCATED_PART(error, "GDEF ControlBlock");
        return FALSE;
    }
                   ...
                   GError **error)
{
    if (size < VARIABLE_SIZE) {
        err_TRUNCATED_PART(error, "GDEF Variable");
        return FALSE;
    }
    ...
    do {
        ...
        for (i = 0; i < block->n_variables; i++) {
            ...
            if (var->type == VAR_DATABLOCK) {
                ...
                ok = gdef_read_variable_data(nestedblock, p, size - (*p - buffer), depth+1, error);
                ...
                *p += var->size;
                if (*p > buffer + size) {
                    if (error) {
                        gchar *s = g_strconcat("GDEF Variable ", var->name, NULL);
                        err_TRUNCATED_PART(error, s);
                        g_free(s);
                    }
                    return FALSE;
                }
                         ...
                         GError **error)
{
    ...
    do {
        ...
        for (i = 0; i < block->n_variables; i++) {
            ...
            if (!gdef_read_variable(block->variables + i, p, size - (*p - buffer), error))
                goto fail;

            if (var->type == VAR_DATABLOCK) {
                gwy_debug("ENTER nested variables (%u)", depth+1);
                var->data = gdef_read_variable_lists(p, size - (*p - buffer), depth + 1, error);
                gwy_debug("EXIT nested variables (%u)", depth+1);
                if (!var->data)
                    goto fail;
            }
        }
        /* Unlike in .gwy, the data are always stored at the top level, i.e. the data of all nested blocks follow the
         * corresponding top-level block.  So we have to postpone data reading until we get to the top level again. */
        if (depth == 0) {
            if (!gdef_read_variable_data(block, p, size - (*p - buffer), depth, error))
                goto fail;
        }
    } while (block->next);
        ...
        for (i = 0; i < block->n_variables; i++) {
            ...
            if (gwy_strequal(var->name, name)) {
                if (type == VAR_UNSPECIFIED)
                    return var;
                if (type == VAR_BOOLEAN && var->type == VAR_BOOL)
                    return var;
                if (type == VAR_INTEGRAL && (var->type == VAR_WORD || var->type == VAR_INTEGER
                                             || var->type == VAR_DWORD || var->type == VAR_BOOL))
                    return var;
                if (type == VAR_REAL && (var->type == VAR_FLOAT || var->type == VAR_DOUBLE))
                    return var;
                if (var->type == type)
                    return var;
            }
...
{
    GDEFVariable *xres_var, *yres_var, *ymis_var, *xreal_var, *yreal_var, *zunit_var, *data_var, *value_var;
    ...
    if (err_DIMENSION(NULL, xres) || err_DIMENSION(NULL, yres) || ymis >= yres) {
    ...
    for (i = 0; i < yres - ymis; i++) {
        gwy_convert_raw_data(rawdata + i*xres*sizeof(gfloat), xres, 1,
                             GWY_RAW_DATA_FLOAT, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                             data + (yres-ymis-1 - i)*xres, 1.0, 0.0);
    }
...
gdef_read_channel_meta(GDEFControlBlock *block)
{
    ...
    for (i = 0; i < block->n_variables; i++) {
        ...
        if (var->type == VAR_INTEGER || var->type == VAR_WORD || var->type == VAR_DWORD) {
            s = g_strdup_printf("%ld", (glong)gdef_get_int_var(var));
            gwy_container_set_string_by_name(meta, var->name, s);
        }
        else if (var->type == VAR_BOOL) {
            const gchar *cs = gdef_get_int_var(var) ? "true" : "false";
            gwy_container_set_const_string_by_name(meta, var->name, cs);
        }
    ...
    for (block = blocks; block; block = block->next) {
        ...
        if ((dfield = gdef_read_channel(block))) {
            ...
            if (var) {
                ...
                /* XXX: That's what I've been told is necessary to obtain values in degrees. */
