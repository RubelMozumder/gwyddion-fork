/*
 *  $Id: wipfile.c 25680 2023-09-18 16:35:33Z yeti-dn $
                           ...
                           gpointer transform)
{
    ...
    if (!strncmp(tag->name, "SpectralTransformationType", 26))
        ...
        sp_transform->unitname = gwy_convert_to_utf8(str, str_len, "ISO-8859-1");
        g_free(str);
    }
...
wip_read_axis_tags(GNode *node, gpointer axis)
{
    ...
    if (!strncmp(tag->name, "UnitName", 8)) {
        str_len = gwy_get_gint32_le(&p);
        str = g_strndup(p, str_len);
        tmp_axis->unitname = gwy_convert_to_utf8(str, str_len, "ISO-8859-1");
        g_free(str);
    }
    ...
    if (!strncmp(tag->name, "TDGraph", 7)) {
        ...
        if (!gmodel) {
            ...
            if (!brick) {
                ...
                gwy_container_set_const_string_by_name(filecontent->data, key->str, caption->str);
                g_string_free(caption, TRUE);
            ...
            g_string_printf(key, "/0/graph/graph/%d",
                            filecontent->numgraph);
            gwy_container_pass_object_by_name(filecontent->data, key->str, gmodel);
        }
    }
