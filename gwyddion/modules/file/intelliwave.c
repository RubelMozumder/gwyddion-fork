/*
 *  $Id: intelliwave.c 25688 2023-09-20 11:08:15Z yeti-dn $
...
/* XXX: The alignmentN bytes are not specified in the documentation.  However, they are in a place where a compiler
 * would reasonably use alignment.  So there is that... */
typedef struct {
            ...
            gboolean only_name)
{
    ...
    if (fileinfo->file_size > HEADER_SIZE + INFO_SIZE && memcmp(fileinfo->head, MAGIC, MAGIC_SIZE) == 0)
        return 100;
          ...
          GError **error)
{
    ...
    for (i = 0; i < N_IS_VALID; i++) {
        ...
        if (offset == 0xffffffffu || offset == 0xfffffffeu || offset == 0x00000001u) {
            g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                        _("Invalid data address 0x%0x found.  File is in some unknown format version."),
                        offset);
            goto fail;
        }
    }

    if (err_DIMENSION(error, intwfile.info.nx) || err_DIMENSION(error, intwfile.info.ny))
        goto fail;
    ...
    if ((offset = check_data_block(&intwfile, i, size, N_INTERFEROGRAMS*sizeof(gint16))))
                    ...
                    const IntWaveFile *intwfile, const guchar *p)
{
    ...
    gdouble *d, *m;
    gdouble q = 1e-6 * intwfile->info.wavelength, invalid = intwfile->info.invalid_value;

    dfield = gwy_data_field_new(xres, yres, xres, yres, FALSE);
    d = gwy_data_field_get_data(dfield);
    gwy_convert_raw_data(p, n, 1, GWY_RAW_DATA_FLOAT, GWY_BYTE_ORDER_LITTLE_ENDIAN, d, 1.0, 0.0);
    ...
    gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(*id), "OPD");
    if (mask)
        gwy_container_pass_object(container, gwy_app_get_mask_key_for_id(*id), mask);

    gwy_file_channel_import_log_add(container, *id, NULL, intwfile->filename);
    (*id)++;
}
                        ...
                        const guchar *p)
{
    ...
    for (i = 0; i < N_INTERFEROGRAMS; i++) {
        dfield = gwy_data_field_new(xres, yres, xres, yres, FALSE);
        d = gwy_data_field_get_data(dfield);
        gwy_convert_raw_data(p + i*n*sizeof(gint16), n, 1, GWY_RAW_DATA_SINT16, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                             d, 1.0, 0.0);
