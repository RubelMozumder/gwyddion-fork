/*
 *  $Id: rhk-sm3.c 25672 2023-09-15 09:13:54Z yeti-dn $
...
typedef struct {
    ...
    /* The actual type depends.  The documentation is a bit confusing as to which type is used when, but at least we
     * know int32 and float32 can occur. */
                    ...
                    gsize len)
{
    ...
    s = gwy_utf16_to_utf8((const gunichar2*)*buffer, n, GWY_BYTE_ORDER_LITTLE_ENDIAN);
                  ...
                  GError **error)
{
    ...
    if (err_DIMENSION(error, page->x_size) || err_DIMENSION(error, page->y_size))
        goto fail;
    ...
    sanitise_real_size(&page->x_scale, "x scale");
    sanitise_real_size(&page->y_scale, "y scale");
    ...
    if (page->type == RHK_TYPE_IMAGE) {
        ...
        /* XPMPro manual says the length field is 4 bytes, but reality seems to disagree vehemently. */
...
rhk_sm3_page_to_data_field(const RHKPage *page)
{
    ...
    dfield = gwy_data_field_new(xres, yres,
                                xres*fabs(page->x_scale), yres*fabs(page->y_scale),
                                FALSE);
    gwy_convert_raw_data(page->page_data, xres*yres, 1, GWY_RAW_DATA_SINT32, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                         gwy_data_field_get_data(dfield), page->z_scale, page->z_offset);
    ...
    if (page->strings[RHK_STRING_X_UNITS]
        ...
        unit = "";
    gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_xy(dfield), unit);
...
rhk_sm3_page_to_spectra(const RHKPage *page)
{
    ...
    if (page->strings[RHK_STRING_X_UNITS])
        ...
        unit = NULL;
    gwy_si_unit_set_from_string(gwy_data_line_get_si_unit_x(dline), unit);
    ...
    if (gwy_strequal(unit, "N/sec"))
        unit = "s^-1";
    gwy_si_unit_set_from_string(gwy_data_line_get_si_unit_y(dline), unit);

    for (i = 0; i < ncurves; i++) {
        GwyGraphCurveModel *gcmodel = gwy_graph_curve_model_new();
        gchar *s = g_strdup_printf("%d", i+1);

        /* FIXME: The docs seem to talk about floats, but AFAICT the data looks like integers. */
        for (j = 0; j < res; j++)
             ...
             GError **error)
{
    ...
    for (i = count = 0; i < rhkfile->len; i++) {
        ...
        gwy_container_pass_object(container, quark, dfield);
        p = rhkpage->strings[RHK_STRING_LABEL];
        cs = gwy_enum_to_string(rhkpage->scan_dir, scan_directions, G_N_ELEMENTS(scan_directions));
        if (p && *p) {
        ...
        gwy_container_pass_object_by_name(container, key->str, meta);
    ...
    for (i = count = 0; i < rhkfile->len; i++) {
        ...
        p = rhkpage->strings[RHK_STRING_LABEL];
        if (!p || !*p)
            p = gwy_enuml_to_string(rhkpage->line_type,
                                    ...
                                    "Noise autocorrelation spectrum", RHK_LINE_NOISE_AUTOCORRELATION_SPECTRUM,
                                    "Multichannel analyser data", RHK_LINE_MULTICHANNEL_ANALYSER_DATA,
                                    "Renormalized I-V", RHK_LINE_RENORMALIZED_IV,
                                    "Image histogram spectra", RHK_LINE_IMAGE_HISTOGRAM_SPECTRA,
        ...
        gwy_container_pass_object(container, quark, spectra);
...
rhk_sm3_get_metadata(RHKPage *rhkpage)
{
    ...
    s = gwy_enuml_to_string(rhkpage->page_type,
                            ...
                            NULL);
    if (s && *s)
        gwy_container_set_const_string_by_name(meta, "Type", s);

    s = gwy_enum_to_string(rhkpage->scan_dir, scan_directions, G_N_ELEMENTS(scan_directions));
    if (s && *s)
        gwy_container_set_const_string_by_name(meta, "Scan Direction", s);
    ...
    s = gwy_enuml_to_string(rhkpage->source_type,
                            ...
                            NULL);
    if (s && *s)
        gwy_container_set_const_string_by_name(meta, "Source", s);

    gwy_container_set_string_by_name(meta, "Bias", g_strdup_printf("%g V", rhkpage->bias));
    gwy_container_set_string_by_name(meta, "Rotation angle", g_strdup_printf("%f", rhkpage->angle));
    gwy_container_set_string_by_name(meta, "Period", g_strdup_printf("%f s", rhkpage->period));
    ...
    if (s && *s)
        gwy_container_set_const_string_by_name(meta, "Label", s);

    s = rhkpage->strings[RHK_STRING_PATH];
    if (s && *s)
        gwy_container_set_const_string_by_name(meta, "Path", s);

    s = rhkpage->strings[RHK_STRING_SYSTEM_TEXT];
    if (s && *s)
        gwy_container_set_const_string_by_name(meta, "System comment", s);

    s = rhkpage->strings[RHK_STRING_SESSION_TEXT];
    if (s && *s)
        gwy_container_set_const_string_by_name(meta, "Session comment", s);

    s = rhkpage->strings[RHK_STRING_USER_TEXT];
    if (s && *s)
        gwy_container_set_const_string_by_name(meta, "User comment", s);
