/*
 *  $Id: intematix.c 26163 2024-02-06 16:56:55Z yeti-dn $
          ...
          GError **error)
{
    ...
    gwy_clear(&image, 1);
    if (!gwy_tiff_get_sint0(tiff, ISDF_TIFFTAG_FILEID, &magic) || magic != ISDF_MAGIC_NUMBER) {
    ...
    if (err_SIZE_MISMATCH(error, t*image.xres*image.yres, t*image.raw_data_len, TRUE))
        goto fail;
    ...
    sanitise_real_size(&image.xreal, "x size");
    sanitise_real_size(&image.yreal, "y size");
    dfield = gwy_data_field_new(image.xres, image.yres,
                                image.xreal*pow10(power10x), image.yreal*pow10(power10y),
                                FALSE);
    ...
    else if (image.raw_data_type == GWY_TIFF_DOUBLE)
        rawtype = GWY_RAW_DATA_DOUBLE;
    else {
        g_assert_not_reached();
    }
    gwy_convert_raw_data(image.raw_data, image.xres*image.yres, 1, rawtype, tiff->byte_order,
    ...
    if (gwy_container_gis_string_by_name(meta, "Data type", &p))
        gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(0), p);
                     ...
                     GError **error)
{
    ...
    if (image->raw_data_type != GWY_TIFF_SLONG && image->raw_data_type != GWY_TIFF_DOUBLE) {
                  ...
                  const ISDFImage *image)
{
    ...
    if ((s = gwy_enuml_to_string(image->file_type, "STM", 1, "AFM", 2, "EMP", 3, NULL)))
        gwy_container_set_const_string_by_name(meta, "File type", s);

    s = gwy_enuml_to_string(image->data_type,
    ...
    if (gwy_tiff_get_float0(tiff, ISDF_TIFFTAG_SCANRATE, &v))
        gwy_container_set_string_by_name(meta, "Scan rate", g_strdup_printf("%g line/s", v));
    if (gwy_tiff_get_float0(tiff, ISDF_TIFFTAG_BIASVOLTS, &v))
        gwy_container_set_string_by_name(meta, "Bias", g_strdup_printf("%g V", v));
