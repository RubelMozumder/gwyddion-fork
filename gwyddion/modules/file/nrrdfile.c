/*
 *  $Id: nrrdfile.c 25989 2023-10-31 17:27:00Z yeti-dn $
              ...
              GError **error)
{
    ...
    if (chanaxis == G_MAXUINT) {
        ...
        gwy_container_pass_object(container, gwy_app_get_brick_key_for_id(0), brick);
        ...
        for (i = 0; i < nchannels; i++) {
            ...
            if ((value = g_hash_table_lookup(fields, "content")))
                gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(i), value);
            if (meta) {
                GwyContainer *chanmeta = gwy_container_duplicate(meta);

                gwy_container_pass_object(container, gwy_app_get_data_meta_key_for_id(i), chanmeta);
            }
                   ...
                   GError **error)
{
    ...
    if (gzcompressed) {
        ...
        *size = prevsize + readbytes;
        ...
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_SPECIFIC,
                    /* TRANSLATORS: %s is replaced by a software library name like zlib. */
                    _("Cannot decompress compressed data.  Gwyddion was built without %s support."), "bzip2");
#endif
    }
    else {
                    ...
                    const guchar *data)
{
    ...
    if ((value = g_hash_table_lookup(fields, "spacings")) && parse_float_vector(value, 2, &dx, &dy)) {
    ...
    /* FIXME: This is probably wrong if dimensions == 3 && chanaxis != 2. */
    if ((value = g_hash_table_lookup(fields, "axismaxs")) && parse_float_vector(value, 2, &dx, &dy)) {
        dx = (dx - xoff)/xres;
        dy = (dy - xoff)/xres;
        sanitise_real_size(&dx, "x step");
        sanitise_real_size(&dy, "y step");
               ...
               const guchar *data)
{
    ...
    if ((value = g_hash_table_lookup(fields, "spacings")) && parse_float_vector(value, 3, &dx, &dy, &dz)) {
        sanitise_real_size(&dx, "x step");
        sanitise_real_size(&dy, "y step");
        sanitise_real_size(&dz, "z step");
    }
