/*
 *  $Id: ols.c 26158 2024-02-06 12:31:23Z yeti-dn $
...
ols_load_tiff(const GwyTIFF *tiff, const gchar *filename, GError **error)
{
    ...
    if (!gwy_tiff_get_string0(tiff, GWY_TIFFTAG_IMAGE_DESCRIPTION, &comment) || !strstr(comment, MAGIC_COMMENT)) {
        g_free(comment);
    ...
    for (dir_num = 0; dir_num < gwy_tiff_get_n_dirs(tiff); dir_num++) {
        ...
        sanitise_real_size(&xy_axis, "size step");
        ...
        for (ch = 0; ch < spp; ch++) {
            ...
            dfield = gwy_data_field_new(reader->width, reader->height,
                                        reader->width * xy_axis * factor, reader->height * xy_axis * factor,
                                        FALSE);
            // units
            ...
            if (spp == 1) {
                if (dir_num == 1)
                    gwy_si_unit_set_from_string_parse(siunit, "nm", &power10);
                else
                    power10 = -6;
            }
            ...
            for (i = 0; i < reader->height; i++)
                gwy_tiff_read_image_row(tiff, reader, ch, i, factor, 0.0, data + i*reader->width);
            ...
            title = NULL;
            if (spp == 1) {
                /* Channel 0 is texture and 1 is topography */
                if (dir_num == 0)
                    title = "Texture";
                else if (dir_num == 1)
                    title = "Height";
