/*
 *  $Id: spip-asc.c 25636 2023-09-07 12:19:33Z yeti-dn $
                ...
                GError **error)
{
    ...
    if (!gwy_parse_doubles(p, gwy_data_field_get_data(field), GWY_PARSE_DOUBLES_FREE_FORM, &yres, &xres, &p, &err)) {
    ...
    if ((value = g_hash_table_lookup(hash, "voidpixels")) && atoi(value)) {
        ...
        if (gwy_parse_doubles(p, gwy_data_field_get_data(mfield), GWY_PARSE_DOUBLES_FREE_FORM,
                              &yres, &xres, &p, NULL)) {
