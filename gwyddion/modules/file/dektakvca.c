/*
 *  $Id: dektakvca.c 26163 2024-02-06 16:56:55Z yeti-dn $
...
/* Points directly to the memory buffer.  We use it for all kind of blocks in the file: containers, raw data regions,
 * strings and also the file itself.
 */
typedef struct {
    const guchar *p;
    guint32 len;
...
/* Quantities have name, symbol and value (in that units presumably). Units have name, symbol, conversion factor *to*
 * (not from) the unit and then some stuff.  Possibly boolean flags? */
typedef struct {
    gdouble value;
    DektakBuf name;
    DektakBuf symbol;
                 ...
                 gboolean only_name)
{
    ...
    if (fileinfo->buffer_len <= MAGIC_SIZE || memcmp(head, MAGIC, MAGIC_SIZE) != 0)
        return 0;
               ...
               GError **error)
{
    ...
    hash = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, dektak_item_free);
    ...
    /* Many things have two values, one in measurement settings and the other in the data.  The value in data seems to
     * be the actual one, the value in settings is nominal? */
...
/* Only returns FALSE when @error is set.  When it does not seem there are 1D data it returns TRUE; must check if
 * @container has anyhthing in it later. */
static gboolean
             ...
             GError **error)
{
    ...
    if (!(arrayitem = find_item(hash, "/1D_Data/Raw/Array", DEKTAK_DOUBLE_ARRAY, FALSE, NULL)))
        return TRUE;

    if (!(item = find_item(hash, MEAS_SETTINGS "/SamplesToLog", DEKTAK_UINT64, TRUE, error)))
        goto fail;

    res = item->data.uq;
    gwy_debug("res %u", res);

    if (!(xunit = find_quantity(hash, MEAS_SETTINGS "/ScanLength", str, &real, error)))
        goto fail;
    gwy_debug("real %g", real);

    if (!(yunit = find_quantity(hash, "/1D_Data/Raw/DataScale", str, &qy, error)))
        goto fail;
    gwy_debug("qy %g", qy);

    if (err_SIZE_MISMATCH(error, DOUBLE_ARRAY_EXTRA + res*sizeof(gdouble), arrayitem->data.buf.len, TRUE))
        goto fail;
    rawydata = arrayitem->data.buf.p + DOUBLE_ARRAY_EXTRA;

    /* Positions -- optional. */
    if ((item = find_item(hash, "/1D_Data/Raw/PositionFunction", DEKTAK_POS_RAW_DATA, TRUE, NULL))) {
        if (err_SIZE_MISMATCH(error, res*sizeof(gdouble), item->data.rawpos1d.buf.len, TRUE))
            goto fail;
    ...
    if ((item = find_item(hash, "/MetaData/DataKind", DEKTAK_STRING, FALSE, NULL)))
    ...
    if (rawxdata) {
        gwy_convert_raw_data(rawxdata, res, 1, GWY_RAW_DATA_DOUBLE, GWY_BYTE_ORDER_LITTLE_ENDIAN, xdata, qx, 0.0);
    }
    else {
        for (i = 0; i < res; i++)
    ...
    gwy_convert_raw_data(rawydata, res, 1, GWY_RAW_DATA_DOUBLE, GWY_BYTE_ORDER_LITTLE_ENDIAN, ydata, qy, 0.0);
    ...
    g_object_set(gmodel,
                 ...
                 "title", str->str,
                 NULL);
    gwy_container_pass_object(container, gwy_app_get_graph_key_for_id(0), gmodel);
...
find_2d_data_matrix(gpointer key, gpointer value, gpointer user_data)
{
    ...
    gwy_debug("found 2D channel %s", (gchar*)g_ptr_array_index(channels, channels->len-1));
}
             ...
             GError **error)
{
    ...
    for (i = 0; i < channels->len; i++) {
        ...
        gwy_si_unit_assign(gwy_data_field_get_si_unit_xy(dfield), xunit);
        gwy_si_unit_assign(gwy_data_field_get_si_unit_z(dfield), zunit);

        gwy_convert_raw_data(rawdata, xres*yres, 1, GWY_RAW_DATA_FLOAT, GWY_BYTE_ORDER_LITTLE_ENDIAN,
        ...
        gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(i),
...
create_meta_item(gpointer key, gpointer value, gpointer user_data)
{
    ...
    if (item->typeid == DEKTAK_BOOLEAN)
        ...
        metavalue = g_strdup_printf("%g %.*s",
                                    item->data.qun.value, item->data.qun.symbol.len, item->data.qun.symbol.p);
    }
              ...
              gdouble *value, GError **error)
{
    ...
    if (!(item = find_item(hash, key, DEKTAK_QUANTITY, TRUE, error)))
        return NULL;
    g_string_truncate(str, 0);
    g_string_append_len(str, item->data.qun.symbol.p, item->data.qun.symbol.len);
    ...
    if (expected_type && item->typeid != expected_type) {
        if (fail_if_not_found) {
            g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                        _("Item `%s' has unexpected type %u instead of %u."), path, item->typeid, expected_type);
        }
        return NULL;
    }
    return item;
...
read_varlen(const DektakBuf *buf, guint32 *pos, guint32 *l)
{
    ...
    if (lenlen == 1) {
        ...
        *l = len;
        ...
        /* XXX: We should to report a different error type here, like "Unsupported length length".  */
        return FALSE;
    }
    return TRUE;
}
...
/* NB: it moves @pos to the end of the structure because @p becomes the inner content buffer. */
static gboolean
...
/* NB: it moves @pos to the end of the structure because @content becomes the inner content buffer. */
static gboolean
          ...
          GError **error)
{
    ...
    /* Container types.  Cannot tell any difference between these two.  Raw data purpose seems to be wrapping actual
     * raw data in something container-like. */
    else if (typeid == DEKTAK_CONTAINER || typeid == DEKTAK_RAW_DATA || typeid == DEKTAK_RAW_DATA_2D) {
        if (!read_structured(buf, pos, &content))
            goto fail;
    ...
    /* Types with string type name (i.e. untyped serialised junk we have to know how to read). */
        ...
        /* The type id is presumably an integral type because in XML it is represented by an integer literal.  But it
         * is represented as a byte buffer.  */
        if (!read_named_struct(buf, pos, &item.typename, &item.data.buf))
            goto fail;
    }
    else if (typeid == DEKTAK_POS_RAW_DATA) {
        /* Unfortunately, we have to know if we are reading 1D or 2D at this point because the structs differ.
         * Bummer. */
        if (g_str_has_prefix(path->str, "/2D_Data")) {
            ...
            if (!read_dimension2d_content(&content, &itempos, &item.data.rawpos2d.unitx, &item.data.rawpos2d.divisorx))
                goto fail;
            if (!read_dimension2d_content(&content, &itempos, &item.data.rawpos2d.unity, &item.data.rawpos2d.divisory))
                goto fail;
        }
        else {
            ...
            /* This is exactly as a UNIT record, but it does not carry the type byte.  Whatever. */
            if (!read_quantunit_content(&content, &itempos, TRUE, &item.data.rawpos1d.unit))
                goto fail;
    ...
    g_hash_table_insert(hash, g_strndup(path->str, path->len), g_slice_dup(DektakItem, &item));
