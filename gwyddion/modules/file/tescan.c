/*
 *  $Id: tescan.c 26345 2024-05-15 13:32:22Z yeti-dn $
...
tsctif_detect(const GwyFileDetectInfo *fileinfo, gboolean only_name)
{
    ...
    if ((tiff = gwy_tiff_load(fileinfo->name, NULL)) && tsctif_find_header(tiff, NULL))
        score = 100;
...
tsctif_load_tiff(GwyTIFF *tiff, const GwyTIFFEntry *entry, GError **error)
{
    ...
    if ((value = g_hash_table_lookup(hash, "Main::PixelSizeX"))) {
        ...
        sanitise_real_size(&xstep, "x step");
    }
    else {
    ...
    if ((value = g_hash_table_lookup(hash, "Main::PixelSizeY"))) {
        ...
        sanitise_real_size(&ystep, "y step");
    }
    else {
    ...
    for (i = 0; i < reader->height; i++) {
        gwy_tiff_read_image_row(tiff, reader, 0, i, 1.0/((1 << reader->bits_per_sample) - 1), 0.0,
                                data + i*reader->width);
    }
    ...
    /* FIXME: Just kidding. */
    gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(0), "Intensity");
...
{
    static const gchar fields[] = "AccFrames=Device=Magnification=PixelSizeX=PixelSizeY=UserName=";
    guint score = 0;
    const gchar *s, *end;
    GString *str;
            ...
            GError **error)
{
    ...
    if (!require_keys(hash, error, "MAIN::PixelSizeX", "MAIN::PixelSizeY", NULL))
        goto fail;
...
{
    static const GFileTest flags = G_FILE_TEST_IS_REGULAR | G_FILE_TEST_IS_SYMLINK;
    guint len = str->len;
