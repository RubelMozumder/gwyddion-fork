/*
 *  $Id: tiaser.c 25795 2023-10-06 15:20:36Z yeti-dn $
         ...
         G_GNUC_UNUSED GwyRunType mode, GError **error)
{
    ...
    GwyDataLine *dline;
    GwySIUnit *siunit;
    gint i, k, offset, dimarraylength, dimarraysize = 0;
    ...
    for (i = 0; i < dimarraylength; i++) {
        p += dimarraysize;
        gwy_clear(&dim, 1);
        if (!tia_load_dimarray(p, &dim, size - TIA_HEADER_SIZE - dimarraysize)) {
            err_FILE_TYPE(error, "FEI TIA");
            goto fail3;
        }
    ...
    if (header->datatypeid == TIA_2D_DATA)
        for (i = 0; i < header->validnumberelements; i++) {
            ...
            if ((offset > size) || (size-offset < TIA_2D_SIZE)) {
                ...
                if (dfield) {
                    ...
                    gwy_container_pass_object(container, key, dfield);

                    strkey = g_strdup_printf("/%u/data/title", i);
                    gwy_container_set_const_string_by_name(container, strkey, "TEM");
                    g_free(strkey);
        ...
        if (brick) {
            gwy_container_pass_object_by_name(container, "/brick/0", brick);
            gwy_container_set_const_string_by_name(container, "/brick/0/title", "TEM Spectroscopy");
