/*
 *  $Id: pt3file.c 25728 2023-09-27 15:24:50Z yeti-dn $
...
/* FIXME: I don't have any v5.0 files (those with TimeHarp magic header) so we don't try to read them. */
...
/* XXX: The fields are not in file order here, we try to maximize the number of items in common. */
typedef struct {
    guint dimensions;
    PicoHarpInstrument instrument;
    guint xres;
...
typedef struct {
    ...
    PicoHarpBoard board;   /* the length is number_of_boards, however, it must be 1 so we ignore any possible boards
                            * after that */
    ...
    guint spec_header_length;    /* stored value has units [4bytes], but we recalculate it upon reading */
             ...
             GError **error)
{
    ...
    if (pt3file.imaging.common.instrument != PICO_HARP_PIE710
        ...
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Only PI E710 and KDT180-100-Im imaging formats are implemented."));
        goto fail;
    }
    ...
    gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(0), "Photon count");
    gwy_container_pass_object(container, gwy_app_get_data_meta_key_for_id(0), pt3file_get_metadata(&pt3file));
    gwy_file_channel_import_log_add(container, 0, NULL, filename);
    gwy_container_pass_object(container, gwy_app_get_graph_key_for_id(1), pt3file_extract_decay(&pt3file, p));
                    ...
                    GError **error)
{
    ...
    if (memcmp(pt3file->ident, MAGIC, MAGIC_SIZE) != 0 || pt3file->crlf[0] != '\r' || pt3file->crlf[1] != '\n') {
    ...
    if (pt3file->number_of_boards != 1) {
        g_warning("Number of boards is %u instead of 1.  Reading one.", pt3file->number_of_boards);
    ...
    if (instr == PICO_HARP_PIE710) {
        ...
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Unknown instrument number %u."), instr);
        return 0;
    }
    ...
    if (err_DIMENSION(error, pt3file->imaging.common.xres) || err_DIMENSION(error, pt3file->imaging.common.yres))
        return 0;
                           ...
                           const guchar *p)
{
    ...
    gwy_debug("accel %u, bidirectional: %d", header->pie710.acceleration, header->pie710.bidirectional);
    gwy_debug("%g %g %g %g",
                           ...
                           GError **error)
{
    ...
    if (lineno < yres) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Number of line triggers %u is smaller than the number of scan lines %u."),
                    lineno, yres);
        g_free(linetriggers);
        return NULL;
    }
    ...
    if (pt3file->imaging.common.instrument == PICO_HARP_PIE710) {
        ...
        for (lineno = 0; lineno < yres; lineno++) {
            ...
            if (lineno % 2 == 0 || !pt3file->imaging.common.bidirectional) {
                ...
                ltr->start = ltr->globaltime + (guint64)((fstart - 1.0)*n*(1.0 - 2*a) + a*n);
                ltr->stop = ltr->globaltime + (guint64)((fstop - 1.0)*n*(1.0 - 2*a) + a*n);
            }
        }
    }
    else {
                       ...
                       const guchar *p)
{
    ...
    sanitise_real_size(&pix_resol, "pixel size");
    pix_resol *= 1e-6;

    dfield = gwy_data_field_new(xres, yres, pix_resol*xres, pix_resol*yres, TRUE);
...
#define add_meta_str(name, field) \
    gwy_container_set_string_by_name(meta, name, g_strndup(pt3file->field, sizeof(pt3file->field)))

#define add_meta_uint(name, field) \
    gwy_container_set_string_by_name(meta, name, g_strdup_printf("%u", pt3file->field))
...
pt3file_get_metadata(PicoHarpFile *pt3file)
{
    ...
    add_meta_uint("Measurement Mode", measurement_mode);
    gwy_container_set_string_by_name(meta, "Offset", g_strdup_printf("%d ns", pt3file->offset));
    gwy_container_set_string_by_name(meta, "Acquisition Time", g_strdup_printf("%d ms", pt3file->acquisition_time));
    add_meta_uint("Hardware Serial Number", board.hardware_serial);
    gwy_container_set_const_string_by_name(meta, "Imaging Device",
                                           gwy_enuml_to_string(pt3file->imaging.common.instrument,
                                                               "PI E710", 1, "KDT180-100-Im", 2, "LSM", 3,
                                                               NULL));
