/*
 *  $Id: nanoscan.c 26168 2024-02-08 10:18:02Z yeti-dn $
...
check_magic(const gchar *header)
{
    return (memcmp(header, MAGIC1, MAGIC1_SIZE) == 0
            ...
            && (strstr(header, MAGIC3a) != NULL || strstr(header, MAGIC3b) != NULL));
}
              ...
              GError **error)
{
    ...
    context = g_markup_parse_context_new(&parser, G_MARKUP_TREAT_CDATA_AS_TEXT, &nfile, NULL);
    ...
    for (i = id = 0; i < nfile.channels->len; i++) {
        NanoScanChannel *channel = &g_array_index(nfile.channels, NanoScanChannel, i);
            ...
            gint id)
{
    ...
    dfield = gwy_data_field_new(nfile->xres, nrows, nfile->xreal, yreal, FALSE);
    if (nfile->xyunits) {
          ...
          gint id)
{
    ...
    for (j = i = 0; j < nfile->channels->len; j++) {
        NanoScanChannel *other = &g_array_index(nfile->channels, NanoScanChannel, j);
        if (other == channel || other->already_added || !other->name || !channel->name
             ...
             gint id)
{
    ...
    for (i = 0; i < nfile->meta->len; i++) {
        ...
        if (meta->value)
            gwy_container_set_const_string_by_name(metadata, meta->name, meta->value);
    }

    gwy_container_pass_object(container, gwy_app_get_data_meta_key_for_id(id), metadata);
}
              ...
              GError **error)
{
    ...
    if (gwy_strequal(nfile->path->str, CHANNEL_PREFIX)) {
        ...
        /* Individual channels are inside <contents> tags, but there can be empty <contents> tags and stuff.  Just
         * create what we see and hope upstream cann sort it out. */
        gwy_clear(&channel, 1);
     ...
     G_GNUC_UNUSED GError **error)
{
    ...
    /* Content is not directly in elements such as <unit>.  Each element, in spite of its content model being a single
     * value, must contain a <v> element that actually contains the value.  Fortunately, the genius that has invented
     * this did not realize that the <v> element cannot directly contain the value either and it needs its own <v>
     * element... */
    if (nfile->path->len < 3
    ...
    if (strncmp(path, RES_PREFIX, RES_PREFIX_SIZE) == 0) {
    ...
    else if (strncmp(path, AXIS_PREFIX, AXIS_PREFIX_SIZE) == 0 && nfile->axes->len) {
        NanoScanAxis *axis = &g_array_index(nfile->axes, NanoScanAxis, nfile->axes->len - 1);
        ...
        /* FIXME: These can be vectors in the energy scan mode! */
            ...
            gwy_debug("axis stop: %g", axis->stop);
             ...
             && nfile->channels->len) {
        NanoScanChannel *channel = &g_array_index(nfile->channels, NanoScanChannel, nfile->channels->len - 1);
        ...
        if (gwy_strequal(path, "/name")) {
            ...
            g_free(channel->data);
            channel->data = read_channel_data(val, nfile->xres * nfile->yres, &channel->ndata, error);
            gwy_debug("DATA: %p", channel->data);
        }
    }
    else if (strncmp(path, DATA_PREFIX, DATA_PREFIX_SIZE) == 0) {
        /* The channels are groupped by direction.  So we need to remember which direction are in because it is just
         * an element along the way. */
    ...
    else if (strncmp(path, SCAN_PREFIX, SCAN_PREFIX_SIZE) == 0 && !strchr(path + SCAN_PREFIX_SIZE + 1, '/')) {
...
add_meta(NanoScanFile *nfile,
         ...
         gchar *value)
{
    /* If the element ends with _units, it should be the units of the previous element. */
    if (g_str_has_suffix(name, "_unit")) {
        if (nfile->meta->len) {
            NanoScanMeta *meta = &g_array_index(nfile->meta, NanoScanMeta, nfile->meta->len - 1);
