/*
 *  $Id: aistfile.c 26163 2024-02-06 16:56:55Z yeti-dn $
...
read_aist_raster(const guchar **p, gsize *size, AistContext *context)
{
    ...
    */
    ...
    for (i = 0; i < raster.yres; i++) {
        gwy_convert_raw_data(data + i*raster.xres*sizeof(gdouble), raster.xres, 1,
                             GWY_RAW_DATA_DOUBLE, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                             d + (raster.yres-1 - i)*raster.xres, qz, 0.0);
    }

    gwy_container_pass_object(context->container, gwy_app_get_data_key_for_id(context->channel_id), dfield);
    gwy_file_channel_import_log_add(context->container, context->channel_id, NULL, context->filename);

    cn = raster.common.name;
    while (cn[0] == '_')
        cn++;
    if ((s = strchr(cn, '[')))
        s = g_strchomp(g_strndup(cn, s - cn));
    else
        s = g_strdup(cn);
    ...
    if (read_qt_byte_array(p, size, &len, &data)) {
        if (len == raster.xres*raster.yres) {
            ...
            if (mfield) {
                ...
                gwy_container_pass_object(context->container, gwy_app_get_mask_key_for_id(context->channel_id), mfield);
            }
        }
...
read_aist_curve(const guchar **p, gsize *size, AistContext *context)
{
    ...
    gwy_container_pass_object(context->container, quark, gmodel);
