/*
 *  $Id: pnifile.c 25719 2023-09-26 13:09:32Z yeti-dn $
         ...
         GError **error)
{
    enum {
        ...
        /* XXX: There are two candidate positions for data type, 0x01aa and 0x019e.  Cannot tell which is value type
         * and which something strongly correlated to it for available files. */
    ...
    const gchar *title, *unitstr = NULL;
    gchar *s;
    ...
    if (value_type == VALUE_TYPE_NM) {
        unitstr = "m";
        zscale *= Nanometer;
    }
    else if (value_type == VALUE_TYPE_MV) {
        unitstr = "V";
        zscale *= Milivolt;
    }
    else {
    ...
    gwy_convert_raw_data(buffer + DATA_START, xres*yres, 1, GWY_RAW_DATA_SINT16, GWY_BYTE_ORDER_LITTLE_ENDIAN,
    ...
    if (title) {
        s = g_strdup_printf("%s (%s)", title, direction ? "Backward" : "Forward");
