/*
 *  $Id: burleigh_bii.c 26155 2024-02-06 12:02:49Z yeti-dn $
...
/* XXX: Unfortunately, BM6 is Windows 3.x bitmap and we cannot claim ownership of that. */
/**
...
enum {
    FILE_IMG_SIZE_POS = 0x12,
    FILE_IMG_DATA_START_POS = 0x36,
    FILE_IMG_XY_SCALE_POS = 0x74,
    FILE_IMG_Z_SCALE_POS = 0xCC,
};
                    ...
                    gboolean only_name)
{
    ...
    if (gwy_memmem(fileinfo->tail, fileinfo->buffer_len, SIGNATURE, sizeof(SIGNATURE)-1))
        return 95;
                  ...
                  GError **error)
{
    ...
    gsize size = 0;
    GError *err = NULL;
    gint width, height, data_end_offset;
    ...
    if (err_SIZE_MISMATCH(error, data_end_offset + FILE_IMG_Z_SCALE_POS + 8, size, FALSE)) {
        gwy_file_abandon_contents(buffer, size, NULL);
        return NULL;
    }
    p = buffer + data_end_offset + FILE_IMG_XY_SCALE_POS;
    x_scale = gwy_get_gdouble_le(&p);
    sanitise_real_size(&x_scale, "x size");
    y_scale = gwy_get_gdouble_le(&p);
    sanitise_real_size(&y_scale, "y size");
    p = buffer + data_end_offset + FILE_IMG_Z_SCALE_POS;
    z_scale = gwy_get_gdouble_le(&p);

    //FIXME: all axes always in nm?
    gwy_debug("dfield: w=%d h=%d, scale: x=%g y=%g z=%g",
              width, height, x_scale, y_scale, z_scale);
    // create new datafield
    dfield = gwy_data_field_new(width, height, x_scale*Nanometre, y_scale*Nanometre, FALSE);
    gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_xy(dfield), "m");
    gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_z(dfield), "m");

    // set pointer to 16bit signed integers to start position of data definition in file and fill datafield
    gwy_convert_raw_data(buffer + FILE_IMG_DATA_START_POS, width*height, 1,
                         GWY_RAW_DATA_SINT16, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                         gwy_data_field_get_data(dfield), z_scale*Nanometre, 0.0);
