/*
 *  $Id: metropro.c 25689 2023-09-20 15:55:12Z yeti-dn $
 *  Copyright (C) 2006,2014-2019 David Necas (Yeti), Petr Klapetek.
              ...
              GError **error)
{
    ...
    g_free(mprofile->intensity_data);
    g_free(mprofile->intensity_mask);
                   ...
                   const guchar *buffer, gsize size, GError **error)
{
    ...
    expected = mprofile->header_size + 2*mprofile->ac_n_buckets*ac_npix + 4*cn_npix;

    /* XXX: We would like do to a strict size check, but apparently the files
     * can contain more stuff... */
    ...
    for (i = 0; i < n; i++) {
        ...
        if (vpmask)
            gwy_container_set_object(container, gwy_app_get_mask_key_for_id(i), vpmask);
        gwy_container_pass_object(container, gwy_app_get_data_meta_key_for_id(i), mprofile_get_metadata(mprofile));
        gwy_file_channel_import_log_add(container, i, NULL, mprofile->filename);
    }
                     ...
                     GError **error)
{
    ...
    gwy_debug("magic: %08x, header_format: %d, header_size: %d",
              mprofile->magic_number, mprofile->header_format, mprofile->header_size);
    for (i = 0; i < G_N_ELEMENTS(header_formats); i++) {
    ...
    gwy_debug("version: %d.%d.%d",
              mprofile->swinfo_vers_maj, mprofile->swinfo_vers_min, mprofile->swinfo_vers_bug);
    ...
    gwy_debug("INTENS xres: %d, yres: %d, xoff: %d, yoff: %d",
              mprofile->ac_width, mprofile->ac_height, mprofile->ac_org_x, mprofile->ac_org_y);
    ...
    gwy_debug("intens_nbytes: %d, expecting: %d",
              mprofile->ac_n_bytes, 2*mprofile->ac_width*mprofile->ac_height*mprofile->ac_n_buckets);
    ...
    gwy_debug("PHASE xres: %d, yres: %d, xoff: %d, yoff: %d",
              mprofile->cn_width, mprofile->cn_height, mprofile->cn_org_x, mprofile->cn_org_y);

    mprofile->cn_n_bytes = gwy_get_guint32_be(&p);
    gwy_debug("phase_nbytes: %d, expecting: %d",
              mprofile->cn_n_bytes, 4*mprofile->cn_width*mprofile->cn_height);
...
set_units(GwyDataField *dfield,
            ...
            const gchar *zunit)
{
    gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_xy(dfield), mprofile->lateral_res ? "m" : NULL);
                 ...
                 const guchar *buffer)
{
    ...
    if (mprofile->ac_n_buckets) {
        ...
        for (id = 0; id < mprofile->ac_n_buckets; id++) {
            ndata++;
            dfield = gwy_data_field_new(mprofile->ac_width, mprofile->ac_height, xreal, yreal, FALSE);
    ...
    if (n) {
        ...
        if (mprofile->phase_res == MPRO_PHASE_RES_NORMAL)
            ...
            g_warning("Unknown phase_res %u, scaling will be wrong.", mprofile->phase_res);
            i = 131072;
        }
        ...
        dfield = gwy_data_field_new(mprofile->cn_width, mprofile->cn_height, xreal, yreal, FALSE);
        vpmask = gwy_data_field_new_alike(dfield, FALSE);
        gwy_data_field_fill(vpmask, 1.0);
        data = gwy_data_field_get_data(dfield);
        mask = gwy_data_field_get_data(vpmask);
...
#define HASH_STORE(key, fmt, field) \
    gwy_container_set_string_by_name(meta, key, g_strdup_printf(fmt, mprofile->field))
...
#define HASH_STORE_ARRAY(key, fmt, field, n_field) \
    do { \
        for (i = 0; i < MIN(G_N_ELEMENTS(mprofile->field), mprofile->n_field); i++) { \
                  ...
                  gchar *field)
{
    ...
    g_strstrip(field);
    /* XXX: This gives inconsistent results on different systems. But it may be what the user expects. */
    if (field[0] && (p = g_locale_to_utf8(field, strlen(field), NULL, NULL, NULL)))
...
mprofile_get_metadata(MProFile *mprofile)
{
    ...
    /* Version */
    p = g_strdup_printf("%d.%d.%d", mprofile->swinfo_vers_maj, mprofile->swinfo_vers_min, mprofile->swinfo_vers_bug);
    gwy_container_set_string_by_name(meta, "Version", p);

    p = g_strdup_printf("%u (%08x)", mprofile->header_format, mprofile->magic_number);
    ...
    gwy_container_set_const_string_by_name(meta, "Date", buffer);

    /* Comments */
    store_meta_string(meta, "Software date", mprofile->swinfo_date);
