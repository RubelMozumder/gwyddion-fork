/*
 *  $Id: ometiff.c 26168 2024-02-08 10:18:02Z yeti-dn $
...
ome_load_tiff(const GwyTIFF *tiff, const gchar *filename, GError **error)
{
    ...
    const gchar *colour_channel_gradients[] = { "RGB-Red", "RGB-Green", "RGB-Blue" };
    ...
    omefile.meta = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
    omefile.path = key = g_string_new(NULL);
    omefile.data = g_array_new(FALSE, FALSE, sizeof(OMEData));
    context = g_markup_parse_context_new(&parser, G_MARKUP_TREAT_CDATA_AS_TEXT, &omefile, NULL);
    if (!g_markup_parse_context_parse(context, comment, -1, &err) || !g_markup_parse_context_end_parse(context, &err)) {
        err_XML(error, &err);
        goto fail;
    }

    gwy_debug("res xy,ztc: %u %u, %u %u %u", omefile.xres, omefile.yres, omefile.zres, omefile.tres, omefile.cres);
    ...
    for (dir_num = 0; dir_num < omefile.ndirs; dir_num++) {
        if (omefile.ifdmap[dir_num].seen) {
            ...
            gwy_debug("IFD#%u z=%u t=%u c=%u",
                      dir_num, omefile.ifdmap[dir_num].z, omefile.ifdmap[dir_num].t, omefile.ifdmap[dir_num].c);
        }
        else {
    ...
    for (dir_num = 0; dir_num < omefile.ndirs; dir_num++) {
        ...
        for (ch = 0; ch < spp; ch++) {
            ...
            /* Do no warn, it seems they happily produce files without physical dimensions. */
            if (omefile.xreal > 0.0 && omefile.yreal > 0.0) {
                xreal = omefile.xreal;
                yreal = omefile.yreal;
            }

            dfield = gwy_data_field_new(reader->width, reader->height, xreal, yreal, FALSE);
            /* units */
            if (omefile.xreal > 0.0 && omefile.yreal > 0.0)
            ...
            for (i = 0; i < reader->height; i++)
                gwy_tiff_read_image_row(tiff, reader, ch, i, zfactor, 0.0, d + i*reader->width);
            ...
            if (spp == 3) {
                channeltitle = g_strdup_printf("Z%u T%u C%u (%s)",
                                               assignment->z, assignment->t, assignment->c, colour_channels[ch]);
            }
            else if (spp > 1) {
                channeltitle = g_strdup_printf("Z%u T%u C%u (%u)",
                                               assignment->z, assignment->t, assignment->c, ch);
            }
            else {
                channeltitle = g_strdup_printf("Z%u T%u C%u",
                                               assignment->z, assignment->t, assignment->c);
            }
            gwy_container_set_string(container, gwy_app_get_data_title_key_for_id(id), channeltitle);

            if (spp == 3) {
                gwy_container_set_const_string(container, gwy_app_get_data_palette_key_for_id(id),
                                               colour_channel_gradients[ch]);
            }
              ...
              GError **error)
{
    ...
    for (i = 0; attribute_names[i]; i++) {
        gwy_debug("%s:%s <%s>", omefile->path->str, attribute_names[i], attribute_values[i]);
    }
    ...
    else if (gwy_stramong(omefile->path->str, "/OME/Image", "/OME/Image/Pixels", NULL)) {
        for (i = 0; attribute_names[i]; i++) {
            const gchar *name = attribute_names[i], *val = attribute_values[i];
            if (gwy_strequal(name, "DimensionOrder")) {
                omefile->dim_order = gwy_string_to_enum(val, dim_orders, G_N_ELEMENTS(dim_orders));
            }
    ...
    else if (gwy_stramong(omefile->path->str, "/OME/Image/TiffData", "/OME/Image/Pixels/TiffData", NULL)) {
        OMEData data;
    ...
    else if (gwy_stramong(omefile->path->str, "/OME/Image/TiffData/UUID", "/OME/Image/Pixels/TiffData/UUID", NULL)) {
        OMEData *data = &g_array_index(omefile->data, OMEData, omefile->data->len-1);
     ...
     G_GNUC_UNUSED GError **error)
{
    ...
    if (gwy_stramong(path, "/OME/Image/AcquisitionDate", "/OME/Image/Description", NULL)) {
    ...
    else if (gwy_stramong(path, "/OME/Image/TiffData/UUID", "/OME/Image/Pixels/TiffData/UUID", NULL)) {
        OMEData *data = &g_array_index(omefile->data, OMEData, omefile->data->len-1);
                        ...
                        GError **error)
{
    ...
    /* Skip image data with different UUIDs because they are presumably in other files and we cannot handle that.  In
     * the case we skip absolutely everything then unskip everything, cross fingers and proceeed... */
    j = 0;
    for (i = 0; i < omefile->data->len; i++) {
        OMEData *data = &g_array_index(omefile->data, OMEData, i);
        if (omefile->uuid && data->uuid && !gwy_strequal(omefile->uuid, data->uuid))
            data->ignored = TRUE;
        else
            j++;
    }
    ...
    for (i = 0; i < omefile->data->len; i++) {
        ...
        for (j = 0; j < data->planecount; j++) {
            ...
            if (z >= omefile->zres || t >= omefile->tres || c >= omefile->cres) {
                g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                            _("ZTC coordinates (%u,%u,%u) fall outside the given ranges."), z, t, c);
                return FALSE;
            }

            if (ifd >= omefile->ndirs) {
                g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                            _("The OME TIFF header specifies more TIFF directories than there are in the file."));
                return FALSE;
            }

            if (assignment->seen && (assignment->z != z || assignment->t != t || assignment->c != c)) {
                g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                            _("TIFF directory %u is assigned to multiple conflicting ZTC coordinates."), ifd);
                return FALSE;
            }
...
/* XXX: It is completely unclear from the specs how we are supposed to wrap around in adimension for which FirstFOO is
 * given.  We wrap to the FirstFOO value, not to 0. */
static void
