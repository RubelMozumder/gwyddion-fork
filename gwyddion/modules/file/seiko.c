/*
 *  $Id: seiko.c 26163 2024-02-06 16:56:55Z yeti-dn $
 ...
 * # Seiko
 * # Several variants, not sure if assignable to specific microscopy types. More can perhaps exist.
           ...
           GError **error)
{
    ...
    /* FIXME: I was not able to reverse-engineer what identifies the type. May need a large set of files.  */
    ...
    g_strstrip(comment);
    if (*comment)
        gwy_container_set_string(container, gwy_app_get_data_title_key_for_id(0), comment);
    else {
        g_free(comment);
                ...
                GError **error)
{
    ...
    if (err_SIZE_MISMATCH(error, imgsize, endfile - datastart, TRUE)) {
        /* The XQ?X files can have multiple images.  And each also comes with an extra headers.  We at least try to
