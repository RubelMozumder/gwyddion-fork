/*
 *  $Id: nanoeducator.c 26163 2024-02-06 16:56:55Z yeti-dn $
             ...
             GError **error)
{
    ...
    if (params.aqui_topo && header.topo_nx && header.topo_ny
        ...
        gwy_container_set_const_string_by_name(container, "/0/data/title", "Topography");
    ...
    if (params.aqui_spectr
        ...
        gwy_container_pass_object_by_name(container, "/sps/0", spectra);
        nobjects++;
    }
    ...
    if (header.addsurf_nx && header.addsurf_ny >= 1) {
        ...
        title = gwy_enuml_to_string(params.aqui_add,
                                    ...
                                    NULL);
        if (title && *title)
            gwy_container_set_const_string_by_name(container, "/1/data/title", title);

        g_object_unref(dfield);
        m = gwy_container_duplicate(meta);
        gwy_container_pass_object_by_name(container, "/1/meta", m);
                        ...
                        GError **error)
{
    ...
    gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_xy(dfield), xyunits);
                        ...
                        GError **error)
{
    ...
    sanitise_real_size(&xy_step, "step size");
                  ...
                  const NanoeduParameterHeader *param)
{
    ...
    gwy_container_set_string_by_name(meta, "Date",
                                     g_strdup_printf("%u-%02u-%02u "
                                                     ...
                                                     param->second));
    if (*(param->material))
        gwy_container_set_string_by_name(meta, "Material", gwy_convert_to_utf8(param->material, -1, "cp1251"));
    if (*(param->scanner_name))
        gwy_container_set_const_string_by_name(meta, "Scanner number", param->scanner_name);
