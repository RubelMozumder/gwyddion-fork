/*
 *  $Id: renishaw.c 25890 2023-10-22 05:21:24Z yeti-dn $
...
typedef struct {
    ...
    WdfMapArea   *maparea;
    gint         nummaps;
    /* FIXME: There seem to be one indirection too many. WdfMapData are allocated one by one and then the pointers
     * placed to an array, resulting in a lot of pointer juggling. A GArray (or a plain array) of WdfMapData – not
     * pointers to them – would probably simplify the entire thing considerably. */
    WdfMapData   **maps;
} WdfFile;
             ...
             GError **error)
{
    ...
    GwySIUnit *siunitx = NULL, *siunity = NULL, *siunitz = NULL, *siunitw = NULL, *siunitsp = NULL, *siunitm = NULL;
    ...
    p = buffer;
    if ((len = wdf_read_header(p, size, &fileheader, error)) != WDF_HEADER_SIZE)
        goto fail;
    p += WDF_HEADER_SIZE;
    ...
    while (size > 0) {
        ...
        if (block.id == WDF_BLOCKID_DATA) {
            ...
            if (filedata.datasize != fileheader.npoints * fileheader.nspectra * sizeof(gfloat)) {
                err_SIZE_MISMATCH(error,
                                  fileheader.npoints * fileheader.nspectra * sizeof(gfloat),
            ...
            if (block.size != WDF_BLOCK_HEADER_SIZE + 2 * sizeof(guint32) + fileheader.npoints * sizeof(gfloat)) {
                err_SIZE_MISMATCH(error,
                                  WDF_BLOCK_HEADER_SIZE + 2 * sizeof(guint32) + fileheader.npoints * sizeof(gfloat),
                                  block.size, TRUE);
                goto fail;
            }
            ...
            /* Valgrind can make a lot of noise about uninitialised memory use originating here. But it seems to
             * happen entirely inside libjpeg. We just inherit it and then anything we try to do with the returned
             * pixbuf is uninitialised memory use. */
            if (!gdk_pixbuf_loader_write(loader, block.data, block.size, &err)) {
            ...
            if (pset_data) {
                if (block.id == WDF_BLOCKID_CALIBRATION)
                    ...
                    key = g_strdup_printf("/");
                gwy_container_transfer(pset_data, metadata, "/", key, TRUE);
                g_free(key);
            ...
            filedata.maps = g_realloc(filedata.maps, (filedata.nummaps + 1) * sizeof(gpointer));
            filedata.maps[filedata.nummaps] = g_new0(WdfMapData, 1);
            filedata.maps[filedata.nummaps]->pset = wdf_read_pset(p, pset_size);
            p += pset_size;
            filedata.maps[filedata.nummaps]->numpoints = gwy_get_guint64_le(&p);
            filedata.maps[filedata.nummaps]->data = (gfloat *)p;
            filedata.nummaps++;
            p -= WDF_BLOCK_HEADER_SIZE + 2 * sizeof(guint32) + pset_size + sizeof(guint64);
        }
        p += len;
        size -= len;
    }
    ...
    if (fileheader.nspectra == 1) { /* Single spectrum */
        ...
        for (m = 0; m < filedata.nummaps; m++) { /* z maps */
            ...
            if (gwy_container_gis_string_by_name(filedata.maps[m]->pset, "/Label", &label)) {
        ...
        for (z = 0; z < filedata.maparea->length[2]; z++) {
            brick = gwy_brick_new(xres, yres, zres, xreal, yreal, zres, TRUE);
            ...
            /* read data */
            /* XXX: Perhaps try simple gwy_convert_raw_data() and then gwy_brick_transpose() to clean up this?
             * It needs more memory, but gwy_convert_raw_data() avoids a function call for each value and
             * gwy_brick_transpose() tries to always employ good memory access patterns. */
            ...
            if (filedata.maparea->flags & WDF_MAPAREA_COLUMNMAJOR) {
                ...
                if (filedata.maparea->flags & WDF_MAPAREA_ALTERNATING) {
                    ...
                    for (i = xstart; i != xend; i += xstep) {
                        for (j = ystart; j != yend; j += ystep)
                            for (k = 0; k < zres; k++) {
                                data[k * xres * yres + i + j * xres] = (gdouble)gwy_get_gfloat_le(&p) * wscale;
                            }
                        if (ystep < 0) {
                            ystart = 0;
                            yend = yres;
                ...
                if (filedata.maparea->flags & WDF_MAPAREA_ALTERNATING) {
                    for (j = 0; j < yres; j += lsize) {
                        for (i = xstart; i != xend; i += xstep)
                            for (l = 0; (l < lsize) && (l + j * lsize < yres); l++) {
                                for (k = 0; k < zres; k++) {
                                    data[k*xres*yres + i + (j*lsize + l)*xres] = gwy_get_gfloat_le(&p) * wscale;
                                }
                            }
                        if (xstep < 0) {
                            xstart = 0;
                ...
                for (j = ystart; j != yend; j += ystep)
                    for (i = xstart; i != xend; i += xstep)
                        for (k = 0; k < zres; k++) {
                            data[k * xres * yres + i + j * xres] = gwy_get_gfloat_le(&p) * wscale;
                        }
            }
            else {
                if (filedata.maparea->flags & WDF_MAPAREA_ALTERNATING) {
            ...
            gwy_convert_raw_data(filedata.xlistdata, zres, 1, GWY_RAW_DATA_FLOAT, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                                 data, zscale, 0.0);
            ...
            /* offsets of the data */
            gwy_brick_set_xoffset(brick, filedata.maparea->location[0] * xscale);
            gwy_brick_set_yoffset(brick, filedata.maparea->location[1] * yscale);

            /* packing */
            gwy_container_pass_object(container, gwy_app_get_brick_key_for_id(z), brick);
            ...
            gwy_container_set_string(container, gwy_app_get_brick_title_key_for_id(z), title);
            gwy_container_pass_object(container, gwy_app_get_brick_meta_key_for_id(z),
                                      gwy_container_duplicate(metadata));
        ...
        for (m = 0; m < filedata.nummaps; m++) {
            ...
            gwy_si_unit_assign(gwy_data_field_get_si_unit_xy(dfield), siunitx);
            gwy_data_field_set_xoffset(dfield, filedata.maparea->location[0] * xscale);
            gwy_data_field_set_yoffset(dfield, filedata.maparea->location[1] * yscale);
            ...
            if (filedata.maparea->flags & WDF_MAPAREA_COLUMNMAJOR) {
                ...
                if (filedata.maparea->flags & WDF_MAPAREA_ALTERNATING) {
                    ...
                    for (i = xstart; i != xend; i += xstep) {
                        for (j = ystart; j != yend; j += ystep)
                            data[i + j * xres] = (gdouble)gwy_get_gfloat_le(&p);
                        if (ystep < 0) {
                            ystart = 0;
                            yend = yres;
                            ystep = 1;
                ...
                if (filedata.maparea->flags & WDF_MAPAREA_ALTERNATING) {
                    for (j = 0; j < yres; j += lsize) {
                        for (i = xstart; i != xend; i += xstep) {
                            for (l = 0; (l < lsize) && (l + j * lsize < yres); l++) {
                                data[i + (j * lsize + l) * xres] = gwy_get_gfloat_le(&p);
                            }
                        }
                        if (xstep < 0) {
                            xstart = 0;
            ...
            gwy_container_pass_object(container, gwy_app_get_data_key_for_id(m + 1), dfield);
            pset_data = filedata.maps[m]->pset;

            if (gwy_container_gis_string_by_name(pset_data, "/Label", &label))
                gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(m + 1), label);

            hasspectrum = FALSE;
            ydata = g_new(gdouble, zres);
            if (gwy_container_gis_string_by_name(pset_data, "/overlaySpectrum", &spectrum)) {
                mapspectra++;
                hasspectrum = TRUE;
            ...
            siunitsp = NULL;
            if (gwy_container_gis_string_by_name(pset_data, "/overlayXListUnits", &specxunit)) {
            ...
            xdata = g_new(gdouble, zres);
            if (gwy_container_gis_string_by_name(pset_data, "/overlayXList", &spectrumx)) {
            ...
            if (gwy_container_gis_string_by_name(pset_data, "/overlayLabel", &speclabel)) {
            ...
            if (hasspectrum) {
                ...
                gwy_graph_model_add_curve(gmodel, gcmodel);
                g_object_unref(gcmodel);
                gwy_container_pass_object(container, gwy_app_get_graph_key_for_id(mapspectra + 1), gmodel);
            }
            ...
            gwy_container_set_object(container, gwy_app_get_data_meta_key_for_id(m + 1), pset_data);
            gwy_file_channel_import_log_add(container, m + 1, NULL, filename);
    ...
    if (filedata.whitelight) {
        ...
        GWY_OBJECT_UNREF(filedata.whitelight);
    }

    fail:
    for (i = 0; i < filedata.nummaps; i++) {
        GWY_OBJECT_UNREF(filedata.maps[i]->pset);
        g_free(filedata.maps[i]);
    }
    g_free(filedata.maps);
    g_free(filedata.maparea);

    GWY_OBJECT_UNREF(metadata);
    GWY_OBJECT_UNREF(siunitx);
    GWY_OBJECT_UNREF(siunity);
    GWY_OBJECT_UNREF(siunitw);
    g_free(zdata);
    g_free(buffer);
                  ...
                  G_GNUC_UNUSED gpointer value, gpointer user_data)
{
    ...
    gchar *replacement_key_text = NULL;

    pdata = (WdfPsetData *)user_data;
    keys_value = g_hash_table_lookup(pdata->keys, key);

    if (!keys_value) {
        key_text = gwy_enum_to_string(GPOINTER_TO_INT(key), wdf_predefined_keys, 9);
        gwy_debug("key_text=%s", key_text);

        if (gwy_strequal(key_text, "")) {
            replacement_key_text = g_strdup_printf("Key%d", GPOINTER_TO_INT(key));
            key_text = replacement_key_text;
        }
        g_hash_table_replace(pdata->keys, key, g_strdup_printf("/%s", key_text));
        g_free(replacement_key_text);
    }
}
                             ...
                             gpointer user_data)
{
    ...
    if (value && value_value) {
        gwy_container_set_const_string_by_name(pdata->data, value, value_value);
    }
}
              ...
              gsize size)
{
    ...
    remaining = size;
    keys = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, g_free);
    values = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, g_free);
        ...
        if (!pset.flag) {
            switch (pset.type) {
                case WDF_PTYPE_CHAR:
                    c = *(buffer++);
                    gwy_debug("c = %d", c);
                    g_hash_table_replace(values, GINT_TO_POINTER(pset.key), g_strdup_printf("%d", c));
                    remaining -= 1;
                break;
                ...
                case WDF_PTYPE_INT16:
                    ...
                    gwy_debug("s = %d", i);
                    g_hash_table_replace(values, GINT_TO_POINTER(pset.key), g_strdup_printf("%d", i));
                    remaining -= 2;
                break;
                ...
                case WDF_PTYPE_INT64:
                    ...
                    g_hash_table_replace(values, GINT_TO_POINTER(pset.key),
                ...
                case WDF_PTYPE_FLOAT:
                    ...
                    gwy_debug("r = %g", d);
                    g_hash_table_replace(values, GINT_TO_POINTER(pset.key), g_strdup_printf("%g", d));
                    remaining -= 4;
                break;
                ...
                case WDF_PTYPE_TIME:
                    ...
                    g_hash_table_replace(values, GINT_TO_POINTER(pset.key), wdf_read_datetime(i64));
                    remaining -= 8;
                break;
                case WDF_PTYPE_STRING:
                    ...
                    g_hash_table_replace(values, GINT_TO_POINTER(pset.key), g_strndup(str, pset.size));
                    g_free(str);
                ...
                case WDF_PTYPE_NESTED:
                    ...
                    if (nested) {
                        gwy_container_transfer(nested, data, "/", "/", FALSE);
                    }
                ...
                case WDF_PTYPE_KEY:
                    ...
                    g_hash_table_replace(keys, GINT_TO_POINTER(pset.key), g_strdup_printf("/%.*s", pset.size, str));
                    g_free(str);
                    remaining -= 4 + pset.size;
                break;
                default:
            ...
            switch (pset.type) {
                case WDF_PTYPE_CHAR:
                    ...
                    g_hash_table_replace(values, GINT_TO_POINTER(pset.key), str);
                    remaining -= pset.size;
                break;
                case WDF_PTYPE_UINT8:
                    c = *(buffer++);
                ...
                case WDF_PTYPE_INT64:
                    ...
                    for (i = 1; i < pset.size; i++) {
                        i64 = gwy_get_gint64_le(&buffer);
                        newstr = g_strdup_printf("%s; %" G_GINT64_FORMAT "", str, i64);
                        g_free(str);
                        str = newstr;
                    }
    ...
    pdata.values = values;
    pdata.data = data;
    g_hash_table_foreach(values, set_standard_keys, &pdata);
    g_hash_table_foreach(keys, extract_metadata_from_values, &pdata);
