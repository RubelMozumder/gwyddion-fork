/*
 *  $Id: magellan.c 26135 2024-01-15 12:54:27Z yeti-dn $
...
mgl_load_tiff(const GwyTIFF *tiff, const gchar *filename, GError **error)
{
    ...
    for (dir_num = 0; dir_num < gwy_tiff_get_n_dirs(tiff); dir_num++) {
        ...
        gwy_container_pass_object(container, gwy_app_get_data_key_for_id(dir_num), dfield);
        if (name && mode) {
            gwy_container_set_string(container, gwy_app_get_data_title_key_for_id(dir_num),
                                     g_strconcat(name, " ", mode, NULL));
        }
        if ((meta = get_meta(hash)))
            gwy_container_pass_object(container, gwy_app_get_data_meta_key_for_id(dir_num), meta);

        gwy_file_channel_import_log_add(container, dir_num, NULL, filename);
    }
