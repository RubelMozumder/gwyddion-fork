/*
 *  $Id: alicona.c 26135 2024-01-15 12:54:27Z yeti-dn $
          ...
          GError **error)
{
    ...
    if (afile.depthoffset) {
        ...
        if (badmask) {
            ...
            gwy_container_pass_object(container, gwy_app_get_mask_key_for_id(id), badmask);
        }
          ...
          gint component)
{
    ...
    if (gradient) {
        quark = gwy_app_get_data_palette_key_for_id(id);
        gwy_container_set_const_string(container, quark, gradient);
    }
}
                 ...
                 GwyDataField **badmask)
{
    ...
    field = gwy_data_field_new(xres, yres, afile->dx*xres, afile->dy*yres, FALSE);
