/*
 *  $Id: opdfile.c 26163 2024-02-06 16:56:55Z yeti-dn $
 *  Copyright (C) 2008-2023 David Necas (Yeti).
 *
...
enum {
    ...
    OPD_DOUBLE = 8,
    OPD_LONG = 12,
    /* Serialised structs; some look like bits of OPDx, some do not.  If this appears positions in the file are off.
     * Unfortunately not just of the binary stuff but apparently also of other things. */
    OPD_BINARY_STUFF = 15,
} OPDDataType;
           ...
           gboolean only_name)
{
    ...
    if (fileinfo->file_size < BLOCK_SIZE + 2 || memcmp(fileinfo->head, MAGIC, MAGIC_SIZE) != 0)
        return 0;
         ...
         GError **error)
{
    ...
    for (i = j = 0; i < nblocks; i++) {
        ...
        if (header[j].size) {
            gwy_debug("<%s> size=0x%08x, pos=0x%08x, type=%u, flags=0x%04x",
                      header[j].name, header[j].size, header[j].pos, header[j].type, header[j].flags);
            j++;
        }
        if (offset > size) {
    ...
    /* XXX: There can be a block called "\xcaxtendedKe\xfds" at the end that provides mapping from the names truncated
     * to 16 bytes to full names. Unfortunately, it is of type OPD_BINARY_STUFF and therefore in a different position
     * in the file than @pos says... */
    ...
    for (i = j = 0; i < nblocks; i++) {
        ...
        if (!gwy_stramong(header[i].name,
                          "OPD", "SAMPLE_DATA", "RAW_DATA", "RAW DATA", "Image", "Intensity", "SecArr_0", "Raw",
                          NULL))
            continue;
        ...
        dfield = mfield = NULL;
        intensity = gwy_stramong(header[i].name, "Image", "Intensity", "SecArr_0", NULL);
        if (intensity) {
            if (!(dfield = get_data_field(header + i, pixel_size, aspect, 1.0, NULL, NULL, error)))
                goto fail;
        }
        else {
            if (!(dfield = get_data_field(header + i, pixel_size, aspect, wavelength, "m", &mfield, error)))
                goto fail;
        }

        gwy_container_pass_object(container, gwy_app_get_data_key_for_id(j), dfield);
        if (mfield)
            gwy_container_pass_object(container, gwy_app_get_mask_key_for_id(j), mfield);

        s = g_strdup_printf("%s/title", g_quark_to_string(gwy_app_get_data_key_for_id(j)));
        if (intensity)
            gwy_container_set_const_string_by_name(container, s, header[i].name);
        else if ((k = find_block(header, nblocks, "Title")) != nblocks)
            gwy_container_set_string_by_name(container, s, g_strndup(header[k].data, header[k].size));
        else
            gwy_app_channel_title_fall_back(container, j);
        g_free(s);

        s = g_strdup_printf("%s/realsquare", g_quark_to_string(gwy_app_get_data_key_for_id(j)));
...
get_block(OPDBlock *block, const guchar **p)
{
    ...
    *p += BLOCK_NAME_SIZE;
    ...
    if (strncmp(block->name, "AdjustVSI_", sizeof("AdjustVSI_")-1) == 0 && block->type == 7 && block->size == 2) {
        gwy_info("Changing the type of field %s from float to short.", block->name);
        block->type = 6;
    }
    else if (gwy_strequal(block->name, "ImageModificat~0") && block->type == 7 && block->size == 40) {
        gwy_info("Changing the size of field %s from 40 to 4.", block->name);
        block->size = 4;
    }
    else if (block->type == 6 && block->size == 4) {
        gwy_info("Changing the type of field %s from short to long.", block->name);
        block->type = OPD_LONG;
    }
}
          ...
          GError **error)
{
    ...
    gwy_debug("trying to read float %s", name);
          ...
          GError **error)
{
    ...
    gwy_debug("trying to read int16 %s", name);
    if ((i = find_block(header, nblocks, name)) == nblocks) {
        err_MISSING_FIELD(error, name);
        return FALSE;
    }
               ...
               GError **error)
{
    ...
    dfield = gwy_data_field_new(xres, yres, xres*pixel_size, aspect*yres*pixel_size, FALSE);

    gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_xy(dfield), "m");
    gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_z(dfield), zunits);
         ...
         guint nblocks)
{
    ...
    for (i = 0; i < nblocks; i++) {
        ...
        s = NULL;
        if (type == OPD_TEXT) {
            s = gwy_convert_to_utf8(header[i].data, header[i].size, "ISO-8859-1");
            if (s && !*s) {
                g_free(s);
                s = NULL;
            }
            ...
            gint32 value = (type == OPD_SHORT) ? gwy_get_gint16_le(&p) : gwy_get_gint32_le(&p);
            s = g_strdup_printf("%d", value);
        }
        else if (type == OPD_FLOAT || type == OPD_DOUBLE) {
            gdouble value = (type == OPD_FLOAT) ? gwy_get_gfloat_le(&p) : gwy_get_gdouble_le(&p);
            ...
            GError **error)
{
    ...
    for (i = 0; i < nblocks; i++) {
        ...
        if (type < G_N_ELEMENTS(sizes)) {
            if (sizes[type]) {
                if (size != sizes[type]) {
                    gwy_debug("for %s (type %d) header says %u, type size is %u",
                              header[i].name, type, size, sizes[type]);
                    err_INVALID(error, header[i].name);
                    return FALSE;
                }
            }
            else if (type == OPD_DIRECTORY) {
                g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA, _("Nested directories found"));
                return FALSE;
            }
                ...
                gwy_debug("%s xres=%u yres=%u type=%u size=%u", header[i].name, xres, yres, type, size);
                if (size < 3*2 + xres*yres*type) {
                    err_INVALID(error, header[i].name);
                    return FALSE;
                }
            }
...
{
    if (only_name)
        return g_str_has_suffix(fileinfo->name_lowercase, EXTENSION_ASC) ? 10 : 0;

    if (fileinfo->file_size < MAGIC_ASC_SIZE + 2 || memcmp(fileinfo->head, MAGIC_ASC, MAGIC_ASC_SIZE) != 0)
        return 0;
             ...
             GError **error)
{
    ...
    if (sscanf(line + MAGIC_ASC_SIZE, "%d %d %d", &tmp, &real_units, &is_float) != 3) {
        err_INVALID(error, "Data File Format");
        goto fail;
    }
    format = tmp;
    gwy_debug("array format %d, real units %d, is float %d", format, real_units, is_float);
    ...
    while ((s = line = gwy_str_next_line(&p))) {
        ...
        *s = '\0';
        ...
        if (gwy_stramong(line,
            ...
            if (!(s = g_hash_table_lookup(hash, "Pixel_size")) || !(pixel_size = fabs(g_ascii_strtod(s, NULL)))) {
                err_MISSING_FIELD(error, "Pixel_size");
                goto fail;
            }
            gwy_debug("pixel_size = %g", pixel_size);
            if (!(s = g_hash_table_lookup(hash, "Wavelength")) || !(wavelength = fabs(g_ascii_strtod(s, NULL)))) {
            ...
            intensity = gwy_stramong(line, "Image", "Intensity", "SecArr_0", NULL);
            ...
            dfield = mfield = NULL;
            dfield = get_asc_data_field(&p, format, xres, yres, pixel_size, aspect, zcalib,
                                        intensity ? "" : "m",
            ...
            gwy_container_set_object(container, gwy_app_get_data_key_for_id(j), dfield);
            if (mfield)
                gwy_container_set_object(container, gwy_app_get_mask_key_for_id(j), mfield);

            s = g_strdup_printf("%s/title", g_quark_to_string(gwy_app_get_data_key_for_id(j)));
            if (intensity)
                gwy_container_set_const_string_by_name(container, s, line);
            else if ((k = g_hash_table_lookup(hash, "Title")))
                gwy_container_set_const_string_by_name(container, s, k);
            else
                gwy_app_channel_title_fall_back(container, j);
            g_free(s);
               ...
               gpointer user_data)
{
    ...
    if (!(cval = gwy_convert_to_utf8(value, -1, "ISO-8859-1")))
        return;
...
clone_meta(GwyContainer *container, GwyContainer *meta, guint nchannels)
{
    ...
    for (i = 0; i < nchannels; i++) {
        ...
        gwy_container_pass_object_by_name(container, s, m);
    }
}
