/*
 *  $Id: lextfile.c 26182 2024-02-16 16:06:42Z yeti-dn $
...
lext_load_tiff(const GwyTIFF *tiff, const gchar *filename, GError **error)
{
    ...
    const gchar *value, *image0title, *keytitle, *channeltitle, *unitstr;
    GError *err = NULL;
    ...
    lfile.hash = hash = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
    lfile.path = key = g_string_new(NULL);
    lfile.toplevel = "TiffTagDescData";
    lfile.xcal = lfile.ycal = lfile.zcal = 1.0;
    context = g_markup_parse_context_new(&parser, G_MARKUP_TREAT_CDATA_AS_TEXT, &lfile, NULL);
    ...
    for (dir_num = 0; dir_num < gwy_tiff_get_n_dirs(tiff); dir_num++) {
        ...
        if (!dir_num) {
            ...
            if (!gwy_tiff_get_string(tiff, dir_num, GWY_TIFFTAG_IMAGE_DESCRIPTION, &title)) {
         ...
         * FIXME: INVALID is probably the mask of invalid pixels and we might want to import it. */
        ...
        g_string_printf(key, "/TiffTagDescData/%sInfo/%sDataPerPixelX", keytitle, keytitle);
        ...
        g_string_printf(key, "/TiffTagDescData/%sInfo/%sDataPerPixelY", keytitle, keytitle);
        if (!(value = g_hash_table_lookup(hash, (gpointer)key->str))) {
            g_warning("Cannot find %s", key->str);
        }
        yscale = Picometer * lfile.ycal * g_ascii_strtod(value, NULL);

        g_string_printf(key, "/TiffTagDescData/%sInfo/%sDataPerPixelZ", keytitle, keytitle);
        if (!(value = g_hash_table_lookup(hash, (gpointer)key->str))) {
            g_warning("Cannot find %s", key->str);
        }
        ...
        for (ch = 0; ch < spp; ch++) {
            dfield = gwy_data_field_new(reader->width, reader->height,
                                        reader->width * xscale, reader->height * yscale,
                                        FALSE);
            // units
            ...
            for (i = 0; i < reader->height; i++)
                gwy_tiff_read_image_row(tiff, reader, ch, i, zfactor, 0.0, data + i*reader->width);
            ...
            if (spp == 3) {
                ...
                gwy_container_set_const_string_by_name(container, key->str, colour_channel_gradients[ch]);
            }
...
guess_image0_title(const GwyTIFF *tiff)
{
    ...
    if (!gwy_tiff_get_uints(tiff, 0, GWY_TIFFTAG_BITS_PER_SAMPLE, spp, FALSE, bpps)) {
        g_free(bpps);
        return NULL;
    }
    bpp0 = bpps[0];
...
add_info_from_exif(LextFile *lfile, const GwyTIFF *tiff)
{
    ...
    if (!(tags = gwy_tiff_scan_ifd(tiff, exifpos, NULL, NULL)) || !gwy_tiff_ifd_is_vaild(tiff, tags, NULL))
        goto fail;

    gwy_tiff_sort_tag_array(tags);
    if (!(entry = gwy_tiff_find_tag_in_dir(tags, xmltag)) || !gwy_tiff_get_string_entry(tiff, entry, &comment))
        goto fail;
    ...
    key = lfile->path;
    g_string_assign(key, "/ExifTagDescData/ImageCommonSettingsInfo/MakerCalibrationValue");
    len = key->len;
    ...
    if ((value = g_hash_table_lookup(lfile->hash, (gpointer)key->str))) {
        ...
        sanitise_real_size(&lfile->xcal, "xcal");
    }
    ...
    if ((value = g_hash_table_lookup(lfile->hash, (gpointer)key->str))) {
        ...
        sanitise_real_size(&lfile->ycal, "ycal");
    }
...
create_metadata(LextFile *lfile)
{
    ...
    /* XXX: We could also extract date & time from EXIT TIFF tags and put them to the metadata... */
}
