/*
 *  $Id: opengps.c 26168 2024-02-08 10:18:02Z yeti-dn $
           ...
           gboolean only_name)
{
    ...
    if (!gwy_memmem(fileinfo->head, fileinfo->buffer_len, MAGIC1, MAGIC1_SIZE)
        && !gwy_memmem(fileinfo->tail, fileinfo->buffer_len, MAGIC1, MAGIC1_SIZE)
        && !gwy_memmem(fileinfo->head, fileinfo->buffer_len, MAGIC2, MAGIC2_SIZE)
        && !gwy_memmem(fileinfo->tail, fileinfo->buffer_len, MAGIC2, MAGIC2_SIZE))
        return 0;

    /* We have to realy look inside.  And since main.xml is a popular name for the main XML document within such
     * files, we also have to see if we find "ISO5436_2" somewehre near the begining of the file. */
...
create_images(const X3PFile *x3pfile, GwyContainer *container)
{
    ...
    for (id = 0; id < x3pfile->zres; id++) {
        ...
        dfield = gwy_data_field_new(x3pfile->xres, x3pfile->yres,
                                    x3pfile->xres*x3pfile->dx, x3pfile->yres*x3pfile->dy,
                                    FALSE);
        ...
        if ((mask = gwy_app_channel_mask_of_nans(dfield, TRUE))) {
            ...
            gwy_container_pass_object(container, quark, mask);
        }
        g_object_unref(dfield);

        if ((meta = get_meta(x3pfile))) {
            g_snprintf(buf, sizeof(buf), "/%u/meta", id);
            gwy_container_pass_object_by_name(container, buf, meta);
        }
    }
}
                ...
                GwyContainer *container)
{
    ...
    for (id = 0; id < x3pfile->zres; id++) {
        ...
        g_object_set(gcmodel,
                     ...
                     NULL);
        g_free(title);
        gwy_graph_curve_model_set_data(gcmodel, (gdouble*)validx->data, (gdouble*)validy->data, validx->len);
                  ...
                  GError **error)
{
    ...
    if (gwy_strequal(path, "/ISO5436_2/Record3/DataLink") || gwy_strequal(path, "/ISO5436_2/Record3/DataList")) {
                ...
                GError **error)
{
    ...
    if (gwy_strequal(path, "/ISO5436_2/Record3/DataList/Datum") && !x3pfile->seen_datum) {
        if (x3pfile->datapos >= x3pfile->ndata) {
            g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                        _("Too many DataList items for given matrix dimensions."));
            return;
        }
         ...
         G_GNUC_UNUSED GError **error)
{
    ...
    if (gwy_strequal(path, "/ISO5436_2/Record3/DataList/Datum")) {
        ...
        /* There can be a ;-separated list.  And it can contain dummy values for the unused y-axis for profiles (poor
         * design).  Since we do not care about PCL, we only ever want the last value of the list, whatever it is. */
        if ((semicolon = strlenrchr(text, ';', text_len)))
            text = semicolon + 1;

        x3pfile->values[x3pfile->datapos] = x3pfile->dz*g_ascii_strtod(text, NULL) + x3pfile->zoff;
               ...
               GError **error)
{
    ...
    if (!x3pfile->hash)
        x3pfile->hash = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
...
/* This is the main verification function that checks we have everything we need and the data are of a supported type.
 * */
static gboolean
data_start(X3PFile *x3pfile, GError **error)
{
    ...
    /* First check axes to get meaningful error messages if their types are not as expected. */
    ...
    s = (gchar*)g_hash_table_lookup(x3pfile->hash, "/ISO5436_2/Record1/FeatureType");
    x3pfile->feature_type = gwy_string_to_enum(s, features, G_N_ELEMENTS(features));
    if (x3pfile->feature_type == (X3PFeatureType)-1) {
        err_UNSUPPORTED(error, "/ISO5436_2/Record1/FeatureType");
        return FALSE;
    }

    if (x3pfile->feature_type != X3P_FEATURE_SUR && x3pfile->feature_type != X3P_FEATURE_PRF) {
        err_UNSUPPORTED(error, "/ISO5436_2/Record1/FeatureType");
        return FALSE;
    }
    ...
    /* Defaults that are good for floating point data conversion.  If a file with floating point data specifies
     * Increment and Offset, we apply them without hesitation.  The behaviour is probably undefined.  */
    x3pfile->dz = 1.0;
    x3pfile->zoff = 0.0;
...
read_binary_data(X3PFile *x3pfile, GwyZipFile zipfile, GError **error)
{
    ...
    if (err_SIZE_MISMATCH(error, x3pfile->ndata * gwy_raw_data_size(rawtype), size, TRUE)) {
        g_free(bindata);
        return FALSE;
    }

    gwy_convert_raw_data(bindata, x3pfile->ndata, 1, rawtype, GWY_BYTE_ORDER_LITTLE_ENDIAN,
    ...
    if (!gwyzip_locate_file(zipfile, s, 1, error) || !(bindata = gwyzip_get_file_content(zipfile, &size, error)))
        return FALSE;
...
add_meta_record(gpointer hkey, gpointer hvalue, gpointer user_data)
{
    ...
    if (!gwy_stramong(key,
                      ...
                      NULL)
        && !g_str_has_prefix(key, "/ISO5436_2/Record2/ProbingSystem/Identification/")
        && !g_str_has_prefix(key, "/ISO5436_2/Record1/Axes/Rotation"))
        return;
