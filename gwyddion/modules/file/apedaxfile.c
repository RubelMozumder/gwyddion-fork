/*
 *  $Id: apedaxfile.c 25789 2023-10-06 14:44:53Z yeti-dn $
              ...
              gboolean only_name)
{
    ...
    gwy_debug("Opening the file with MiniZIP/libzip");
    uFile = gwyzip_open(fileinfo->name, NULL);
            ...
            GError **error)
{
    ...
    gwy_debug("Opening the file with MiniZIP/libzip");
    uFile = gwyzip_open(filename, error);
    ...
    channels_data_result = apedax_get_channels_data(uFile, buffer, content_size,
                                                    filename, container, meta,
                                                    &scan_size, error);
                ...
                gboolean apdt_file)
{
    ...
    doc = xmlReadMemory(scan_xml_content, content_size, XML_FILE_NAME, NULL, 0);
    ...
    while (cur) {
        if (cur->type == XML_ELEMENT_NODE) {
            ...
            while (cur_child) {
                if (cur_child->type == XML_ELEMENT_NODE) {
                    ...
                    if (value != NULL) {
                        ...
                        gwy_container_set_const_string_by_name(meta, concat_key, (gchar*)value);
                        g_free(concat_key);
                        xmlFree(value);
                        have_child = TRUE;
                    }
            ...
            if (!have_child) {
                ...
                if (value != NULL) {
                    gwy_container_set_const_string_by_name(meta, key, (gchar*)value);
                    xmlFree(value);
                }
            }
        }
    ...
    if (apdt_file) {
        ...
        if (gwy_container_contains_by_name(meta, SPM_MODE_NAME)) {
            ...
            current_SPM_mode = gwy_string_to_enum(buffer, spm_modes_names, G_N_ELEMENTS(spm_modes_names));
            spm_mode_disp = gwy_enum_to_string(current_SPM_mode, spm_modes_display_names,
                                               G_N_ELEMENTS(spm_modes_display_names));
            gwy_container_set_const_string_by_name(meta, SPM_MODE_NAME, spm_mode_disp);
            ...
            for (i = 0; i < fields_size; i++) {
                if (gwy_container_contains_by_name(meta, fields[i].name)) {
                    gwy_container_rename_by_name(meta, fields[i].name, fields[i].display_name, FALSE);
                }
                else {
    ...
    if (path_object != NULL) {
        ...
        if (!xmlXPathNodeSetIsEmpty(path_object->nodesetval)) {
            ...
            if (node_set->nodeNr == 1) {
                node_xstring = xmlNodeListGetString(doc, node_set->nodeTab[0]->xmlChildrenNode, 1);
                      ...
                      GError **error)
{
    ...
    gwy_convert_raw_data(buffer, scan_size->XRes * scan_size->YRes, 1,
    ...
    for (i = 0; i < node_set->nodeNr; i++) {
        ...
        while (cur) {
            ...
            if (buffer != NULL) {
                ...
                /* Factor */
                else if (gwy_strequal(curname, XML_CHANNEL_CONVERSION_FACTOR_NAME)) {
                ...
                /* Binary file name */
                    ...
                    gwy_container_set_const_string_by_name(channel_meta, curname, (gchar*)buffer);
                }
                xmlFree(buffer);
            }
            cur = cur->next;
        ...
        if (bin_filename != NULL && zUnit_string != NULL && label != NULL) {
            ...
            scale_factor *= pow10(power10);

            dfield = apedax_get_data_field(uFile, bin_filename, scan_size, zUnit_string, scale_factor, error);
            if (dfield) {
                gwy_container_pass_object(container, gwy_app_get_data_key_for_id(i), dfield);
                gwy_container_set_object(container, gwy_app_get_data_meta_key_for_id(i), channel_meta);
...
apedax_is_xml_valid(guchar *scan_xml_content, gsize content_size)
{
    ...
    for (i = 0; i < num_elements; i++) {
        if (!apedax_is_xml_node_present(doc, scanSize_string, scanSizeArray[i], TRUE)) {
            xmlFreeDoc(doc);
            return FALSE;
        }
    }
    ...
    for (i = 0; i < num_elements; i++) {
        if (!apedax_is_xml_node_present(doc, channel_string, channel[i], FALSE)) {
            channel_ok = FALSE;
            break;
        }
    }
                           ...
                           gboolean unique)
{
    ...
    path_object = xmlXPathEvalExpression((const xmlChar*)node_absolute_path, context);
