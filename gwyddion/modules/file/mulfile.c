/*
 *  $Id: mulfile.c 25636 2023-09-07 12:19:33Z yeti-dn $
...
/* This is actually the number and address of the first data as there is no real identifier.  It should be constant,
 * though. */
...
typedef struct {
    ...
    gdouble xdim, ydim;    /* In Angström; they are ints in the file but useful to keep as doubles. */
    gdouble xoff, yoff;    /* In Angström; ditto. */
               ...
               MulIndexEntry *image_index)
{
    ...
    for (i = 0; i < MUL_INDEX_LENGTH; i++) {
        ...
        if (image_index[n].id) {
            if (image_index[n].addr >= size/MUL_BLOCK_SIZE || image_index[n].addr < 3)
                g_warning("Address of block %u is invalid.", image_index[n].id);
            else
                n++;
        }
        else if (first_zero) {
            /* The first zero-id entry contains the file size, apparently. */
            if (image_index[n].addr && MUL_BLOCK_SIZE * image_index[n].addr != size) {
                g_warning("The sentinel zero-id address is 0x%08lx but we would expect 0x%08lx.",
                          (gulong)image_index[n].addr, (gulong)(size/MUL_BLOCK_SIZE));
            }
            first_zero = FALSE;
        }
    }
                     ...
                     GError **error)
{
    ...
    if (label->id != entry->id) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Image number in the label %u does not match the number %u in the index."),
                    label->id, entry->id);
        return FALSE;
    }
    ...
    gwy_debug("[%u] xres: %u, yres: %u, zres: %u", label->id, label->xres, label->yres, label->zres);
    if (err_DIMENSION(error, label->xres) || err_DIMENSION(error, label->yres))
        return FALSE;
    /* The extra MUL_BLOCK_SIZE is for the label itself. */
    if (err_SIZE_MISMATCH(error, 2*label->xres * label->yres + MUL_BLOCK_SIZE, label->size*MUL_BLOCK_SIZE, FALSE))
        return FALSE;
    ...
    gwy_debug("[%u] %u-%u-%u %u:%u:%u",
              label->id, label->year, label->month, label->day, label->hour, label->minute, label->second);
    label->xdim = gwy_get_gint16_le(&p);
    label->ydim = gwy_get_gint16_le(&p);
    gwy_debug("[%u] (%u, %u)", label->id, label->xdim, label->ydim);
    sanitise_real_size(&label->xdim, "x size");
    sanitise_real_size(&label->ydim, "y size");
    ...
    gwy_debug("[%u] sample: <%s>, title: <%s>", label->id, label->sample, label->title);
...
mul_read_image(GwyContainer *container,
               ...
               const gchar *filename)
{
    const guchar *p = (buffer + (entry->addr + 1)* MUL_BLOCK_SIZE);
    GwyDataField *field;
    ...
    field = gwy_data_field_new(label->xres, label->yres, Angstrom*label->xdim, Angstrom*label->ydim, FALSE);
    /*
    ...
    */
    ...
    rawtype = (label->mode == MUL_MODE_CURRENT ? GWY_RAW_DATA_UINT16 : GWY_RAW_DATA_SINT16);
    data = gwy_data_field_get_data(field);
    for (i = 0; i < label->yres; i++) {
        gwy_convert_raw_data(p + i*label->xres*gwy_raw_data_size(rawtype), label->xres, 1,
                             rawtype, GWY_BYTE_ORDER_LITTLE_ENDIAN, data + (label->yres-1 - i)*label->xres, q, 0.0);
    }

    g_snprintf(key, sizeof(key), "/%d/data", label->id);
    gwy_container_pass_object_by_name(container, key, field);
    ...
    gwy_container_pass_object_by_name(container, key, meta);
...
mul_get_meta(const MulImageLabel *label)
{
    ...
    gwy_container_set_string_by_name(meta, "Date",
                                     g_strdup_printf("%u-%02u-%02u %02u:%02u:%02u",
                                                     label->year, label->month, label->day,
                                                     label->hour, label->minute, label->second));
    gwy_container_set_string_by_name(meta, "X-Offset", g_strdup_printf("%g nm", label->xoff/10.0));
    gwy_container_set_string_by_name(meta, "Y-Offset", g_strdup_printf("%g nm", label->yoff/10.0));
    gwy_container_set_string_by_name(meta, "Z-Scale", g_strdup_printf("%u V", label->zscale));
    gwy_container_set_string_by_name(meta, "Tilt", g_strdup_printf("%u deg", label->tilt));
    gwy_container_set_string_by_name(meta, "Scan duration", g_strdup_printf("%g s", label->speed/100.0));
    gwy_container_set_string_by_name(meta, "Bias", g_strdup_printf("%g V", -10.0*label->bias/32768.0));
    gwy_container_set_string_by_name(meta, "Current", g_strdup_printf("%g nA", label->current/100.0));
    gwy_container_set_string_by_name(meta, "Bias (corrected)",
                                     g_strdup_printf("%g V", -10.0*label->bias/32768.0 - 1e-6*label->current));
    gwy_container_set_string_by_name(meta, "Postprocessing", g_strdup_printf("%u", label->postpr));
    gwy_container_set_string_by_name(meta, "Current factor", g_strdup_printf("%u", label->curr_factor));
    gwy_container_set_string_by_name(meta, "Mode", g_strdup_printf("%u", label->mode));
    gwy_container_set_string_by_name(meta, "R_Nr", g_strdup_printf("%u", label->n_point_scans));
    gwy_container_set_string_by_name(meta, "Unit number", g_strdup_printf("%u", label->unitnr));
    gwy_container_set_string_by_name(meta, "Version", g_strdup_printf("%u", label->version));
    gwy_container_set_string_by_name(meta, "Gain", g_strdup_printf("%u", label->spare_60));
    gwy_container_set_string_by_name(meta, "STM", g_strdup_printf("%u", label->spare_61));
