/*
 *  $Id: burleigh.c 25675 2023-09-18 05:41:59Z yeti-dn $
...
typedef struct {
    ...
    guint32 yrangemax;
    guint32 zrangemax;
    gdouble xrange;  /* Represented as int in the file, but we keep it as double. */
    gdouble yrange;  /* Ditto. */
    guint32 zrange;
    guint32 data_type;
    guint32 scan_speed;
    gdouble z_gain;
              ...
              GError **error)
{
    ...
    if (imgfile.version_int == 21) {
        ...
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("File format version %.1f is not supported."), imgfile.version);
        gwy_file_abandon_contents(buffer, size, NULL);
        return NULL;
    }

    zoom = burleigh_get_zoom_v21(&imgfile);
    if (err_DIMENSION(error, imgfile.xres) || err_DIMENSION(error, imgfile.yres)) {
        gwy_file_abandon_contents(buffer, size, NULL);
        return NULL;
    }
    ...
    switch (imgfile.data_type) {
        ...
        unit = "A";
        gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(0), "Current");
                  ...
                  GError **error)
{
    ...
    sanitise_real_size(&imgfile->xrange, "x size");
    sanitise_real_size(&imgfile->yrange, "y size");
