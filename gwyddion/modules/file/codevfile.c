/*
 *  $Id: codevfile.c 26163 2024-02-06 16:56:55Z yeti-dn $
...
split_line_in_place(gchar *line)
{
    ...
    n = 0;
    for (i = 0; line[i]; i++) {
        if ((!i || g_ascii_isspace(line[i-1]) || !line[i-1]) && (line[i] && !g_ascii_isspace(line[i])))
            strs[n++] = line + i;
         ...
         GError **error)
{
    ...
    /* Skip comments. */
    p = buffer;
    for (line = gwy_str_next_line(&p); line && line[0] == '!'; line = gwy_str_next_line(&p)) {
    ...
    if (!fields
        ...
        || !(xres = atoi(fields[1]))
        || !(yres = atoi(fields[2]))
        || !(type = gwy_stramong(fields[3], "SUR", "WFR", "FIL", "THV", "BIR", "CAO", NULL))
        || !gwy_strequal(fields[4], "WVL")
        || (!(wavelength = g_ascii_strtod(fields[5], &end)) && end == fields[5])) {
        err_FILE_TYPE(error, "Code V INT");
        goto fail;
    }
    ...
    sanitise_real_size(&scale_size, "SSZ");
    fi++;
    ...
    sanitise_real_size(&x_scale, "XSC");
    ...
    if (type == CODEV_INT_SURFACE_DEFORMATION) {
        ...
        g_warning("Don't know how to convert this grid data type to physical units.");
        title = fields[3];
    }
    ...
    for (i = 0; i < xres*yres; i++, p = end) {
        ...
        if (value != no_data_value && (type != CODEV_INT_INTENSITY_FILTER || value >= 0)) {
            mdata[i] = 1.0;
            data[i] = q*value;
        }
    }
     ...
     * from F. Riguet : apparently no flip is needed (the raw data import module gives the correct orientation without
     * further flipping) */
    ...
    gwy_container_set_const_string_by_name(meta, "Comment", comment);
    gwy_container_set_const_string_by_name(meta, "Interpolation", nearest_neighbour ? "NNB" : "Linear");
    gwy_container_set_string_by_name(meta, "Wavelength", g_strdup_printf("%g μm", wavelength));
