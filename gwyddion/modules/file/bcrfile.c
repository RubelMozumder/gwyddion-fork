/*
 *  $Id: bcrfile.c 25669 2023-09-14 14:49:06Z yeti-dn $
...
#define MAGIC3 "f\0i\0l\0e\0f\0o\0r\0m\0a\0t\0 \0=\0 \0b\0c\0r\0s\0t\0m\0_\0u\0n\0i\0c\0o\0d\0e\0\n\0"
#define MAGIC_SIZE3 (sizeof(MAGIC3) - 1)

/* floats, text header is in Microsoft-style wide characters (utf16) */
#define MAGIC4 "f\0i\0l\0e\0f\0o\0r\0m\0a\0t\0 \0=\0 \0b\0c\0r\0f\0_\0u\0n\0i\0c\0o\0d\0e\0\n\0"
             ...
             GError **error)
{
    ...
    if (size > 2*HEADER_SIZE && (memcmp(buffer, MAGIC3, MAGIC_SIZE3) == 0 || memcmp(buffer, MAGIC4, MAGIC_SIZE4) == 0))
        utf16 = TRUE;
    ...
    if (utf16) {
        ...
        header = gwy_utf16_to_utf8((const gunichar2*)buffer, header_size/2, GWY_BYTE_ORDER_LITTLE_ENDIAN);
        if (!header) {
            g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                        _("Cannot convert string from UTF-16."));
            gwy_file_abandon_contents(buffer, size, NULL);
            return NULL;
        }
    ...
    dfield = file_load_real(buffer + header_size, size, header_size, bcrmeta, &voidmask, error);
    gwy_file_abandon_contents(buffer, size, NULL);
    if (dfield) {
        ...
        if (voidmask)
            gwy_container_pass_object(container, gwy_app_get_mask_key_for_id(0), voidmask);
...
/* Newer files contain header size embedded in the header.  Change header_size if we find it. */
static void
                 ...
                 gsize *header_size)
{
    ...
    s = gwy_memmem(buffer, *header_size, HEADER_SIZE_MAGIC, sizeof(HEADER_SIZE_MAGIC)-1);
    if (!s)
        return;
    ...
    while (s - buffer < *header_size && (c = gwy_get_guint16_le(&s)) == ' ')
        ;
    s -= sizeof(guint16);
    while (s - buffer < *header_size && (c = gwy_get_guint16_le(&s)) && g_ascii_isdigit(c)) {
        size *= 10;
        size += c - '0';
    }
    if (size > 0) {
...
{
    GwyDataField *dfield;
    GwySIUnit *siunit2 = NULL;
    gboolean voidpixels = FALSE;
    GwyByteOrder byteorder = GWY_BYTE_ORDER_LITTLE_ENDIAN;
    GwyRawDataType rawtype;
    gdouble q, qq;
    ...
    if ((s = g_hash_table_lookup(meta, "intelmode")))
        byteorder = (atol(s) ? GWY_BYTE_ORDER_LITTLE_ENDIAN : GWY_BYTE_ORDER_BIG_ENDIAN);

    if (err_SIZE_MISMATCH(error, xres*yres*gwy_raw_data_size(rawtype), (guint)(size - header_size), FALSE))
        return NULL;

    /* Do not trust `voidpixels' and always read data assuming the rules for
     * bad pixels. */
    dfield = read_data_field(buffer, xres, yres, rawtype, byteorder, voidmask);

    if ((s = g_hash_table_lookup(meta, "xlength")) && (q = g_ascii_strtod(s, NULL)) > 0) {
        if (!(s = g_hash_table_lookup(meta, "xunit")))
            s = "nm";

        gwy_si_unit_set_from_string_parse(gwy_data_field_get_si_unit_xy(dfield), s, &power10);
        q *= pow10(power10);
        gwy_data_field_set_xreal(dfield, q);
    }

    if ((s = g_hash_table_lookup(meta, "ylength")) && (q = g_ascii_strtod(s, NULL)) > 0) {
        if (!(s = g_hash_table_lookup(meta, "yunit")))
            s = "nm";

        siunit2 = gwy_si_unit_new_parse(s, &power10);
        q *= pow10(power10);
        if (!gwy_si_unit_equal(gwy_data_field_get_si_unit_xy(dfield), siunit2))
    ...
    if (rawtype == GWY_RAW_DATA_SINT16) {
        if ((s = g_hash_table_lookup(meta, "bit2nm")) && (qq = g_ascii_strtod(s, NULL)) > 0)
            gwy_data_field_multiply(dfield, q*qq);
    }
    else if (rawtype == GWY_RAW_DATA_FLOAT)
        gwy_data_field_multiply(dfield, q);

    if ((s = g_hash_table_lookup(meta, "zmin")) && (qq = g_ascii_strtod(s, NULL)) > 0)
    ...
    if (voidpixels) {
        ...
        gwy_si_unit_assign(gwy_data_field_get_si_unit_xy(*voidmask), gwy_data_field_get_si_unit_xy(dfield));
        gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_z(*voidmask), NULL);
    }
...
bcrfile_get_metadata(GHashTable *bcrmeta)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(metakeys); i++) {
        ...
        if (metakeys[i].unit)
            gwy_container_set_string_by_name(meta, metakeys[i].key, g_strdup_printf("%s %s", value, metakeys[i].unit));
        else
            gwy_container_set_const_string_by_name(meta, metakeys[i].key, value);
    }
                ...
                GwyDataField **voidmask)
{
    ...
    *voidmask = gwy_data_field_new(xres, yres, 1e-6, 1e-6, FALSE);
    ...
    gwy_convert_raw_data(buffer, n, 1, rawtype, byteorder, data, 1.0, 0.0);
    if (rawtype == GWY_RAW_DATA_SINT16) {
        for (i = 0; i < n; i++) {
            if (data[i] == 32767.0)
                voids[i] = 0.0;
