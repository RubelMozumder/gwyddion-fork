/*
 *  $Id: apefile.c 25689 2023-09-20 15:55:12Z yeti-dn $
...
typedef struct {
    ...
    gdouble range_y;
    guint subversion;
    /* The new fields change the conversion rules, however, the simple factor above are written so that old software
     * performs the conversion corerectly (they contain pre-multiplied or pre-divided values).  So we use that and do
     * not care about hv_gain_z or fast2_* */
    /* Since 2.1 */
    gdouble hv_gain_z;
    /* Since 2.2 */
    gdouble fast2_0;
               ...
               gboolean only_name)
{
    ...
    vbtype = gwy_get_guint16_le(&buffer);
                                        /* reserve */
    if (version >= 1 && version <= 2 && mode < SPM_MODE_LAST+2 && vbtype == 7) {
        score = 60;
             ...
             GError **error)
{
    ...
    /* Read everything since the header is long enough, check the version later when we decide whether to use these
     * values or not. */
    /* Since 2.1 */
    ...
    apefile.xreal = apefile.maxr_x * apefile.x_piezo_factor * apefile.range_x * apefile.hv_gain/65535.0 * 1e-9;
    apefile.yreal = apefile.maxr_y * apefile.y_piezo_factor * apefile.range_y * apefile.hv_gain/65535.0 * 1e-9;
    sanitise_real_size(&apefile.xreal, "x size");
    sanitise_real_size(&apefile.yreal, "y size");

    gwy_debug("version = %u.%u, spm_mode = %u", apefile.version, apefile.subversion, apefile.spm_mode);
    gwy_debug("scan_date = %f", apefile.scan_date);
    gwy_debug("maxr_x = %g, maxr_y = %g", apefile.maxr_x, apefile.maxr_y);
    gwy_debug("x_offset = %u, y_offset = %u", apefile.x_offset, apefile.y_offset);
    ...
    gwy_debug("v_ref = %g, vpmt1 = %g, vpmt2 = %g", apefile.v_ref, apefile.vpmt1, apefile.vpmt2);
    gwy_debug("x_piezo_factor = %u, y_piezo_factor = %u, z_piezo_factor = %u",
              apefile.x_piezo_factor, apefile.y_piezo_factor, apefile.z_piezo_factor);
    gwy_debug("hv_gain = %g, freq_osc_tip = %g, rotate = %g", apefile.hv_gain, apefile.freq_osc_tip, apefile.rotate);
    gwy_debug("slope_x = %g, slope_y = %g", apefile.slope_x, apefile.slope_y);
    gwy_debug("topo_means = %u, optical_means = %u, error_means = %u",
              apefile.topo_means, apefile.optical_means, apefile.error_means);
    gwy_debug("channel bitmask = %03x, ndata = %u", apefile.channels, apefile.ndata);
    gwy_debug("range_x = %g, range_y = %g", apefile.range_x, apefile.range_y);

    n = (apefile.res + 1)*(apefile.res + 1)*sizeof(float);
    if (size - (p - buffer) != n*apefile.ndata) {
        g_warning("Expected data size %u, but it's %u.", n*apefile.ndata, (guint)(size - (p - buffer)));
        gwy_debug("original ndata %u, corrected ndata %u (n = %u)", apefile.ndata, (guint)(size - (p - buffer))/n, n);
    ...
    for (b = apefile.channels, n = 0, i = 0; b; b = b >> 1, i++) {
        ...
        gwy_container_set_object(container, gwy_app_get_data_key_for_id(n), dfield);
        g_object_unref(apefile.data[n]);

        /*
         * Channel labelling based on SPM Mode
         */
        if (apefile.spm_mode == SPM_MODE_SNOM) {
        ...
        else if (apefile.spm_mode == SPM_MODE_AFM_NONCONTACT
                 || apefile.spm_mode == SPM_MODE_AFM_CONTACT
                 || apefile.spm_mode == SPM_MODE_PHASE_DETECT_AFM) {
            title = gwy_enuml_to_string(i,
                                        "Height",    APE_HEIGHT,
                                        "Height-R",  APE_HEIGHT_R,
                                        "IN1",       APE_NSOM,
        ...
        if (title && *title)
            gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(n), title);

        gwy_container_pass_object(container, gwy_app_get_data_meta_key_for_id(n), gwy_container_duplicate(meta));
                 ...
                 const guchar *buffer)
{
    ...
    for (b = apefile->channels, n = 0, k = 0; b; b = b >> 1, k++) {
        ...
        dfield = gwy_data_field_new(apefile->res, apefile->res, apefile->xreal, apefile->yreal, FALSE);
        ...
        for (i = 0; i < apefile->res; i++) {
            ...
            buffer += sizeof(float);
            gwy_convert_raw_data(buffer, apefile->res, 1, GWY_RAW_DATA_FLOAT, GWY_BYTE_ORDER_LITTLE_ENDIAN,
...
#define HASH_STORE(key, fmt, field) \
    gwy_container_set_string_by_name(meta, key, g_strdup_printf(fmt, apefile->field))
...
apefile_get_metadata(APEFile *apefile)
{
    ...
    gwy_container_set_string_by_name(meta, "Version", g_strdup_printf("%u.%u", apefile->version, apefile->subversion));

    /*
     * In SPM_MODE_SNOM or SPM_MODE_AFM_NONCONTACT freq_osc_tip holds the tip oscillation frequency.
     * If spm_mode=SPM_MODE_AFM_CONTACT then the freq_osc_tip field holds the force the tip applies to the sample in
     * nN.
     * If spm_mode=SPM_MODE_STM then freq_osc_tip holds the bias voltage in Volts.
     */
    if (apefile->spm_mode == SPM_MODE_SNOM || apefile->spm_mode == SPM_MODE_AFM_NONCONTACT)
        HASH_STORE("Tip oscillation frequency", "%g Hz", freq_osc_tip);
    else if (apefile->spm_mode == SPM_MODE_AFM_CONTACT)
        HASH_STORE("Force", "%g nN", freq_osc_tip);
    else if (apefile->spm_mode == SPM_MODE_STM)
    ...
    if (apefile->version > 1 && apefile->subversion >= 4) {
        gwy_container_set_const_string_by_name(meta, "XY HV Status",
                                               gwy_enum_to_string(apefile->xy_hv_status, hva_statuses,
                                                                  G_N_ELEMENTS(hva_statuses)));
        gwy_container_set_const_string_by_name(meta, "Z HV Status",
                                               gwy_enum_to_string(apefile->z_hv_status, hva_statuses,
                                                                  G_N_ELEMENTS(hva_statuses)));
    }
    ...
    if (apefile->remark && *apefile->remark && (p = gwy_convert_to_utf8(apefile->remark, -1, "ISO-8859-1")))
        gwy_container_set_string_by_name(meta, "Comment", p);
    gwy_container_set_const_string_by_name(meta, "SPM mode",
                                           gwy_enum_to_string(apefile->spm_mode, spm_modes, G_N_ELEMENTS(spm_modes)));
...
format_vt_date(gdouble vt_date)
{
    ...
    gwy_debug("Date: %d-%d-%d %d:%d:%d",
              udate.st.wYear, udate.st.wMonth, udate.st.wDay, udate.st.wHour, udate.st.wMinute, udate.st.wSecond);
