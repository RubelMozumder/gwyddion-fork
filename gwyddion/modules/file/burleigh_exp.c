/*
 *  $Id: burleigh_exp.c 26163 2024-02-06 16:56:55Z yeti-dn $
                  ...
                  GError **error)
{
    ...
    dfield = gwy_data_field_new(header.xres, header.yres, header.xscale, header.yscale, FALSE);
    data = gwy_data_field_get_data(dfield);

    if (header.binary) {
        gwy_convert_raw_data(buffer + header.length, n, 1, GWY_RAW_DATA_SINT16, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                             data, 1.0, 0.0);
    }
    else {
    ...
    gwy_si_unit_assign(gwy_data_field_get_si_unit_xy(dfield), header.xyunits);
    gwy_si_unit_assign(gwy_data_field_get_si_unit_z(dfield), header.zunits);
                         ...
                         GError **error)
{
    ...
    while ((line = gwy_str_next_line(&p))) {
        if (sscanf(line, ".Binary Format, Header Length=%u, Integer %u bits", &header->length, &header->bpp))
            header->binary = TRUE;
        if (!p || p[0] != '.')
            break;
    }
    ...
    if (!parse_scale(&p, "Z Scale", &header->zscale, &header->zunits, error))
        return FALSE;
    if (!parse_scale(&p, "Z Res.(value/digital)", &header->zres, &dummy, error))
        return FALSE;
