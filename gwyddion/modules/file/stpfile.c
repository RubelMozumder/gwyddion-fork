/*
 *  $Id: stpfile.c 26344 2024-05-15 13:27:20Z yeti-dn $
             ...
             GError **error)
{
    ...
    if (ok) {
        ...
        for (i = 0; i < stpfile->n; i++) {
            if (err_DIMENSION(error, stpfile->buffers[i].xres) || err_DIMENSION(error, stpfile->buffers[i].yres)) {
                ok = FALSE;
                break;
            }
    ...
    if (ok) {
        ...
        for (i = 0; i < stpfile->n; i++) {
            ...
            dfield = gwy_data_field_new(stpfile->buffers[i].xres, stpfile->buffers[i].yres, 1.0, 1.0, FALSE);
                 ...
                 gchar *buffer)
{
    ...
    stpfile->meta = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
    meta = stpfile->meta;
                 ...
                 GwyContainer *container)
{
    ...
    dfield = gwy_container_get_object(container, gwy_app_get_data_key_for_id(id));
    ...
    switch (mode) {
        ...
        if (stpfile_get_double(stpfile->meta, "z_v_to_angs", &q) && stpfile_get_double(data->meta, "servo_range", &r)) {
    ...
    for (i = 0; i < G_N_ELEMENTS(global_metadata); i++) {
        ...
        gwy_container_set_const_string_by_name(meta, global_metadata[i].meta, str->str);
    }
    ...
    for (i = 0; i < G_N_ELEMENTS(local_metadata); i++) {
        ...
        gwy_container_set_const_string_by_name(meta, local_metadata[i].meta, str->str);
    }

    /* Special */
    if ((p = g_hash_table_lookup(data->meta, "Date")) && (s = g_hash_table_lookup(data->meta, "time")))
    ...
    if ((p = g_hash_table_lookup(data->meta, "collect_mode"))) {
        if (gwy_strequal(p, "1"))
            gwy_container_set_const_string_by_name(meta, "Line direction", "Left to right");
        else if (gwy_strequal(p, "2"))
            gwy_container_set_const_string_by_name(meta, "Line direction", "Right to left");
    }
                ...
                STPData *stpdata)
{
    ...
    gwy_convert_raw_data(stpdata->data, xres*yres, 1,
