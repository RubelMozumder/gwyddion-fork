/*
 *  $Id: nt-mdt.c 26168 2024-02-08 10:18:02Z yeti-dn $
         ...
         GError **error)
{
    ...
    for (i = 0; i <= mdtfile.last_frame; i++) {
        if (mdtfile.frames[i].type == MDT_FRAME_SCANNED) {
            ...
            gwy_container_pass_object_by_name(data, key->str, dfield);
            ...
            gwy_container_pass_object_by_name(data, key->str, meta);
            g_free(sdframe);
            ...
            if (mdaframe->nDimensions == 2 && mdaframe->nMesurands == 1) {
                ...
                gwy_container_pass_object_by_name(data, key->str, gmodel);
                n++;
            }
            ...
            gwy_container_pass_object_by_name(data, key->str, gspectra);
            g_free((gpointer)spframe->title);
            g_free(spframe);
            n++;
        }
...
mdt_get_metadata(MDTFile *mdtfile, guint i)
{
    ...
    || (frame->type == MDT_FRAME_CURVES)) {
        ...
        gwy_container_set_const_string_by_name(meta, "Date", s->str);

        g_string_printf(s, "%d.%d",
                        frame->version/0x100, frame->version % 0x100);
        gwy_container_set_const_string_by_name(meta, "Version", s->str);
        ...
        gwy_container_set_const_string_by_name(meta, "Scan direction", s->str);
        ...
        if ((v = gwy_enuml_to_string(sdframe->channel_index,
                                    ...
                                    NULL)))
            gwy_container_set_const_string_by_name(meta, "ADC Mode", v);
    }
    ...
    for (i = 0; i < G_N_ELEMENTS(metas); i++) {
        ...
        if (end != t) {
            value = gwy_flat_enum_to_string(v, metas[i].len,
                                            ...
                                            metas[i].names);
            if (value && *value)
                gwy_container_set_const_string_by_name(meta, metas[i].name, value);
        }
        g_free(t);
                      ...
                      G_GNUC_UNUSED GError **error)
{
    ...
    for (i = 0; i < bCount; i++) {
        /* FIXME: Why do we need charset conversion here? */
        frame->blocks[i].name = gwy_convert_to_utf8((const gchar*)p, frame->blocks[i].nameLen, "UTF-8");
    ...
    if (indexBlock) {
        ...
        if (g_markup_parse_context_parse(context, indexBlock->data, indexBlock->len, NULL)
            && g_markup_parse_context_end_parse(context, NULL)) {
            /* XXX: There is no need to create a new GMarkupParseContext. It is reusable as long as no error occurs. */
            g_markup_parse_context_free(context);
            context = g_markup_parse_context_new(&parser,
                                                 G_MARKUP_TREAT_CDATA_AS_TEXT,
                                                 frame, NULL);
    ...
    && (indexBlock = findMDTBlock(blockParamsFile, frame))) {
        ...
        context = g_markup_parse_context_new(&parser,
                                             G_MARKUP_TREAT_CDATA_AS_TEXT,
                                             frame, NULL);
        xmlstuff = gwy_utf16_to_utf8(indexBlock->data, indexBlock->len/2,
                                     GWY_BYTE_ORDER_LITTLE_ENDIAN);

        ind = g_strstr_len(xmlstuff, 5, "<"); /* skip BOM */

        if (g_markup_parse_context_parse(context, ind, -1, NULL))
            g_markup_parse_context_end_parse(context, NULL);
        g_markup_parse_context_free(context);
        g_free(xmlstuff);
    }
                      ...
                      GError **error)
{
    ...
    if ((frame_size - (p - fstart)) > 4) {
        ...
        if (frame->title_len
            ...
            frame->title = gwy_convert_to_utf8((const gchar*)p, frame->title_len, "cp1251");
            p += frame->title_len;
    ...
    if ((frame_size - (p - fstart)) > 4) {
        ...
        if (len && (guint)(frame_size - (p - fstart)) >= len) {
            frame->xmlstuff = gwy_utf16_to_utf8((const gunichar2*)p, len/2,
                                                GWY_BYTE_ORDER_LITTLE_ENDIAN);
            p += len;
        }
    }
                           ...
                           GError **error)
{
    ...
    if ((frame_size - (p - fstart)) > 4) {
        ...
        if (len && (guint)(frame_size - (p - fstart)) >= len) {
            /* FIXME: This one is not freed (says valgrind). */
            frame->xmlstuff = gwy_utf16_to_utf8((const gunichar2*)p, len/2,
                                                GWY_BYTE_ORDER_LITTLE_ENDIAN);
            p += len;
        }
             ...
             G_GNUC_UNUSED GError **error)
{
    ...
    if (CommSize && (guint) (frame_size - (p - fstart)) >= CommSize) {
        frame->xmlstuff = gwy_utf16_to_utf8((const gunichar2*)p, CommSize/2,
                                            GWY_BYTE_ORDER_LITTLE_ENDIAN);
        p += CommSize;
    }
    else
...
extract_mda_data(MDTMDAFrame * dataframe)
{
    ...
    if (zAxis->unit && zAxis->unitLen) {
        ...
        gwy_debug("z unit: %s", unit);
        siunitz = gwy_si_unit_new_parse(unit, &power10z);
        g_free(unit);
    }
    else if (zAxis->name && zAxis->nameLen) {
        unit = g_strndup(zAxis->name, zAxis->nameLen);
        gwy_debug("y unit: %s", unit);
        siunitz = gwy_si_unit_new_parse(unit, &power10z);
        g_free(unit);
    }    
    else {
...
extract_mda_spectrum(MDTMDAFrame *dataframe, guint number)
{
    ...
    else if (yAxis->name && yAxis->nameLen) {
        unit = g_strndup(yAxis->name, yAxis->nameLen);
        gwy_debug("y unit: %s", unit);
        siunity = gwy_si_unit_new_parse(unit, &power10y);
        g_free(unit);
    }
    ...
    if (dataframe->nDimensions) {
        if (xAxis->commentLen && xAxis->comment) {
            ...
            context = g_markup_parse_context_new(&parser,
                                                 G_MARKUP_TREAT_CDATA_AS_TEXT,
                                                 &params, NULL);
            if (g_markup_parse_context_parse(context, xAxis->comment, xAxis->commentLen, NULL))
                g_markup_parse_context_end_parse(context, NULL);
            g_markup_parse_context_free(context);
        }
    }
              ...
              const gchar *filename)
{
    ...
    context = g_markup_parse_context_new(&parser, G_MARKUP_TREAT_CDATA_AS_TEXT,
                                         &comment, NULL);
    if (!g_markup_parse_context_parse(context, dataframe->xmlstuff, -1, NULL)
        || !g_markup_parse_context_end_parse(context, NULL)) {
    ...
    gwy_debug("xyoffset = %d", xyoffset);
    gwy_debug("xstep = %d", xstep);
    gwy_debug("ystep = %d", ystep);
    ...
    if (frame_type
        ...
        if (nmes > 1) {
            ...
            for (i = 0; i < nmes - 1; i++) {
                ...
                quark = gwy_app_get_data_key_for_id(*numdata);
                gwy_container_pass_object(container, quark, scandata);
                quark = gwy_app_get_data_title_key_for_id(*numdata);
                gwy_container_set_string(container, quark,
                                         g_strdup_printf("%s (%u)",
                                                         scanname, framenum));
                g_free(scanname);
                quark = gwy_app_get_data_meta_key_for_id(*numdata);
        ...
        quark = gwy_app_get_brick_key_for_id(*numdata);
        gwy_container_set_object(container, quark, brick);
        quark = gwy_app_get_brick_title_key_for_id(*numdata);
        gwy_container_set_const_string(container, quark, title);
        quark = gwy_app_get_brick_meta_key_for_id(*numdata);
        gwy_container_set_object(container, quark, metadata);
        ...
        quark = gwy_app_get_brick_key_for_id(*numdata);
        gwy_container_set_object(container, quark, brick2);
        quark = gwy_app_get_brick_title_key_for_id(*numdata);
