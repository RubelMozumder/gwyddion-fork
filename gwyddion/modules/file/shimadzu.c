/*
 *  $Id: shimadzu.c 26159 2024-02-06 14:01:34Z yeti-dn $
...
static GwyDataField* read_binary_data     (const gchar *buffer,
                                           ...
                                           GHashTable *hash,
                                           GError **error);
static GwyDataField* read_text_data       (gchar *buffer,
                                           GHashTable *hash,
                                           GError **error);
              ...
              GError **error)
{
    ...
    if (ok) {
        ...
        gwy_container_pass_object(container, gwy_app_get_data_meta_key_for_id(0), shimadzu_get_metadata(hash));
                 ...
                 GError **error)
{
    ...
    gwy_convert_raw_data(buffer + HEADER_SIZE, xres*yres, 1,
                         ...
                         gwy_data_field_get_data(dfield), zscale, zoff);
               ...
               GError **error)
{
    ...
    gwy_si_unit_set_from_string_parse(gwy_data_field_get_si_unit_z(dfield), p, &power10);
    zscale = pow10(power10);

    g_strdelimit(buffer, ",;", ' ');
    if (!gwy_parse_doubles(buffer, gwy_data_field_get_data(dfield), 0, &yres, &xres, NULL, &err)) {
