/*
 *  $Id: omicron.c 26157 2024-02-06 12:30:33Z yeti-dn $
             ...
             GError **error)
{
    ...
    /* @text must not be destroyed while @ofile is still in used because all strings are only references there */
    ...
    for (i = 0; i < ofile.topo_channels->len; i++) {
        ...
        gwy_container_pass_object_by_name(container, key, dfield);
     ...
     * Single point which is stored by SCALA as an ascii file.  Any number of single point spectrums may be aquired,
     * but the number is normally quite small. These files are identified by their filename *.cs[0..3]
     *
     * b) Binary Spectroscopy Files
     * When large numbers of spectra are aquired on a regular grid they are stored in BE binary. These data are
     * aquired during the scan, and so can be aquired during the forward scan or the backward scan.
     *
    ...
    if (ofile.spectro_channels) {
        for (i = 0; i < ofile.spectro_channels->len; i++) {
            ...
            if (omicron_has_extension(channel->filename, "cs")) {
            ...
            else if (omicron_has_extension(channel->filename, "sf") || omicron_has_extension(channel->filename, "sb")) {
                gchar *t;
                GQuark quark;
                ...
                g_warning("Cannot determine spectra type of %s", channel->filename);
            }
        }
    }
                    ...
                    GError **error)
{
    ...
    while ((line = gwy_str_next_line(&buffer))) {
        ...
        *val = '\0';
        ...
        comment = strchr(val, ';');
        if (comment) {
            /* If the coment has the form ;[units], move the [units] part after the number. */
            if (comment[1] == '[' && (g_ascii_isalpha(comment[2]) || comment[2] == '%')) {
                gchar *c, *s = comment-1;
    ...
    sanitise_real_size(&ofile->xreal, "x size");
    sanitise_real_size(&ofile->yreal, "y size");
                  ...
                  GError **error)
{
    ...
    *datafilename = filename;
    ...
    scale = (channel->max_phys - channel->min_phys)/(channel->max_raw - channel->min_raw);
    dfield = gwy_data_field_new(ofile->xres, ofile->yres, ofile->xreal, ofile->yreal, FALSE);
    ...
    gwy_si_unit_set_from_string_parse(gwy_data_field_get_si_unit_z(dfield), channel->units, &power10);
    if (power10)
                     ...
                     GError **error)
{
    ...
    while ((line = gwy_str_next_line(&buffer))) {
        ...
        if (strstr(line, "BEGIN COORD")) {
            ...
            while ((line = gwy_str_next_line(&buffer))) {
                ...
                val2 = line+16;
                x = g_ascii_strtod(line, &val2) * Nanometer;
                y = g_ascii_strtod(val2, NULL) * Nanometer;
        ...
        if (strstr(line, "BEGIN") && !strstr(line, "COORD")) {
            ...
            if (siunit) {
                gwy_si_unit_assign(gwy_data_line_get_si_unit_x(dline), siunit);
            ...
            ylabel = channel->name;
            gwy_si_unit_set_from_string_parse(gwy_data_line_get_si_unit_y(dline), channel->units, &power10);
    ...
    for (i = 0; i < ncurves; i++) {
        ...
        gwy_spectra_add_spectrum(spectra, dline, coords[i*2], ofile->yreal - coords[i*2+1]);
                     ...
                     GError **error)
{
    gdouble xreal = channel->end - channel->start,
            q = (channel->max_phys - channel->min_phys)/(channel->max_raw - channel->min_raw),
...
add_meta(gpointer key, gpointer value, gpointer user_data)
{
    ...
    gwy_container_set_const_string_by_name(meta, strkey, strvalue);
}
