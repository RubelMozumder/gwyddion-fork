/*
 *  $Id: ecsfile.c 26341 2024-05-15 09:06:23Z yeti-dn $
         ...
         GError **error)
{
    ...
    g_free(s);
    s = NULL;
    gwy_debug("xreal: %g q: %g unit: %s", xreal, q, c == 0x8f ? "Angstrom" : "Nanometer");
    ...
    if (s) {
        ...
        if (s2) {
            gwy_container_set_string_by_name(meta, "Date", g_strconcat(s, " ", s2, NULL));
            g_free(s2);
            s2 = NULL;
        }
        g_free(s);
