/*
 *  $Id: sdfile.c 26162 2024-02-06 14:58:50Z yeti-dn $
                ...
                gboolean only_name)
{
    ...
    if (only_name)
        return g_str_has_suffix(fileinfo->name_lowercase, MICROMAP_EXTENSION) ? 18 : 0;
                ...
                GError **error)
{
    ...
    if (mfield)
        gwy_container_pass_object(container, gwy_app_get_mask_key_for_id(0), mfield);
              ...
              GError **error)
{
    ...
    objectivemag = g_ascii_strtod(g_hash_table_lookup(sdfile.extras, "OBJECTIVEMAG"), NULL);
    tubemag = g_ascii_strtod(g_hash_table_lookup(sdfile.extras, "TUBEMAG"), NULL);
    cameraxpixel = g_ascii_strtod(g_hash_table_lookup(sdfile.extras, "CAMERAXPIXEL"), NULL);
    cameraypixel = g_ascii_strtod(g_hash_table_lookup(sdfile.extras, "CAMERAYPIXEL"), NULL);

    sdfile_set_units(&sdfile, dfield);
    gwy_data_field_set_xreal(dfield, Micrometer * sdfile.xres * objectivemag * tubemag * cameraxpixel);
    gwy_data_field_set_yreal(dfield, Micrometer * sdfile.yres * objectivemag * tubemag * cameraypixel);
             ...
             GError **error)
{
    ...
    if (err_DIMENSION(error, sdfile->xres) || err_DIMENSION(error, sdfile->yres))
        return FALSE;
                       ...
                       GError **error)
{
    ...
    gwy_debug("version [%.*s]", (gint)sizeof(sdfile->version), sdfile->version);
    gwy_debug("manufacturer [%.*s]", (gint)sizeof(sdfile->manufacturer), sdfile->manufacturer);
    gwy_debug("creation [%.*s]", (gint)sizeof(sdfile->creation), sdfile->creation);
    gwy_debug("modification [%.*s]", (gint)sizeof(sdfile->modification), sdfile->modification);
    ...
    if (sdfile->data_type < SDF_NTYPES)
        sdfile->expected_size = type_sizes[sdfile->data_type] * sdfile->xres * sdfile->yres;
    else
        sdfile->expected_size = -1;
    gwy_debug("expected size %d", sdfile->expected_size);

    /* Olympus software exports `ISO-1.0' files with 8 bytes longer header. It is really the header, not trailer
     * because if we assume it's the trailer the image is shifted.  The values are fixed, 5 and 1.  Try to detect that
    ...
    if (memcmp(sdfile->version, "bISO-1.0", 8) == 0
        ...
        gwy_debug("file is 8 bytes too long; assuming Olympus header with extra fields");
        sdfile->iso_extra1 = gwy_get_guint32_le(p);
        sdfile->iso_extra2 = gwy_get_guint32_le(p);
        gwy_debug("extra1 %d, extra2 %d", sdfile->iso_extra1, sdfile->iso_extra2);
    }
    sdfile->data = (gchar*)*p;
...
/* NB: Buffer must be writable and nul-terminated, its initial part is overwritten */
static gboolean
...
sdfile_read_data_bin(SDFile *sdfile, GwyDataField **mfield)
{
    ...
    dfield = gwy_data_field_new(sdfile->xres, sdfile->yres,
                                sdfile->xres * sdfile->xscale, sdfile->yres * sdfile->yscale,
                                FALSE);
    ...
    /* We assume Intel byteorder, although the format does not specify any order.  But it was developed in PC
     * context... */
    ...
    switch (sdfile->data_type) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (p == end) {
                g_object_unref(dfield);
                g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                            _("End of file reached when reading sample #%d of %d"), i, n);
                return NULL;
            }
            p = end;
        }
        ...
        case SDF_FLOAT:
        case SDF_DOUBLE:
        if (!gwy_parse_doubles(p, data, GWY_PARSE_DOUBLES_FREE_FORM, &sdfile->yres, &sdfile->xres, &end, &err)) {
