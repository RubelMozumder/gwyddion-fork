/*
 *  $Id: rhk-spm32.c 26015 2023-11-10 17:06:42Z yeti-dn $
    ...
    for (i = 0; i < rhkfile->len; i++) {
        ...
        if (rhkpage->type == RHK_TYPE_IMAGE) { // niv - just leaving this alone
            ...
            if ((gmodel = spectra_to_graph(spectra)))
                gwy_container_pass_object(container, gwy_app_get_graph_key_for_id(i+1), gmodel);
        ...
        if (rhkpage->type == RHK_TYPE_IMAGE)
            ...
            /* this doesn't really work, but at least the meta data stays with the graph, even if the metadata viewer
             * won't show it*/
            g_string_printf(key, "%s/%d/meta", GRAPH_PREFIX, i+1);
...
rhkspm32_read_data(RHKPage *rhkpage)
{
    ...
    dfield = gwy_data_field_new(xres, yres, xres*fabs(rhkpage->x.scale), yres*fabs(rhkpage->y.scale), FALSE);

    gwy_si_unit_set_from_string_parse(gwy_data_field_get_si_unit_xy(dfield), rhkpage->x.units, &power10);
    if (power10) {
        q = pow10(power10);
    ...
    gwy_convert_raw_data(rhkpage->buffer + rhkpage->data_offset, xres*yres, 1,
                         rhkpage->rawtype, GWY_BYTE_ORDER_LITTLE_ENDIAN, gwy_data_field_get_data(dfield),
...
rhkspm32_read_spectra(RHKPage *rhkpage)
{
    ...
    for (i = 0; i < rhkpage->yres; i++) {
        ...
        gwy_convert_raw_data(rhkpage->buffer + rhkpage->data_offset + i*rowstride, rhkpage->xres, 1,
                             rhkpage->rawtype, GWY_BYTE_ORDER_LITTLE_ENDIAN, gwy_data_line_get_data(dline),
                             rhkpage->z.scale, rhkpage->z.offset);
        gwy_si_unit_set_from_string(gwy_data_line_get_si_unit_x(dline), rhkpage->x.units);
