/*
 *  $Id: ezdfile.c 25689 2023-09-20 15:55:12Z yeti-dn $
...
static GwyDataField* read_data_field       (EZDSection *section,
                                            GError **error);
             ...
             GError **error)
{
    ...
    if (strncmp(buffer, MAGIC, MAGIC_SIZE) || !(header_size = find_data_start(buffer, size))) {
    ...
    n = find_data_offsets(buffer + header_size, size - header_size, ezdfile, error);
    if (!n) {
        gwy_file_abandon_contents(buffer, size, NULL);
        return NULL;
    }
    ...
    for (n = 0; n < ezdfile->len; n++) {
        ...
        if (!section->data)
            continue;
        if (err_DIMENSION(NULL, section->xres) || err_DIMENSION(NULL, section->yres))
            continue;
        if (!(dfield = read_data_field(section, NULL)))
            continue;
                 ...
                 GError **error)
{
    ...
    while ((line = gwy_str_next_line(&buffer))) {
        ...
        if (line[0] == '[' && line[len-1] == ']') {
            ...
            section->meta = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
                  ...
                  GError **error)
{
    ...
    if (!(p = g_hash_table_lookup(dataset->meta, "GroupCount")) || (ngroups = atol(p)) <= 0) {
    ...
    for (i = 0; i < ngroups; i++) {
        ...
        /* Scan channels inside a group, note it's OK there's less channels than specified */
        for (j = 0; j < nchannels; j++) {
            ...
            gwy_debug("Data %s at offset %u from data start", grkey->str, required_size);
            gwy_debug("xres = %d, yres = %d, bpp = %d, z-name = %s",
                      section->xres, section->yres, section->bitdepth, section->zrange.name);
            if (section->yres < 2) {
                gwy_debug("Skipping 1D data Gr%d-Ch%d. FIXME.", i, j);
                continue;
            }
            if (section->firsterror) {
                gwy_debug("Skipping data Gr%d-Ch%d due to previous errors.", i, j);
                continue;
            }
            ndata++;
            section->data = buffer + required_size;
            required_size += section->xres * section->yres * (section->bitdepth/8);
           ...
           GwyContainer *container)
{
    ...
    dfield = GWY_DATA_FIELD(gwy_container_get_object(container, gwy_app_get_data_key_for_id(idx)));

    /* Fix value scale */
    gwy_si_unit_set_from_string_parse(gwy_data_field_get_si_unit_z(dfield), section->zrange.unit, &power10);
    r = pow10(power10);
    gwy_data_field_multiply(dfield, r*section->zrange.range);
    gwy_data_field_add(dfield, r*section->zrange.min);

    /* Fix lateral scale */
    gwy_si_unit_set_from_string_parse(gwy_data_field_get_si_unit_xy(dfield), section->xrange.unit, &power10);
    ...
    if (section->zrange.name) {
        ...
        gwy_container_set_string(container, gwy_app_get_data_title_key_for_id(idx),
                                 g_strdup_printf("%s%s", section->zrange.name, s));
    }
}
...
static GwyDataField*
read_data_field(EZDSection *section,
                GError **error)
{
    ...
    if (section->bitdepth == 8)
        rawtype = GWY_RAW_DATA_SINT8;
    else if (section->bitdepth == 16)
        rawtype = GWY_RAW_DATA_SINT16;
    else if (section->bitdepth == 32)
        rawtype = GWY_RAW_DATA_SINT32;
    else {
        err_BPP(error, section->bitdepth);
        return NULL;
    }

    g_assert(section->data);
    field = gwy_data_field_new(section->xres, section->yres, 1.0, 1.0, FALSE);
    q = gwy_powi(2.0, section->bitdepth);
    z0 = q/2.0;
    gwy_convert_raw_data(section->data, section->xres*section->yres, 1, rawtype, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                         gwy_data_field_get_data(field), 1.0/q, z0/q);
...
check_section_ranges(gpointer data, G_GNUC_UNUSED gpointer user_data)
{
    ...
    if (section->data) {
        ...
        sanitise_real_size(&section->xrange.range, "x size");
        sanitise_real_size(&section->yrange.range, "y size");
        sanitise_real_size(&section->zrange.range, "z size");
    }
}
