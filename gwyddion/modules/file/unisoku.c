/*
 *  $Id: unisoku.c 26190 2024-02-22 10:47:47Z yeti-dn $
                    ...
                    GError **error)
{
    ...
    if (unisoku_sscanf(line, "ii", &type1, &type2) != 2) {
        /* FIXME */
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    "Missing or invalid some integers heaven knows what they mean but that should be here.");
        return FALSE;
    }
    ufile->dim_x = type1;
    ufile->dim_y = type2;
    ...
    NEXT(buffer, line, error);
    /* The log flags seem to be missing occasionally.  Do not abort when it happens. */
    if (unisoku_sscanf(line, "ddi",
                       &ufile->start_x, &ufile->end_x, &ufile->log_flag_x) < 2) {
    ...
    if (unisoku_sscanf(line, "ddii",
                       &ufile->start_y, &ufile->end_y, &ufile->ineq_flag, &ufile->log_flag_y) < 3) {
    ...
    if (unisoku_sscanf(line, "ddddi",
                       &ufile->max_raw_z, &ufile->min_raw_z, &ufile->max_z, &ufile->min_z, &ufile->log_flag_z) < 4) {
    ...
    if (unisoku_sscanf(line, "dddi",
                       &ufile->stm_voltage, &ufile->stm_current, &ufile->scan_time, &ufile->accum) != 4) {
                        ...
                        GError **error)
{
    ...
    const gchar *unit;
    GwyDataField *dfield;
    gdouble q, pmin, pmax, rmin, rmax, xreal, yreal;

    /* FIXME: what to do when ascii_flag is set? */
    if (ufile->data_type == UNISOKU_UINT8)
        rawtype = GWY_RAW_DATA_UINT8;
    else if (ufile->data_type == UNISOKU_SINT8)
        rawtype = GWY_RAW_DATA_SINT8;
    else if (ufile->data_type == UNISOKU_UINT16)
        rawtype = GWY_RAW_DATA_UINT16;
    else if (ufile->data_type == UNISOKU_SINT16)
        rawtype = GWY_RAW_DATA_SINT16;
    else if (ufile->data_type == UNISOKU_FLOAT)
    ...
    xreal = ufile->end_x - ufile->start_x;
    sanitise_real_size(&xreal, "x size");
    yreal = ufile->end_y - ufile->start_y;
    sanitise_real_size(&yreal, "y size");

    dfield = gwy_data_field_new(ufile->xres, ufile->yres, xreal, yreal, FALSE);
    gwy_convert_raw_data(buffer, n, 1, rawtype, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                         gwy_data_field_get_data(dfield), 1.0, 0.0);

    unit = ufile->unit_x;
    if (!*unit)
        unit = "nm";
    gwy_si_unit_set_from_string_parse(gwy_data_field_get_si_unit_xy(dfield), unit, &power10);
    q = pow10(power10);
    ...
    gwy_si_unit_set_from_string_parse(gwy_data_field_get_si_unit_z(dfield), ufile->unit_z, &power10);
    q = pow10(power10);
    pmin = q*ufile->min_z;
    pmax = q*ufile->max_z;
...
unisoku_get_metadata(UnisokuFile *ufile)
{
    ...
    gwy_container_set_string_by_name(meta, "Date", g_strconcat(ufile->date, " ", ufile->time, NULL));
    if (*ufile->remark)
        gwy_container_set_const_string_by_name(meta, "Remark", ufile->remark);
    if (*ufile->sample_name)
        gwy_container_set_const_string_by_name(meta, "Sample name", ufile->sample_name);
    if (*ufile->ad_name)
        gwy_container_set_const_string_by_name(meta, "AD name", ufile->ad_name);
