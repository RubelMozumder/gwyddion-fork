/*
 *  $Id: keyence.c 25636 2023-09-07 12:19:33Z yeti-dn $
 ...
 * <mime-type type="application/x-keyence-vk3">
 *   <comment>Keyence VK3 profilometry data</comment>
 *   <magic priority="80">
 *     <match type="string" offset="0" value="VK3_"/>
 *   </magic>
 * </mime-type>
 **/
...
/**
 * [FILE-MAGIC-FREEDESKTOP]
 * <mime-type type="application/x-keyence-vk7">
 *   <comment>Keyence VK7 profilometry data</comment>
 *   <magic priority="80">
 *     <match type="string" offset="0" value="VK7">
 *       <match type="string" offset="7" value="BM"/>
 *     </match>
 *   </magic>
 * </mime-type>
 ...
 * # Keyence VK3.
 * 0 string VK3_ Keyence profilometry VK3 data
 **/
 ...
 * # Keyence VK6.
 * 0 string VK6
 * >7 string BM Keyence profilometry VK7 data
 **/
 ...
 * Keyence profilometry VK3, VK4, VK6, VK7
 * .vk3, .vk4, .vk6, .vk7
 * Read
 **/
...
#define MAGIC7 "VK7"
#define MAGIC7_SIZE (sizeof(MAGIC6)-1)
...
#define EXTENSION3 ".vk3"
#define EXTENSION4 ".vk4"
#define EXTENSION6 ".vk6"
#define EXTENSION7 ".vk7"

#define Picometre (1e-12)

/* VK3 and VK4 share many structures; we generally declare VK4 structures and use them also for VK3. */
enum {
...
static gint          keyence3_detect     (const GwyFileDetectInfo *fileinfo,
                                          gboolean only_name);
static gint          keyence4_detect     (const GwyFileDetectInfo *fileinfo,
                                          gboolean only_name);
static GwyContainer* keyence3_load       (const gchar *filename,
                                          GwyRunType mode,
                                          GError **error);
...
static GwyContainer* keyence6_load             (const gchar *filename,
                                                GwyRunType mode,
                                                GError **error);
static gint          keyence7_detect           (const GwyFileDetectInfo *fileinfo,
                                                gboolean only_name);
static GwyContainer* keyence7_load             (const gchar *filename,
                                                GwyRunType mode,
                                                GError **error);
static GwyContainer* keyence67_load_common     (const gchar *filename,
                                                gint version,
                                                GError **error);
...
static GwyModuleInfo module_info = {
    ...
    &module_register,
    N_("Imports Keyence VK3, VK4, VK6 and VK7 files."),
...
{
    gwy_file_func_register("keyence3",
                           N_("Keyence VK3 data files (.vk3)"),
                           (GwyFileDetectFunc)&keyence3_detect,
                           (GwyFileLoadFunc)&keyence3_load,
                           NULL,
                           NULL);
    ...
    gwy_file_func_register("keyence6",
                           ...
                           NULL,
                           NULL);
    gwy_file_func_register("keyence7",
                           N_("Keyence VK7 data files (.vk7)"),
                           (GwyFileDetectFunc)&keyence7_detect,
                           (GwyFileLoadFunc)&keyence7_load,
                           NULL,
                           NULL);
#endif
    return TRUE;
...
static gint
keyence3_detect(const GwyFileDetectInfo *fileinfo, gboolean only_name)
{
    gint score = 0;
    ...
    if (fileinfo->buffer_len > MAGIC4_SIZE + KEYENCE4_HEADER_SIZE
        && memcmp(fileinfo->head, MAGIC3, MAGIC3_SIZE) == 0
...
static gint
keyence7_detect(const GwyFileDetectInfo *fileinfo, gboolean only_name)
{
    if (only_name)
        return g_str_has_suffix(fileinfo->name_lowercase, EXTENSION7) ? 15 : 0;

    if (fileinfo->buffer_len <= KEYENCE6_HEADER_SIZE + BMP_HEADER_SIZE
        || memcmp(fileinfo->head, MAGIC7, MAGIC7_SIZE) != 0
        || memcmp(fileinfo->head + KEYENCE6_HEADER_SIZE, MAGICBMP, MAGICBMP_SIZE) != 0)
        return 0;
...
static GwyContainer*
keyence3_load(const gchar *filename,
              ...
              GError **error)
{
    ...
    data = keyence4_load_membuf(buffer, size, 3, error);
              ...
              GError **error)
{
    ...
    data = keyence4_load_membuf(buffer, size, 4, error);
    gwy_file_abandon_contents(buffer, size, NULL);
    return data;
}
                     ...
                     GError **error)
{
    ...
    gwy_debug("format version %d", version);
    if (!read_header(&p, &size, &kfile->header, version, error)
        || !read_offset_table(&p, &size, &kfile->offset_table, error)
        || !read_meas_conds(&p, &size, &kfile->meas_conds, error)
        || (version == 4 && !read_assembly_info(kfile, error))
            ...
            GError **error)
{
    ...
    if (*size < KEYENCE4_HEADER_SIZE) {
        err_TRUNCATED_PART(error, version == 3 ? "Keyence3Header" : "Keyence4Header");
        return FALSE;
    }
    ...
    if (version == 3) {
        if (memcmp(header->magic, MAGIC3, MAGIC3_SIZE) != 0 || memcmp(header->file_type, MAGIC0, MAGIC0_SIZE) != 0) {
            err_FILE_TYPE(error, "Keyence VK3");
            return FALSE;
        }
    }
    else {
                   ...
                   GError **error)
{
    ...
    if (remsize/nfiles < KEYENCE4_ASSEMBLY_FILE_SIZE) {
        /* Apparently there can be large counts but no actual assembly data. I do not understand but we to not use the
         * infomation for anything anyway. */
               ...
               GError **error)
{
    ...
    /* XXX: We should use the real length even though the format description seems to specify a fixed length.  Also
     * note that only the first data block is supposed to be used; the rest it reserved. */
                  ...
                  gboolean is_height)
{
    ...
    q = (is_height ? measconds->z_length_per_digit * Picometre : gwy_powi(0.5, image->bit_depth));
               ...
               const gchar *title, gint i, const gchar *gradient)
{
    ...
    gwy_container_pass_object(data, gwy_app_get_data_key_for_id(*id), dfield);

    if (i >= 0) {
        gchar *t = g_strdup_printf("%s %u", title, i);
        gwy_container_set_string(data, gwy_app_get_data_title_key_for_id(*id), t);
    }
    else
        gwy_container_set_const_string(data, gwy_app_get_data_title_key_for_id(*id), title);

    if (meta) {
        tmpmeta = gwy_container_duplicate(meta);
        gwy_container_pass_object(data, gwy_app_get_data_meta_key_for_id(*id), tmpmeta);
    }

    if (gradient)
        gwy_container_set_const_string(data, gwy_app_get_data_palette_key_for_id(*id), gradient);
...
keyence6_load(const gchar *filename,
              ...
              GError **error)
{
    return keyence67_load_common(filename, 6, error);
}

static GwyContainer*
keyence7_load(const gchar *filename,
              G_GNUC_UNUSED GwyRunType mode,
              GError **error)
{
    return keyence67_load_common(filename, 7, error);
}

static GwyContainer*
keyence67_load_common(const gchar *filename,
                      gint version,
                      GError **error)
{
    GwyContainer *data = NULL, *meta = NULL;
    ...
    if (version == 7) {
        if (size <= KEYENCE6_HEADER_SIZE + BMP_HEADER_SIZE
            || memcmp(buffer, MAGIC7, MAGIC7_SIZE) != 0
            || memcmp(buffer + KEYENCE6_HEADER_SIZE, MAGICBMP, MAGICBMP_SIZE) != 0) {
            err_FILE_TYPE(error, "Keyence VK7");
            goto fail;
        }
        p = buffer + MAGIC7_SIZE;
    }
    else {
        if (size <= KEYENCE6_HEADER_SIZE + BMP_HEADER_SIZE
    ...
    if (!(zipfile = gwyzip_make_temporary_archive(p, zipsize, "gwyddion-keyence6-XXXXXX.zip", &zipfilename, error))
        || !(gwyzip_locate_file(zipfile, "Vk4File", 0, error))
        || !(vk4buf = gwyzip_get_file_content(zipfile, &vk4size, error)))
        goto fail;

    if (!(data = keyence4_load_membuf(vk4buf, vk4size, 4, error)))
        goto fail;
...
add_vk6_hdr_images(GwyContainer *data, GwyZipFile zipfile)
{
    ...
    if (!(hdrzipfile = gwyzip_make_temporary_archive(buffer, size,
                                                     "gwyddion-keyence6hdr-XXXXXX.zip", &hdrfilename, NULL))) {
        g_free(buffer);
        return;
    }
    g_free(buffer);
    ...
    for (i = 0; ids[i] >= 0; i++) {
        if (errfield && !gwy_data_field_check_compatibility(field, errfield, GWY_DATA_COMPATIBILITY_RES)) {
            ...
            gwy_container_pass_object(data, gwy_app_get_mask_key_for_id(ids[i]), mask);
        }
    }
    g_free(ids);
...
read_vk6_measure_condition(GwyZipFile zipfile)
{
    ...
    if (!(mczipfile = gwyzip_make_temporary_archive(buffer, size,
                                                    "gwyddion-keyence6mc-XXXXXX.zip", &mcfilename, NULL))) {
        g_free(buffer);
        return NULL;
    }
    g_free(buffer);
...
parse_xml_metadata(const gchar *buffer, gsize size)
{
    GMarkupParser parser = {
        &keyence6_start_element, &keyence6_end_element, &keyence6_text, NULL, NULL,
    };
