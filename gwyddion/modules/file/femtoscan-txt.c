/*
 *  $Id: femtoscan-txt.c 25687 2023-09-19 14:44:44Z yeti-dn $
...
/* FIXME: This seems to be a pretty safe bet but the axes may be in principle different than in nm and starting from
 * 0. */
#define MAGIC "\tX,nm\t0\t"
           ...
           GError **error)
{
    ...
    while ((line = gwy_str_next_line(&p))) {
        ...
        if ((n = read_values(data, end)) != xcal->len) {
            g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                        _("Data line length %u does not correspond to the number of X abscissas %u."),
                        n, (guint)xcal->len);
            goto fail;
        }
    }
    ...
    xreal = (g_array_index(xcal, gdouble, xres-1) - g_array_index(xcal, gdouble, 0))*xres/(xres - 1.0)*Nanometre;
    yreal = (g_array_index(ycal, gdouble, yres-1) - g_array_index(ycal, gdouble, 0))*yres/(yres - 1.0)*Nanometre;
    dfield = gwy_data_field_new(xres, yres, xreal, yreal, FALSE);
    gwy_assign(dfield->data, data->data, xres*yres);

    gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_xy(dfield), "m");
    gwy_si_unit_assign(gwy_data_field_get_si_unit_z(dfield), unit);
    if (power10)
