/*
 *  $Id: miffile.c 25689 2023-09-20 15:55:12Z yeti-dn $
                ...
                GError **error)
{
    ...
    /* Version 1.7 is the only actually implemented.  Other can be added upon request and, namely, provision of the
     * files as each file version is different. */
               ...
               const guchar **p)
{
    ...
    if (block->size) {
        gwy_debug("%s offset: %zu (0x%zx), size: %zu (0x%zx)",
                  name, block->offset, block->offset, block->size, block->size);
    }
}
                     ...
                     GError **error)
{
    ...
    for (i = 0; i < INFO_N_IMAGES; i++) {
        ...
        if (items[i].image.size || items[i].image_type) {
            gwy_debug("item #%u: type: %u, offset: %zu, size: %zu",
                      i, items[i].image_type, items[i].image.offset, items[i].image.size);
        }
    }
                    ...
                    GError **error)
{
    if (file_version >= 0x103 && file_version <= 0x107) {
        ...
        sanitise_real_size(&setup->xreal, "x size");
        sanitise_real_size(&setup->yreal, "y size");
                      ...
                      GError **error)
{
    ...
    if (!mif_read_scan_setup(&header->setup, p, size - (*p - buffer), file_version, error))
        return FALSE;
    if (!mif_read_image_configuration(&header->configuration, p, size - (*p - buffer), file_version, error))
        return FALSE;
    ...
    if (file_version == 0x107) {
        ...
        if (!mif_read_axis_info(&header->z_axis, p, size - (*p - buffer), file_version, error))
            return FALSE;

        if (!mif_read_id(&header->id.primary, p, size - (*p - buffer), error)
            || !mif_read_id(&header->id.secondary, p, size - (*p - buffer), error))
            return FALSE;
    }
    else {
        g_return_val_if_reached(FALSE);
         ...
         GError **error)
{
    ...
    if (!mif_read_image_items(mfile.images, buffer, size, &mfile.header.info, error)) {
        gwy_file_abandon_contents(buffer, size, NULL);
        return NULL;
    }
    ...
    for (i = 0; i < mfile.header.nimages; i++) {
        ...
        /* XXX: We cannot use item->image.size because it's bogus, i.e. too short.  Apparently there is some
         * unaccounted-for space until the next block starts, 120 bytes for v1.7 files, after the image header which
         * is in fact still occupied by the image header. MIFBlock says 714 bytes but the true size is
         * 834 = 714 + 120. */
        if (!mif_read_image_header(&image_header, &p, size - (p - buffer), mfile.header.file_version, error))
            continue;
        ...
        gwy_debug("image header true size: %zu", (gsize)(p - (buffer + item->image.offset)));
                    ...
                    GError **error)
{
    ...
    if (!block->size || block->offset > size || block->size > size || block->offset + block->size > size) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
    ...
    if (datasize + yres*sizeof(gint16) <= block->size) {
        ...
        gwy_convert_raw_data(buffer + block->offset + datasize, yres, 1,
                             GWY_RAW_DATA_SINT16, GWY_BYTE_ORDER_LITTLE_ENDIAN, linecorrdata, 1.0, 0.0);
        for (i = 0; i < yres; i++) {
            linecorrdata[i] = gwy_powi(2.0, linecorrdata[i] - 13);
    ...
    // XXX: Don't know why this range.  A user got it from DME.  Have no idea why scan_int_to_meter does not appear
    // here.
    ...
    for (i = 0; i < yres; i++) {
        ...
        gwy_convert_raw_data(buffer + block->offset + i*xres*sizeof(gint16), xres, 1,
                             GWY_RAW_DATA_SINT16, GWY_BYTE_ORDER_LITTLE_ENDIAN, data + (yres-1 - i)*xres, qi, 0.0);
    }
