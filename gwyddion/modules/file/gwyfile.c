/*
 *  $Id: gwyfile.c 25719 2023-09-26 13:09:32Z yeti-dn $
             ...
             GError **error)
{
    ...
    if (!memcmp(buffer, MAGIC, MAGIC_SIZE)) {
        object = gwy_container_deserialize_old(buffer + MAGIC_SIZE, size - MAGIC_SIZE, &pos);
    ...
    if (!GWY_IS_CONTAINER(object)) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Data deserialization succeeded, but resulted in an unexpected object %s."),
             ...
             GError **error)
{
    ...
    gwy_container_gis_string_by_name(data, "/filename", (const guchar**)&filename_orig_utf8);
    filename_orig_utf8 = g_strdup(filename_orig_utf8);

    filename_utf8 = g_filename_to_utf8(filename, -1, NULL, NULL, NULL);
    if (!filename_utf8)
        gwy_container_remove_by_name(data, "/filename");
    else if (filename_orig_utf8 && gwy_strequal(filename_orig_utf8, filename_utf8)) {
    ...
    /* Serialize first.  If we get OOM and hard-abort here, at least keep any existing file intact. */
    ...
    if (!ok && restore_filename) {
        if (filename_orig_utf8)
            gwy_container_set_string_by_name(data, "/filename", filename_orig_utf8);
        else
            gwy_container_remove_by_name(data, "/filename");
        filename_orig_utf8 = NULL;
    }
...
gwyfile_gather_meta(gpointer key, gpointer value, gpointer user_data)
{
    gwyfile_gather_one_meta(GPOINTER_TO_UINT(key),
                            (GValue*)value, (GwyContainer*)user_data, "/meta", sizeof("/meta")-1);
}
...
{
    gwyfile_gather_one_meta(GPOINTER_TO_UINT(key),
                            (GValue*)value, (GwyContainer*)user_data, "/0/data/meta", sizeof("/0/data/meta")-1);
}
 ...
 *
 * Pack scattered metadata under "/meta" to a container at "/0/meta" and metadata scattered under "/0/data/meta" to
 * a container at "/0/meta".
 **/
static void
gwyfile_pack_metadata(GwyContainer *data)
{
    ...
    /* Mindlessly run one packing after another.  Losing some metadata is not a tragedy and the file has to be borken
     * in the first place to have metadata conflicts. */
    gwy_container_foreach(data, "/meta", &gwyfile_gather_meta, meta);
    gwy_container_foreach(data, "/0/data/meta", &gwyfile_gather_0_data_meta, meta);
...
gwyfile_gather_old_rect_selection(GwyContainer *data)
{
    ...
    if (!type
        || !gwy_container_gis_boolean_by_name(data, "/0/select/rect/selected", &selected)
        || !selected)
        return NULL;
...
gwyfile_gather_old_point_selection(GwyContainer *data)
{
    ...
    if (!type || !gwy_container_gis_int32_by_name(data, "/0/select/points/nselected", &nselected))
        return NULL;
...
gwyfile_gather_old_line_selection(GwyContainer *data)
{
    ...
    if (!type || !gwy_container_gis_int32_by_name(data, "/0/select/lines/nselected", &nselected))
        return NULL;
...
gwyfile_remove_old_data(GObject *object)
{
    ...
    if (rect) {
        gwy_container_pass_object_by_name(data, "/0/select/rectangle", rect);
    }
    if (point) {
        gwy_container_pass_object_by_name(data, "/0/select/point", point);
    }
    if (line) {
        gwy_container_pass_object_by_name(data, "/0/select/line", line);
    }
                             ...
                             gsize *position)
{
    ...
    gwy_debug("buf = %p, size = %" G_GSIZE_FORMAT ", pos = %" G_GSIZE_FORMAT, buffer, size, *position);
    g_assert(buffer);
    g_assert(position);
...
#else
    gwy_memcpy_byte_swap(buffer + *position, (guint8*)&value, sizeof(gdouble), 1, sizeof(gdouble) - 1);
#endif
                              ...
                              gsize *position)
{
    ...
    pos = gwy_serialize_check_string(buffer, size, *position, g_type_name(GWY_TYPE_CONTAINER));
    ...
    while (pos < mysize) {
        ...
        switch (type) {
            ...
            if ((object = gwy_serializable_deserialize(buf, mysize, &pos)))
                gwy_container_pass_object(container, key, object);
            break;

            case G_TYPE_BOOLEAN:
            gwy_container_set_boolean(container, key, gwy_serialize_unpack_boolean(buf, mysize, &pos));
            break;

            case G_TYPE_UCHAR:
            gwy_container_set_uchar(container, key, gwy_serialize_unpack_char(buf, mysize, &pos));
            break;

            case G_TYPE_INT:
            gwy_container_set_int32(container, key, gwy_serialize_unpack_int32(buf, mysize, &pos));
            break;

            case G_TYPE_INT64:
            gwy_container_set_int64(container, key, gwy_serialize_unpack_int64(buf, mysize, &pos));
            break;

            case G_TYPE_DOUBLE:
            gwy_container_set_double(container, key, gwy_serialize_unpack_double(buf, mysize, &pos));
            break;

            case G_TYPE_STRING:
            gwy_container_set_string(container, key, gwy_serialize_unpack_string(buf, mysize, &pos));
            break;
