/*
 *  $Id: matfile.c 25888 2023-10-22 05:18:08Z yeti-dn $
    ...
    for (tagno = 0; cursor.size; tagno++) {
        ...
        if (cursor.data_type == MAT5_MATRIX) {
            if ((dfield = try_read_datafield(&cursor, name))) {
                ...
                gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(id), name->str);
                   ...
                   GString *name)
{
    ...
    buf = gwy_data_field_new(yres, xres, yres, xres, FALSE);
    ...
    gwy_convert_raw_data(cursor.zp, xres*yres, 1,
                         ...
                         gwy_data_field_get_data(buf), 1.0, 0.0);
    gwy_data_field_flip_xy(buf, dfield, FALSE);
    g_object_unref(buf);
