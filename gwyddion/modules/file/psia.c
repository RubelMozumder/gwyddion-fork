/*
 *  $Id: psia.c 26163 2024-02-06 16:56:55Z yeti-dn $
...
typedef struct {
    ...
    gboolean square;
    /* Only in version 2+
     * NB: This must be interpreted as the new version can have different data types. */
...
typedef struct {
    ...
    /* Version 2+ only */
    gint spec_point_per_x;   /* If volume_image && !spec_point_per_x, then points are arranged in a square matrix */
...
psia_detect(const GwyFileDetectInfo *fileinfo, gboolean only_name)
{
    ...
    /* Weed out non-TIFFs */
    if (!gwy_tiff_detect(fileinfo->head, fileinfo->buffer_len, &tversion, &byteorder))
        return 0;
...
psia_load_tiff(GwyTIFF *tiff, GError **error)
{
    ...
    if (!entry) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA, _("Data tag is missing."));
        return NULL;
    }
    p = entry->value;
    ...
    dfield = gwy_data_field_new(header.xres, header.yres, header.xreal, header.yreal, FALSE);
    ...
    gwy_container_pass_object_by_name(container, "/0/data", dfield);
    gwy_app_channel_check_nonsquare(container, 0);

    if (header.source_name && *header.source_name)
        gwy_container_set_const_string_by_name(container, "/0/data/title", header.source_name);
    ...
    g_free(comment);
    gwy_container_set_string_by_name(meta, "Version", g_strdup_printf("%08x", version));

    gwy_container_pass_object_by_name(container, "/0/meta", meta);
...
psia_free_image_header(PSIAImageHeader *header)
{
    ...
    g_free(header->xy_servo_mode);
    g_free(header->xy_voltage_mode);
    g_free(header->z_voltage_mode);
    g_free(header->cantilever_name);
}
                     ...
                     gdouble z0)
{
    ...
    gwy_convert_raw_data(p, xres*yres, 1, rawdatatype, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                         gwy_data_field_get_data(dfield), q*z_scale, q*z0);
}
                       ...
                       GError **error)
{
    ...
    if ((version == PSIA_VERSION1 && size < 356) || (version == PSIA_VERSION2 && size < 580)) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Header is too short (only %lu bytes)."), (gulong)size);
        return 0;
    }
    ...
    if (header->image_type != PSIA_2D_MAPPED && header->image_type != PSIA_SPECTROSCOPY) {
        err_NO_DATA(error);
        return 0;
    }
    header->source_name = psia_wchar_to_utf8(&p, 32);
    header->image_mode = psia_wchar_to_utf8(&p, 8);
    gwy_debug("source_name: <%s>, image_mode: <%s>", header->source_name, header->image_mode);
    ...
    if (header->data_type == PSIA_DATA_INT16)
        bps = 2;
    else if (header->data_type == PSIA_DATA_INT32 || header->data_type == PSIA_DATA_FLOAT)
        bps = 4;
    else
                   ...
                   guint len)
{
    ...
    s = gwy_utf16_to_utf8((const gunichar2*)*src, len, GWY_BYTE_ORDER_LITTLE_ENDIAN);
    *src += len*sizeof(gunichar2);
                  ...
                  guint version)
{
    ...
    if (header->source_name && *header->source_name) {
        gwy_container_set_string_by_name(meta, "Source name", header->source_name);
        header->source_name = NULL;
    }
    if (header->image_mode && *header->image_mode) {
        gwy_container_set_string_by_name(meta, "Image mode", header->image_mode);
    ...
    gwy_container_set_string_by_name(meta, "Overscan", g_strdup_printf("%g %%", 100*header->overscan_rate));
    gwy_container_set_const_string_by_name(meta, "Fast direction", header->swap_xy ? "Y" : "X");
    gwy_container_set_string_by_name(meta, "Angle", g_strdup_printf("%g°", header->angle));
    gwy_container_set_const_string_by_name(meta, "Scanning direction",
                                           header->scan_up ? "Bottom to top" : "Top to bottom");
    gwy_container_set_const_string_by_name(meta, "Line direction", header->forward ? "Left to right" : "Right to left");
    gwy_container_set_const_string_by_name(meta, "Sine scan", header->sine_scan ? "Yes" : "No");
    gwy_container_set_string_by_name(meta, "Scan rate", g_strdup_printf("%g Hz", header->scan_rate));
    ...
    if (header->xy_voltage_mode && *header->xy_voltage_mode) {
        gwy_container_set_string_by_name(meta, "XY voltage mode", header->xy_voltage_mode);
        header->xy_voltage_mode = NULL;
    }

    if (header->z_voltage_mode && *header->z_voltage_mode) {
        gwy_container_set_string_by_name(meta, "Z voltage mode", header->z_voltage_mode);
        header->z_voltage_mode = NULL;
    }

    if (header->xy_servo_mode && *header->xy_servo_mode) {
        gwy_container_set_string_by_name(meta, "XY servo mode", header->xy_servo_mode);
        header->xy_servo_mode = NULL;
    }

    if (header->cantilever_name && *header->cantilever_name) {
        gwy_container_set_string_by_name(meta, "Cantilever", header->cantilever_name);
        header->cantilever_name = NULL;
    }

    gwy_container_set_string_by_name(meta, "Z scanner range", g_strdup_printf("%g", header->z_scanner_range));
    gwy_container_set_string_by_name(meta, "Z servo gain", g_strdup_printf("%g", header->z_servo_gain));
    gwy_container_set_string_by_name(meta, "Head tilt angle", g_strdup_printf("%g°", header->head_rotation_angle));
