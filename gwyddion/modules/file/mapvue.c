/*
 *  $Id: mapvue.c 25689 2023-09-20 15:55:12Z yeti-dn $
...
/* This is actually image type (dec 1111 for phase or 2222 for intensity) + the first reference tag (dec 2) */
...
/* XXX: Format documentation seems to mix 851 and 852 together, but the files split them similarly to 52 and 53 */
/* Fields names */
typedef struct {
    gint reftag;
    gchar field1[256];
...
#define MAPVUE_GROUP_READER(x) \
    static guint mapvue_read_group##x(const guchar *p, gsize size, gpointer grpdata, GError **error)
            ...
            GError **error)
{
    ...
    if (size < MAGIC_SIZE || (memcmp(buffer, MAGIC1, MAGIC_SIZE) != 0 && memcmp(buffer, MAGIC2, MAGIC_SIZE) != 0)) {
    ...
    if (mapvuefile.group551.reftag == 551) {
        ...
        if (mapvuefile.group155.reftag == 155
            && calculate_scales_from_fids(&mapvuefile.group155, aspratio, &xreal, &yreal)) {
            xreal *= xres;
            yreal *= yres;
        }
        else {
            g_warning("Cannot figure out dimensions from fiducials, setting y size to 1m.");
            yreal = 1.0;
...
err_TAG_SIZE(GError **error, guint reftag, guint expected, guint real)
{
    ...
    g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                _("Expected tag %u size is %u bytes, but the actual size is %u bytes."),
...
mapvue_group_size(const guchar **p, gsize size, guint reftag, GError **error)
{
    ...
    if (size < 2) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Tag %u size is %u bytes, which is not enough to hold the tag marker."),
                    reftag, (guint)size);
        return 0;
    }
    ...
    if (marker == TAG_MARKER2) {
        if (size < 4) {
            g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                        _("Tag %u size is %u bytes, which is not enough to hold the tag marker and size."),
                        reftag, (guint)size);
            return 0;
        }
        ...
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Tag marker is missing on an unknown tag %u."), reftag);
        return 0;
    }
                     ...
                     GError **error)
{
    ...
    if (size < tagsize + 4) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Tag %u size is %u which is not sufficient to hold its content."),
                    155, (guint)size);
        return 0;
    }
    ...
    if (bpp == 4) {
        ...
        for (i = 0; i < yres; i++) {
            for (j = 0; j < xres; j++) {
                ...
                // FIXME: The scale factor 7.61 was just estimated from image comparison!
                if (v != G_MAXINT16)
                           ...
                           gdouble *xscale, gdouble *yscale)
{
    ...
    if (!h) {
        gwy_debug("Pixel distance of fiducials is zero, that would lead to infinite dimensions.");
        return FALSE;
    }
