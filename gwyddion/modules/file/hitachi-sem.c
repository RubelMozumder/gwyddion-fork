/*
 *  $Id: hitachi-sem.c 25797 2023-10-06 15:38:40Z yeti-dn $
               ...
               gboolean only_name)
{
    ...
    if (only_name)
        return g_str_has_suffix(fileinfo->name_lowercase, HEADER_EXTENSION) ? 10 : 0;
             ...
             GError **error)
{
    ...
    if (!require_keys(hash, error, "ImageName", "DataSize", "PixelSize", NULL))
        goto fail;
    ...
    if (hxres != pxres || hyres != pyres) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Field DataSize %ux%u does not match image dimensions %ux%u."),
    ...
    if ((value = g_hash_table_lookup(hash, "SampleName")) && *value)
        gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(0), value);
    else
        gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(0), "SEM");
           ...
           gpointer user_data)
{
    ...
    if (g_utf8_validate(value, -1, NULL))
        ...
        gchar *s = gwy_convert_to_utf8(value, -1, "CP1252");
        if (s)
