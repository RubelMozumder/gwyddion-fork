/*
 *  $Id: witfile.c 26343 2024-05-15 09:23:24Z yeti-dn $
...
enum {
    ...
    WITEC_SIZE_FOOTER = 38 + 6*2 + 4 + 30*20 + 30*57 + 1170 + 2*WITEC_SIZE_SCALE,
    WITEC_SIZE_RANGE_OPTIONS = 3*8 + 2*8 + 2 + 1 + 4 + 1 + 8 + 1 + 2*2 + 4 + 2*1 + 73,
    WITEC_SIZE_IMAGE_OPTIONS = 2*8 + 3*2 + 2*4 + 4*1 + 2*1 + 2*1 + 1 + 2 + 4*4 + 4*1 + 8 + 65 + 4,
             ...
             gboolean only_name)
{
    ...
    /* The format has no magic header.  So we try to read the short header and check whether we get the right file
     * size if we assume it's a WIT file. */
    if (fileinfo->file_size >= WITEC_SIZE_MIN
        ...
        if (witec_read_header(&p, &len, &header)) {
            expected = (WITEC_SIZE_HEADER
                        + header.channels*(2*header.rows*header.pixels + WITEC_SIZE_SCALE)
                        + WITEC_SIZE_FOOTER
           ...
           GError **error)
{
    ...
    for (i = 0; i < witfile.header.channels; i++) {
        ...
        gwy_container_pass_object(container, gwy_app_get_data_meta_key_for_id(i), witec_get_metadata(&witfile, i));
...
witec_image_to_data_field(WITecFile *witfile,
                          guint i)
{
    const WITecRangeOptions *rangeopts = &witfile->range_options;
    const WITecImageOptions *imageopts = &witfile->image_options;
    gchar unit[9];
    GwyDataField *dfield;
    GwySIUnit *siunit;
    ...
    xres = witfile->header.pixels;
    yres = witfile->header.rows;
    if (xres != imageopts->points_per_line) {
        g_warning("pixels (%d) != points_per_line (%d). Someone has to tell me what it means.",
                  xres, imageopts->points_per_line);
    }
    if (yres != imageopts->lines_per_image) {
        g_warning("rows (%d) != lines_per_image (%d). Someone has to tell me what it means.",
                  yres, imageopts->lines_per_image);
    }
    dfield = gwy_data_field_new(xres, yres,
                                witfile->footer.xscale.scale * imageopts->points_per_line,
                                witfile->footer.yscale.scale * imageopts->lines_per_image,
                                FALSE);
    q = witfile->scales[i].scale;
    z0 = witfile->scales[i].offset;
    gwy_convert_raw_data(witfile->images[i], xres*yres, 1, GWY_RAW_DATA_SINT16, GWY_BYTE_ORDER_LITTLE_ENDIAN,
                         gwy_data_field_get_data(dfield), q, 32768*q + z0);

    if (strncmp(rangeopts->unit_x, rangeopts->unit_y, sizeof(rangeopts->unit_x)) != 0) {
        g_warning("X and Y units differ, using X");
    }
    unit[sizeof(unit)-1] = 0;
    memcpy(unit, rangeopts->unit_x, sizeof(rangeopts->unit_x));
    ...
    memcpy(unit, witfile->scales[i].measure, sizeof(witfile->scales[i].measure));
    siunit = gwy_si_unit_new_parse(unit, &power10);
                   ...
                   G_GNUC_UNUSED gint i)
{
    ...
    if (witfile->footer.title[0])
        gwy_container_set_const_string_by_name(meta, "Title", witfile->footer.title);
    if (witfile->footer.comments[0])
        gwy_container_set_const_string_by_name(meta, "Comments", witfile->footer.comments);
    ...
    if ((v = witfile->range_options.overscan_range))
        gwy_container_set_string_by_name(meta, "Overscan range", g_strdup_printf("%g", v));
    if ((v = witfile->image_options.scan_time[WITEC_FORWARD]))
        gwy_container_set_string_by_name(meta, "Scan time forward", g_strdup_printf("%g s", v));
    if ((v = witfile->image_options.scan_time[WITEC_BACKWARD]))
        gwy_container_set_string_by_name(meta, "Scan time backward", g_strdup_printf("%g s", v));
                 ...
                 WITecScale *scale)
{
    ...
    gwy_debug("scale: %g %g <%s> [%s]", scale->scale, scale->offset, scale->measure, scale->name);
                  ...
                  WITecHeader *header)
{
    ...
    gwy_debug("rows = %d, pixels = %d, channels = %d", header->rows, header->pixels, header->channels);
                  ...
                  WITecFooter *footer)
{
    ...
    gwy_debug("time: %d-%02d-%02d %02d:%02d:%02d",
              footer->year, footer->month, footer->day, footer->hour, footer->minute, footer->second);
    ...
    sanitise_real_size(&footer->xscale.scale, "x scale");
    sanitise_real_size(&footer->yscale.scale, "y scale");
                         ...
                         WITecImageOptions *options)
{
    ...
    gwy_debug("lines_per_image: %d, points_per_line = %d", options->lines_per_image, options->points_per_line);
