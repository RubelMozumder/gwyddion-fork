/*
 *  $Id: jeol.c 26163 2024-02-06 16:56:55Z yeti-dn $
...
/* Magic includes TIFF directory start address and size because they are fixed numbers for JEOL enabling to quicky
 * weed out most of other TIFF files. */
          ...
          GError **error)
{
    ...
    if (err_DIMENSION(error, image_header.xres) || err_DIMENSION(error, image_header.yres))
        goto fail;
    ...
    sanitise_real_size(&image_header.xreal, "x size");
    sanitise_real_size(&image_header.yreal, "y size");

    dfield = jeol_read_data_field(buffer + JEOL_DATA_START, &image_header);
    if (!dfield) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("The type of data is unknown.  Please report it to the developers."));
        goto fail;
    }
    ...
    /* Title */
    s = gwy_convert_to_utf8(image_header.internal_filename, -1, "ISO-8859-1");
    if (s)
        g_strstrip(s);
    if (s && *s)
        gwy_container_set_string(container, gwy_app_get_data_title_key_for_id(0), s);
    else {
        title = gwy_flat_enum_to_string(image_header.data_source, G_N_ELEMENTS(data_sources),
    ...
    /* Meta */
    gwy_container_pass_object(container, gwy_app_get_data_meta_key_for_id(0), jeol_get_metadata(&image_header));
                       ...
                       JEOLImageHeader *header)
{
    ...
    gwy_debug("res: (%d,%d), real: (%g,%g)", header->xres, header->yres, header->xreal, header->yreal);
    ...
    /* XXX: This does not match what the documentation says, there seems to be a four-byte quantity missing between
     * back_scan_bias and mode (the size of SPM params struct is also 4 bytes shorter than the space alloted for it).
     * */
    p += 4;  /* whatever */
                     ...
                     const JEOLImageHeader *header)
{
    ...
    dfield = gwy_data_field_new(header->xres, header->yres, Nanometer*header->xreal, Nanometer*header->yreal, FALSE);
...
jeol_get_metadata(const JEOLImageHeader *header)
{
    ...
    format_meta(meta, "Date and time of measurement",
                ...
                date->year_1980, date->month, date->day, time_->hour, time_->minute, time_->second, time_->second_100);
    ...
    if ((s = gwy_flat_enum_to_string(header->data_source, G_N_ELEMENTS(data_sources), data_sources, data_sources_name)))
    ...
    format_bit(meta, "Measurement signal",
               G_N_ELEMENTS(measurement_signals), measurement_signals, measurement_signals_name,
               header->spm_misc_param.measurement_signal);
    format_bit(meta, "SPM mode", G_N_ELEMENTS(spm_modes), spm_modes, spm_modes_name, header->spm_misc_param.spm_mode);
    format_bit(meta, "AFM mode", G_N_ELEMENTS(afm_modes), afm_modes, afm_modes_name, header->spm_misc_param.afm_mode);
