/*
 *  $Id: nanoscantech.c 26163 2024-02-06 16:56:55Z yeti-dn $
            ...
            if (gwy_strequal(line, "3d")) {
                ...
                if (dfield) {
                    ...
                    if (metadata) {
                        ...
                        gwy_container_pass_object(container, quark, metadata);
                    }
...
{
    return is_utf ? g_strdup(s) : gwy_convert_to_utf8(s, -1, "cp1251");
}
...
exit2:
    if (brick) {
        if (zcrop < zres) {
            brick_cropped = gwy_brick_new_part(brick, 0, 0, 0, xres, yres, zcrop, TRUE);
            g_object_unref(brick);
            brick = brick_cropped;
            zres = zcrop;
        }
        ...
        for (i = 0; i < zres; i++)
            *(data++) = 1e-9*(header->centerwl + header->dispersion * header->pixelxsize * (i - header->centralpixel));
        gwy_si_unit_set_from_string(gwy_data_line_get_si_unit_y(calibration), "m");
        ...
        gwy_si_unit_set_from_string(gwy_brick_get_si_unit_z(brick), "m");
        gwy_si_unit_set_from_string(gwy_brick_get_si_unit_w(brick), "Counts");
    }
    *metadata = meta;
