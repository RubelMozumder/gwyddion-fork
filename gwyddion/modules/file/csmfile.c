/*
 *  $Id: csmfile.c 25689 2023-09-20 15:55:12Z yeti-dn $
...
/* XXX: The file looks like Windows bitmap until you look beyond the nominal file end.  And the type of data inside is
 * not what the BMP header says at all.  And the dimensions in the added footer are wrong.
 * Epic FAIL. */
               ...
               gboolean only_name)
{
    ...
    // Weed out incorrect Windows bitmaps but also those no extra data beyond the nominal end.
    if (fileinfo->file_size <= size)
        return 0;

    // Look for "Version = CSPM" somewhere near the end.
    if (!gwy_memmem(fileinfo->tail, fileinfo->buffer_len, MAGIC, MAGIC_SIZE))
        return 0;
             ...
             GError **error)
{
    ...
    if (!read_bmp_header(buffer, &xres, &yres, &bmpsize) || size <= bmpsize) {
    ...
    /* Do NOT use the fields Image width, Image height from the added footer. Even though it is specifically added by
     * Benyuan it can disagree with the BMP diemsions and when they disagree the BMP diemsions are apparently right.
     * */
    ...
    dfield = gwy_data_field_new(xres, yres, real*Nanometre, real*Nanometre, FALSE);
    ...
    if ((value = g_hash_table_lookup(hash, "sTitle")) && g_utf8_validate(value, -1, NULL))
           ...
           gpointer user_data)
{
    ...
    if (g_utf8_validate(value, -1, NULL)) {
        ...
        gchar *s = gwy_convert_to_utf8(value, -1, "GB2312");
        if (s)
