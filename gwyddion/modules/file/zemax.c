/*
 *  $Id: zemax.c 25670 2023-09-15 08:23:12Z yeti-dn $
           ...
           GError **error)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(fields); i++) {
        fields[i] = gwy_data_field_new(zmxfile.xres, zmxfile.yres,
                                       zmxfile.xres*zmxfile.dx*q, zmxfile.yres*zmxfile.dy*q,
                                       FALSE);
        gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_xy(fields[i]), "m");
    }
    ...
    for (k = 0; k < zmxfile.xres*zmxfile.yres; k++) {
        ...
        for (i = 0; i < G_N_ELEMENTS(fields); i++) {
            ...
            if (end == line) {
                if (i < 4) {
                    g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                                _("Data line %u does not contain four items."), k+1);
                    goto fail;
                }
                fields[i]->data[k] = 0.0;
            }
    ...
    gwy_container_set_string_by_name(meta, "Decenter X", g_strdup_printf("%g m", q*zmxfile.xoff));
    gwy_container_set_string_by_name(meta, "Decenter Y", g_strdup_printf("%g m", q*zmxfile.yoff));

    container = gwy_container_new();
    str = g_string_new(NULL);
    for (i = 0; i < nchannels; i++) {
        gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_z(fields[i]), units[i]);
        gwy_container_set_object(container, gwy_app_get_data_key_for_id(i), fields[i]);
        gwy_container_set_const_string(container, gwy_app_get_data_title_key_for_id(i), titles[i]);
        if (havemask)
            gwy_container_pass_object(container, gwy_app_get_mask_key_for_id(i), gwy_data_field_duplicate(fields[4]));
        gwy_container_pass_object(container, gwy_app_get_data_meta_key_for_id(i), gwy_container_duplicate(meta));
...
/* XXX: This is a weak detection.  We basically accept anything that has seven reasonable numbers on the first line. */
static guint
                  ...
                  GError **error)
{
    ...
    /* Avoid premature memory allocations and stuff since this is also used in the detection path which should be
     * fast.  Find the first line which does not start with '!'. */
    while (header[0] == '!') {
        while (*header && *header != '\n' && *header != '\r')
            header++;
        while (*header == '\n' || *header == '\r')
    ...
    if (end == header)
        goto fail;
    sanitise_real_size(&zmxfile->dx, "x step");
    header = end;
    ...
    if (end == header)
        goto fail;
    sanitise_real_size(&zmxfile->dy, "y step");
    header = end;
    ...
    if (zmxfile->unit >= ZEMAX_NUNITS) {
        g_free(line);
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
                    _("Unit code %d is invalid or unsupported."), zmxfile->unit);
        return 0;
    }
    header = end;
