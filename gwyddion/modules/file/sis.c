/*
 *  $Id: sis.c 26154 2024-02-06 11:38:33Z yeti-dn $
         ...
         GError **error)
{
    ...
    n = 0;
    gwy_clear(&sisfile, 1);
    sisfile.params = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, g_free);
    if (sis_real_load(buffer, size, &sisfile, error)) {
        ...
        for (i = 0; i < sisfile.nchannels; i++) {
            for (j = 0; j < sisfile.channels[i].nimages; j++) {
                ...
                if (dfield) {
                    g_clear_error(&err);
                    gwy_container_pass_object(data, gwy_app_get_data_key_for_id(n), dfield);
                    s = gwy_enuml_to_string(sisfile.channels[i].data_type,
                                            ...
                                            NULL);
                    if (s)
                        gwy_container_set_const_string(data, gwy_app_get_data_title_key_for_id(n), s);
             ...
             GError **error)
{
    ...
    dfield = gwy_data_field_new(image->width, image->height, xreal, yreal, FALSE);

    d = gwy_data_field_get_data(dfield);
    n = image->width * image->height;
    gwy_convert_raw_data(image->image_data, n, 1, rawtypes[image->bpp], GWY_BYTE_ORDER_LITTLE_ENDIAN,
                         d, zreal/(gwy_powi(256.0, image->bpp) - 1.0), 0.0);

    gwy_si_unit_set_from_string(gwy_data_field_get_si_unit_xy(dfield), "m");
    if (channel->data_type == SIS_DATA_TYPE_TOPOGRAPHY)
             ...
             GwyContainer *data)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(good_metadata); i++) {
        ...
        if (sis_parameters[j].type == G_TYPE_STRING)
            value = g_strdup((gchar*)p);
        else if (sis_parameters[j].type == G_TYPE_INT) {
            if (sis_parameters[j].units != -1)
                value = g_strdup_printf("%d %s", *(guint*)p, sis_parameters_units + sis_parameters[j].units);
            else
                value = g_strdup_printf("%d", *(guint*)p);
        }
        else if (sis_parameters[j].type == G_TYPE_DOUBLE) {
            if (sis_parameters[j].units != -1)
                value = g_strdup_printf("%.5g %s", *(gdouble*)p, sis_parameters_units + sis_parameters[j].units);
            else
        ...
        gwy_container_set_string_by_name(meta, sis_parameters_meta + sis_parameters[j].meta, value);
    }
    ...
    gwy_container_pass_object(data, gwy_app_get_data_meta_key_for_id(id), meta);
}

/* FIXME: what a mess. And in reality, the files look different than the specs say anyway... */
static gboolean
              ...
              GError **error)
{
    ...
    if (size - (p - buffer) < docinfosize - 6 || docinfosize < 8) {
        g_set_error(error, GWY_MODULE_FILE_ERROR, GWY_MODULE_FILE_ERROR_DATA,
    ...
    for (i = 0; i < nparams; i++) {
        ...
        if (sis_parameters[sisparam].type == G_TYPE_STRING) {
            g_hash_table_insert(sisfile->params, idp, g_strndup(p, len));
            gwy_debug("Value = %s", (gchar*)g_hash_table_lookup(sisfile->params, idp));
            p += len;
        }
        else if (sis_parameters[sisparam].type == G_TYPE_INT && len == sizeof(gint16)) {
            j = gwy_get_guint16_le(&p);
            g_hash_table_insert(sisfile->params, idp, g_memdup(&j, sizeof(j)));
            gwy_debug("Value = %u", j);
        }
        else if (sis_parameters[sisparam].type == G_TYPE_DOUBLE && len == sizeof(gdouble)) {
            d = gwy_get_gdouble_le(&p);
            g_hash_table_insert(sisfile->params, idp, g_memdup(&d, sizeof(d)));
            gwy_debug("Value = %g", d);
        }
        else {
            g_warning("Invalid parameter with id %u.", id);
            p += len;
        }
    }
    ...
    for (i = 0; i <= sisfile->nchannels; ) {
        ...
        if (i == sisfile->nchannels && channel && channel->nimages == channel->processing_steps) {
            gwy_debug("OK!");
            return TRUE;
        }
        ...
        switch (id) {
            ...
            channel->nimages++;
            channel->images = g_renew(SISImage, channel->images, channel->nimages);
            ...
            for (j = 0; j < G_N_ELEMENTS(processing_steps); j++) {
                if (memcmp(image->processing_step, processing_steps_symbol + processing_steps[j].symbol, 4) == 0) {
                    procstep = j;
            ...
            if (procstep == -1) {
                g_warning("UNKNOWN processing step %.4s", image->processing_step);
            }
            ...
            /* XXX: len is unreliable bogus, some data files have samples instead of bytes here... but we have to
             * figure out whether there is some data or not */
            /*p += len;*/
            if (len == 26) {
                ...
                if (err_DIMENSION(error, image->width) || err_DIMENSION(error, image->height))
                    return FALSE;
