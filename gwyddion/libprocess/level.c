/*
 *  $Id: level.c 26068 2023-12-20 12:25:29Z yeti-dn $
 ...
 * @data_field: A data field
 * @mask: Mask of values to take values into account, or %NULL for full @data_field.  Values equal to 0.0 and below
 *        cause corresponding @data_field samples to be ignored, values equal to 1.0 and above cause inclusion of
 *        corresponding @data_field samples.  The behaviour for values inside (0.0, 1.0) is undefined (it may be
 ...
 *
 * The coefficients can be used for plane leveling using the same relation as in gwy_data_field_fit_plane(), counting
...
gwy_data_field_area_fit_plane(GwyDataField *data_field,
                              ...
                              gdouble *pa, gdouble *pbx, gdouble *pby)
{
    gwy_data_field_area_fit_plane_mask(data_field, mask, GWY_MASK_INCLUDE, col, row, width, height, pa, pbx, pby);
}
...
#pragma omp parallel for if(gwy_threads_are_enabled()) default(none) \
        ...
        det = (n*sumxx*sumyy) + (2*sumx*sumxy*sumy) - (sumx*sumx*sumyy) -(sumy*sumy*sumxx) - (n*sumxy*sumxy);
 ...
 * using gwy_data_field_plane_level() and the entire process is repeated until it converges.  A convergence criterion
 * may be sufficiently small values of the x and y plane coefficients.  Note that since gwy_data_field_plane_level()
 * uses pixel-based lateral coordinates, the coefficients must be divided by gwy_data_field_get_dx(data_field) and
 * gwy_data_field_get_dy(data_field) to obtain physical plane coefficients.
 *
        ...
        for (j = 1; j < xres; j++) {
            if (masking == GWY_MASK_IGNORE
                || (masking == GWY_MASK_INCLUDE
                    && newmrow[j] >= 1.0 && mrow[j] >= 1.0 && newmrow[j-1] >= 1.0 && mrow[j-1] >= 1.0)
                || (masking == GWY_MASK_EXCLUDE
                    && newmrow[j] <= 0.0 && mrow[j] <= 0.0 && newmrow[j-1] <= 0.0 && mrow[j-1] <= 0.0)) {
                n++;
    ...
    for (i = 1; i < yres; i++) {
        ...
        for (j = 1; j < xres; j++) {
            if (masking == GWY_MASK_IGNORE
                ...
                vx = 0.5*(newrow[j] + row[j] - newrow[j-1] - row[j-1])/xr;
                vy = 0.5*(newrow[j-1] + newrow[j] - row[j-1] - row[j])/yr;
                /* XXX: I thought q alone (i.e., normal normalization) would give nice facet leveling, but the higher
                 * norm values has to be suppressed much more -- it seems */
 ...
 *
 * Lines that have no intersection with area selected by @ulcol, @ulrow, @brcol, @brrow are always leveled as a whole.
 * Lines that have intersection with selected area, are leveled using polynomial coefficients computed only from data
 * inside (or outside for @exclude = %TRUE) the area.
 **/
void
                         ...
                         GwyOrientation orientation)
{
    ...
    real = (orientation == GWY_ORIENTATION_HORIZONTAL) ? data_field->xreal : data_field->yreal;
        ...
        for (i = 0; i < yres; i++) {
            ...
            if (i >= row && i < row + height) {
                if (exclude) {
                    ...
                    gwy_assign(ydata->data + col, hlp->data + col + width, xres - col - width);
                    gwy_math_fit_polynom(xres - width, xdata->data, ydata->data, degree, coefs);
                }
                else
                    gwy_data_line_part_fit_polynom(hlp, degree, coefs, col, col + width);
            }
            else
        ...
        for (i = 0; i < xres; i++) {
            ...
            if (i >= col && i < col + width) {
                if (exclude) {
                    ...
                    gwy_assign(ydata->data + row, hlp->data + row + height, yres - row - height);
                    gwy_math_fit_polynom(yres - height, xdata->data, ydata->data, degree, coefs);
                }
                else
                    gwy_data_line_part_fit_polynom(hlp, degree, coefs, row, row + height);
            }
            else
                gwy_data_line_fit_polynom(hlp, degree, coefs);
            gwy_data_line_subtract_polynom(hlp, degree, coefs);
 ...
 * @coeffs: An array of size (@row_degree+1)*(@col_degree+1) to store the coefficients to, or %NULL (a fresh array is
 *          allocated then).
 *
 ...
 * @row_degree: Degree of polynomial to fit row-wise (y-coordinate).
 * @coeffs: An array of size (@row_degree+1)*(@col_degree+1) to store the coefficients to, or %NULL (a fresh array is
 *          allocated then), see gwy_data_field_area_fit_polynom() for details.
 *
                           ...
                           gdouble *coeffs)
{
    ...
    return gwy_data_field_area_fit_polynom(data_field, 0, 0, data_field->xres, data_field->yres,
 ...
 * @coeffs: An array of size (@row_degree+1)*(@col_degree+1) with coefficients, see gwy_data_field_area_fit_polynom()
 *          for details.
 *
 ...
 *
 * The coefficients are organized exactly like in gwy_data_field_area_fit_polynom(), but they are not coefficients of
...
{
    g_return_val_if_fail(GWY_IS_DATA_FIELD(data_field), NULL);
    return gwy_data_field_area_fit_legendre(data_field, 0, 0, data_field->xres, data_field->yres,
                                            col_degree, row_degree, coeffs);
}
 ...
 * @col_degree: Degree of polynomial to subtract column-wise (x-coordinate).
 * @row_degree: Degree of polynomial to subtract row-wise (y-coordinate).
 * @coeffs: An array of size (@row_degree+1)*(@col_degree+1) with coefficients, see gwy_data_field_area_fit_legendre()
 *          for details.
 *
...
{
    g_return_if_fail(GWY_IS_DATA_FIELD(data_field));
    gwy_data_field_area_subtract_legendre(data_field, 0, 0, data_field->xres, data_field->yres,
                                          col_degree, row_degree, coeffs);
}
 ...
 * @coeffs: An array of size (@max_degree+1)*(@max_degree+2)/2 to store the coefficients to, or %NULL (a fresh array
 *          is allocated then).
 *
 * Fits two-dimensional polynomial with limited total degree to a rectangular part of a data field.
 *
 * See gwy_data_field_area_fit_legendre() for description.  This function differs by limiting the total maximum
 * degree, while gwy_data_field_area_fit_legendre() limits the maximum degrees in horizontal and vertical directions
 * independently.
 *
 * Returns: Either @coeffs if it was not %NULL, or a newly allocated array with coefficients.
 **/
...
{
    g_return_val_if_fail(GWY_IS_DATA_FIELD(data_field), NULL);
    return gwy_data_field_area_fit_poly_max(data_field, 0, 0, data_field->xres, data_field->yres, max_degree, coeffs);
}
 ...
 *          gwy_data_field_area_fit_poly_max() for details.
 *
...
{
    g_return_if_fail(GWY_IS_DATA_FIELD(data_field));
    gwy_data_field_area_subtract_poly_max(data_field, 0, 0, data_field->xres, data_field->yres,
                                          max_degree, coeffs);
}
 ...
 * @data_field: A data field.
 * @mask_field: Mask of values to take values into account, or %NULL for full @data_field.  Values equal to 0.0 and
 *              below cause corresponding @data_field samples to be ignored, values equal to 1.0 and above cause
 *              inclusion of corresponding @data_field samples.  The behaviour for values inside (0.0, 1.0) is
 ...
 * @coeffs: Array of size @nterms to store the coefficients to, or %NULL to allocate a new array.
 *
 ...
 * corresponds to the left/topmost pixel and 1 corresponds to the bottom/rightmost pixel of the area.
 *
 * Returns: Either @coeffs if it was not %NULL, or a newly allocated array with coefficients.
 *
 * Since: 2.11
...
{
    g_return_val_if_fail(GWY_IS_DATA_FIELD(data_field), NULL);
    return gwy_data_field_area_fit_poly(data_field, mask_field, 0, 0, data_field->xres, data_field->yres,
                                        nterms, term_powers, exclude, coeffs);
}
...
{
    g_return_if_fail(GWY_IS_DATA_FIELD(data_field));
    gwy_data_field_area_subtract_poly(data_field, 0, 0, data_field->xres, data_field->yres,
 ...
 * @results: An array to store quantities to, may be %NULL to allocate a new one which must be freed by caller then.
 ...
 *
 * Returns: An array of data fields with requested quantities, that is @results unless it was %NULL and a new array
 *          was allocated.
 **/
GwyDataField**
                                     ...
                                     GwyDataField **results)
{
    ...
    for (ri = 0; ri < nresults; ri++) {
        ...
        g_return_val_if_fail(!results || !results[ri] || GWY_IS_DATA_FIELD(results[ri]), NULL);
    }
    if (!results)
    ...
    for (ri = 0; ri < nresults; ri++) {
        if (!results[ri])
            results[ri] = gwy_data_field_new(width, height, xreal, yreal, FALSE);
        else {
            gwy_data_field_resample(results[ri], width, height, GWY_INTERPOLATION_NONE);
...
{
    g_return_val_if_fail(GWY_IS_DATA_FIELD(data_field), NULL);
    return gwy_data_field_area_fit_local_planes(data_field, size, 0, 0, data_field->xres, data_field->yres,
...
gwy_data_field_area_local_plane_quantity(GwyDataField *data_field,
                                         ...
                                         GwyDataField *result)
{
    gwy_data_field_area_fit_local_planes(data_field, size, col, row, width, height, 1, &type, &result);
    return result;
}
