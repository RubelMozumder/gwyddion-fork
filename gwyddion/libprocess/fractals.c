/*
 *  $Id: fractals.c 26282 2024-04-17 13:23:35Z yeti-dn $
 *  Copyright (C) 2004 Jindrich Bilek.
 *  Copyright (C) 2024 David Nečas (Yeti).
                            ...
                            G_GNUC_UNUSED GwyInterpolationType interpolation)
{
    ...
    gwy_data_field_psdf(data_field, yresult, GWY_ORIENTATION_HORIZONTAL,
                        GWY_INTERPOLATION_NONE, GWY_WINDOWING_HANN, -1);
    res = yresult->res;
    gwy_data_line_resample(xresult, res-1, GWY_INTERPOLATION_NONE);

    for (i = 1; i < res; i++) {
        xresult->data[i-1] = log(i);
        yresult->data[i-1] = log(yresult->data[i]);
    }
    gwy_data_line_resize(yresult, 0, res-1);
}
                            ...
                            GwyDataLine *yresult)
{
    ...
    gwy_data_field_hhcf(data_field, yresult, GWY_ORIENTATION_HORIZONTAL, GWY_INTERPOLATION_NONE, -1);
    res = yresult->res;
    outres = MIN(res-1, (res + 5)/10 + GWY_ROUND(sqrt(res)));
    gwy_data_line_resample(xresult, outres, GWY_INTERPOLATION_NONE);

    for (i = 1; i <= outres; i++) {
        xresult->data[i-1] = log(i);
        yresult->data[i-1] = log(yresult->data[i]);
    }
    gwy_data_line_resize(yresult, 0, outres);
}
 ...
 **/
gdouble
gwy_data_field_fractal_hhcf_dim(GwyDataLine *xresult, GwyDataLine *yresult,
                                gdouble *a, gdouble *b)
{
    gwy_data_field_fractal_fit(xresult, yresult, a, b);
