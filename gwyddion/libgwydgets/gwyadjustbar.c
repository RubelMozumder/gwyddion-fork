/*
 *  $Id: gwyadjustbar.c 24993 2022-09-02 14:39:52Z yeti-dn $
...
gwy_adjust_bar_class_init(GwyAdjustBarClass *klass)
{
    ...
    properties[PROP_ADJUSTMENT] = g_param_spec_object("adjustment",
                                                      "Adjustment",
                                                      "Adjustment representing the value.",
                                                      GTK_TYPE_ADJUSTMENT,
                                                      G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
    properties[PROP_SNAP_TO_TICKS] = g_param_spec_boolean("snap-to-ticks",
                                                          "Snap to ticks",
                                                          "Whether only values that are multiples of step "
                                                          "size are allowed.",
                                                          FALSE,
                                                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
    properties[PROP_MAPPING] = g_param_spec_enum("mapping",
                                                 "Mapping",
                                                 ...
                                                 G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
    properties[PROP_HAS_CHECK_BUTTON] = g_param_spec_boolean("has-check-button",
                                                             "Has check button",
     ...
     **/
    signals[SGNL_CHANGE_VALUE] = g_signal_new("change-value",
...
gwy_adjust_bar_init(GwyAdjustBar *adjbar)
{
    ...
    priv = adjbar->priv = G_TYPE_INSTANCE_GET_PRIVATE(adjbar, GWY_TYPE_ADJUST_BAR, AdjustBar);
                      ...
                      GtkCallback callback, gpointer cbdata)
{
    ...
    GTK_CONTAINER_CLASS(gwy_adjust_bar_parent_class)->forall(container, include_internals, callback, cbdata);
}
...
gwy_adjust_bar_map(GtkWidget *widget)
{
    ...
    if (check && GTK_WIDGET_VISIBLE(check) && gtk_widget_get_child_visible(check) && !GTK_WIDGET_MAPPED(check))
...
gwy_adjust_bar_size_request(GtkWidget *widget, GtkRequisition *requisition)
{
    ...
    if (priv->check) {
        ...
        /* Reproduce checkbutton's indicator spacing internally.  It puts two spacings between the check and label.
         * So use that instead of our default border. */
...
gwy_adjust_bar_size_allocate(GtkWidget *widget, GtkAllocation *allocation)
{
    ...
    if (priv->input_window) {
        ...
        gdk_window_move_resize(priv->input_window, allocation->x + priv->x, allocation->y, width, allocation->height);
    }
...
#endif
        cairo_translate(cr, widget->allocation.x + adjbar->priv->x, widget->allocation.y);
        draw_bar(adjbar, cr);
        cairo_destroy(cr);
    }
...
create_input_window(GwyAdjustBar *adjbar)
{
    ...
    gwy_debug("CREATE INPUT WINDOW %dx%d at (%d,%d)", attributes.width, attributes.height, attributes.x, attributes.y);
    priv->input_window = gdk_window_new(gtk_widget_get_parent_window(widget), &attributes, attributes_mask);
 ...
 * contains a label with an overlaid bar that can be clicked, dragged or modified by the scroll-wheel by the user.
 * Since the widget does not take keyboard focus, it should be paired with a #GtkSpinButton, sharing the same
 * adjustment.  This spin button would also be the typical mnemonic widget for the adjustment bar.
 *
