/*
 *  $Id: gwynullstore.c 24900 2022-08-18 16:06:57Z yeti-dn $
                            ...
                            guint ito)
{
    ...
    g_return_if_fail(GWY_IS_NULL_STORE(store));
    g_return_if_fail(ifrom < store->n);
    g_return_if_fail(ito < store->n);
                             ...
                             GtkTreeIter *iter)
{
    ...
    if (!iter || iter->stamp != store->stamp || GPOINTER_TO_UINT(iter->user_data) >= store->n)
        return FALSE;
 ...
 *
 * #GwyNullStore is a very simple class which pretends to be a #GtkTreeModel with one column of type %G_TYPE_UINT
 ...
 * gwy_null_store_set_n_rows().  For convenience, a method to emit "row-changed" signal on a row by its index is
