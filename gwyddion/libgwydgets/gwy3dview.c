/*
 *  $Id: gwy3dview.c 26045 2023-12-01 09:48:31Z yeti-dn $
...
gwy_3d_view_init(Gwy3DView *gwy3dview)
{
    ...
    gwy3dview->priv = G_TYPE_INSTANCE_GET_PRIVATE(gwy3dview, GWY_TYPE_3D_VIEW, Gwy3DViewPrivate);

    gwy3dview->variables = g_hash_table_new_full(g_str_hash, g_str_equal, NULL, g_free);
...
gwy_3d_view_destroy(GtkObject *object)
{
    ...
    if (gwy3dview->ovlays) {
        for (i = 0; i < gwy3dview->novlays; i++) {
            GWY_SIGNAL_HANDLER_DISCONNECT(gwy3dview->ovlays[i], gwy3dview->ovlay_updated_id[i]);
                         ...
                         GParamSpec *pspec)
{
    ...
    switch (prop_id) {
        ...
        case PROP_MASK_KEY:
        g_value_set_static_string(value, g_quark_to_string(GWY_3D_VIEW_GET_PRIVATE(view)->mask_key));
        break;
...
gwy_3d_view_new(GwyContainer *data)
{
    ...
    if (!gtk_widget_set_gl_capability(GTK_WIDGET(gwy3dview), glconfig, NULL, TRUE, GDK_GL_RGBA_TYPE))
...
static void
get_view_data_ranges(Gwy3DView *gwy3dview,
                     gdouble *data_min, gdouble *data_max,
                     gdouble *view_min, gdouble *view_max)
{
    Gwy3DViewPrivate *priv = GWY_3D_VIEW_GET_PRIVATE(gwy3dview);
    GwyDataField *field = gwy3dview->data_field, *mask = priv->mask_field;
    gboolean masking = gwy3dview->setup->hide_masked && mask;
    gint xres = gwy_data_field_get_xres(field), yres = gwy_data_field_get_yres(field);
    gdouble min, max;

    /* Non-masked quantities are cached so do not be afraid getting them repeatedly. */
    ...
    if (view_min || view_max)
        gwy_data_field_area_get_min_max_mask(field, mask, GWY_MASK_EXCLUDE, 0, 0, xres, yres, view_min, view_max);
}
                         ...
                         GwySIValueFormat *vf)
{
    ...
    vf = gwy_si_unit_get_format_with_resolution(unit, GWY_SI_UNIT_FORMAT_VFMARKUP, maximum, step, vf);
    g_snprintf(buffer, sizeof(buffer), "%.*f %s", vf->precision, value/vf->magnitude, vf->units);
    s = g_hash_table_lookup(gwy3dview->variables, key);
    if (!s || !gwy_strequal(s, buffer))
        g_hash_table_insert(gwy3dview->variables, (gpointer)key, g_strdup(buffer));
...
gwy_3d_view_update_labels(Gwy3DView *gwy3dview)
{
    ...
    gdouble xreal, yreal, view_min, view_max, range, maximum;
    ...
    get_view_data_ranges(gwy3dview, NULL, NULL, &view_min, &view_max);
    range = fabs(view_max - view_min);
    maximum = MAX(fabs(view_min), fabs(view_max));

    format = NULL;
    format = gwy_3d_view_update_label(gwy3dview, "x", xreal, xreal, xreal/12.0, FALSE, format);
    format = gwy_3d_view_update_label(gwy3dview, "y", yreal, yreal, yreal/12.0, FALSE, format);
    format = gwy_3d_view_update_label(gwy3dview, "max", view_max, maximum, range/12.0, TRUE, format);
    format = gwy_3d_view_update_label(gwy3dview, "min", view_min, maximum, range/12.0, TRUE, format);
                              ...
                              GCallback callback)
{
    ...
    detailed_signal = g_newa(gchar, sizeof("item-changed::") + strlen(data_key_string));
    g_stpcpy(g_stpcpy(detailed_signal, "item-changed::"), data_key_string);
    *id = connect_swapped_after(gwy3dview->data, detailed_signal, callback, gwy3dview);
}
                         ...
                         const gchar *key)
{
    ...
    gwy_3d_view_container_connect(gwy3dview, g_quark_to_string(quark), &gwy3dview->data_item_id,
...
gwy_3d_view_data_field_connect(Gwy3DView *gwy3dview)
{
    ...
    if (!gwy_container_gis_object(gwy3dview->data, gwy3dview->data_key, &gwy3dview->data_field))
        return;

    g_object_ref(gwy3dview->data_field);
    gwy3dview->data_id = g_signal_connect_swapped(gwy3dview->data_field, "data-changed",
                                                  G_CALLBACK(gwy_3d_view_data_field_changed), gwy3dview);
}
                      ...
                      guint novlays)
{
    ...
    for (i = 0; i < novlays; i++) {
        ...
        gwy3dview->ovlay_updated_id[i] = g_signal_connect_swapped(gwy3dview->ovlays[i], "updated",
                                                                  G_CALLBACK(gwy_3d_view_queue_draw), gwy3dview);
    }
    gwy_3d_view_update_lists(gwy3dview);
}
 ...
 * @gwy3dview: A 3D data view widget.
 * @key: Container string prefix for keys identifying the view setup parameters.  The #Gwy3DSetup is stored at key
 *       <literal>"/setup"</literal> under this prefix.  Label objects are also stored under this prefix.
 *
                             ...
                             const gchar *key)
{
    ...
    gwy_3d_view_container_connect(gwy3dview, g_quark_to_string(quark), &gwy3dview->setup_item_id,
...
gwy_3d_view_ensure_setup(Gwy3DView *gwy3dview)
{
    ...
    if (!gwy_container_gis_object_by_name(gwy3dview->data, key->str, &setup)) {
...
gwy_3d_view_setup_connect(Gwy3DView *gwy3dview)
{
    ...
    g_string_append(key, "setup");
    gwy_container_gis_object_by_name(gwy3dview->data, key->str, &gwy3dview->setup);
    g_assert(gwy3dview->setup);
    g_object_ref(gwy3dview->setup);
    gwy3dview->setup_id = g_signal_connect_swapped(gwy3dview->setup, "notify",
                                                   G_CALLBACK(gwy_3d_view_setup_changed), gwy3dview);
    ...
    for (i = 0; i < GWY_3D_VIEW_NLABELS; i++) {
        ...
        g_string_truncate(key, len);
        g_string_append(key, labels[i].key);
        gwy_container_gis_object_by_name(gwy3dview->data, key->str, &gwy3dview->labels[i]);
        g_assert(gwy3dview->labels[i]);
        g_object_ref(gwy3dview->labels[i]);
        gwy3dview->label_ids[i] = g_signal_connect_swapped(gwy3dview->labels[i], "notify",
                                                           G_CALLBACK(gwy_3d_view_label_changed), gwy3dview);
    }
    g_string_free(key, TRUE);
}
...
gwy_3d_view_setup_disconnect(Gwy3DView *gwy3dview)
{
    ...
    for (i = 0; i < GWY_3D_VIEW_NLABELS; i++) {
        GWY_SIGNAL_HANDLER_DISCONNECT(gwy3dview->labels[i], gwy3dview->label_ids[i]);
                          ...
                          GParamSpec *pspec)
{
    ...
    if (pspec) {
        if (!gwy_3d_visualisation_has_light(setup->visualization)
            && gwy_stramong(pspec->name, "light-theta", "light-phi", NULL))
            return;
        if (!setup->axes_visible && gwy_strequal(pspec->name, "labels-visible"))
            return;
        ...
        if (!setup->axes_visible && gwy_strequal(pspec->name, "line-width"))
            return;
        if (!setup->fmscale_visible
            && gwy_stramong(pspec->name, "fmscale-size", "fmscale-y-align", "fmscale-reserve-space", NULL))
            return;
    }
                             ...
                             const gchar *key)
{
    ...
    if (gwy3dview->ovlays && gwy3dview->ovlays[0])
        gwy_layer_basic_set_gradient_key(GWY_LAYER_BASIC(gwy3dview->ovlays[0]), key);

    GWY_SIGNAL_HANDLER_DISCONNECT(gwy3dview->data, gwy3dview->gradient_item_id);
    gwy_3d_view_gradient_disconnect(gwy3dview);
    gwy3dview->gradient_key = quark;
    gwy_3d_view_gradient_connect(gwy3dview);
    gwy_3d_view_container_connect(gwy3dview, g_quark_to_string(quark), &gwy3dview->gradient_item_id,
...
gwy_3d_view_gradient_connect(Gwy3DView *gwy3dview)
{
    ...
    gwy3dview->gradient_id = g_signal_connect_swapped(gwy3dview->gradient, "data-changed",
                                                      G_CALLBACK(gwy_3d_view_gradient_changed), gwy3dview);
}
                            ...
                            const gchar *key)
{
    ...
    gwy_3d_view_container_connect(gwy3dview, g_quark_to_string(quark), &gwy3dview->material_item_id,
...
gwy_3d_view_material_connect(Gwy3DView *gwy3dview)
{
    ...
    gwy3dview->material_id = g_signal_connect_swapped(gwy3dview->material, "data-changed",
                                                      G_CALLBACK(gwy_3d_view_material_changed), gwy3dview);
}
 ...
 *
 * Data larger than reduced size may be shown downsampled during transforms and other changes to speed up the
 * rendering.  Final, full-size rendering is then performed after a timeout.
 *
 * In case of the original data are not square, the @reduced_size is the greater size of the downsampled data, the
 * other dimension is proportional to the original size.
 *
 * Changes in reduced size do not cause immediate redraw when an operation is pending and the view is shown in reduced
 * size.  It only affects future downsampling.
 *
 * Note since 2.44 the reduced size has no influence because the 3D view does not downsample anything.
 **/
void
...
gwy_3d_view_get_pixbuf(Gwy3DView *gwy3dview)
{
    ...
    gwy_3d_view_expose(GTK_WIDGET(gwy3dview), NULL);
 ...
 * The size of the pixbuf is indentical to the size of the widget.
 *
 * The rendered surface is cut perfectly.  However, if the scene contains also antialiased labels, these are cut in
 * a way which currently only works with a white background.
 *
 * Returns: A newly allocated GdkPixbuf a copy of the framebuffer.
 *
 * Since: 2.54
...
gwy_3d_view_get_pixbuf_with_alpha(Gwy3DView *gwy3dview)
{
    ...
    /* This reads 1 (infinity) in background values and smaller values where the shape or axes are visible.  But it
     * does not distinguish labels and colour bar because we draw them as pixmaps. */
    ...
    for (i = 0; i < height; i++) {
        ...
        for (j = width; j; j--, m++, d++, p += 4) {
            ...
            if (*d < 1.0)
                continue;
            /* Outside.  This we may want to delete, but only if there is nothing drawn. */
            if (p[0] != 0xff || p[1] != 0xff || p[2] != 0xff)
                *m = 1.0;
        }
    }

    /* XXX: We don't know where exactly OpenGL drew the text after transformations. But all non-white pixels and all
     * pixels enclosed by non-white pixels should be preserved.  This retains insides of letters with holes, but it's
     * better than nothing... */
 ...
 * @gwy3dview: A 3D data view widget.
 * @min_scale: Minimum zoom of the 3D data view, pass 0.0 to keep the current value.
 * @max_scale: Maximum zoom of the 3D data view, pass 0.0 to keep the current value.
 *
...
gwy_3d_view_realize(GtkWidget *widget)
{
    ...
    gwy_debug("realizing a Gwy3DView (%ux%u)", widget->allocation.width, widget->allocation.height);
                      ...
                      GdkEventConfigure *event)
{
    if (GTK_WIDGET_CLASS(gwy_3d_view_parent_class)->configure_event)
        GTK_WIDGET_CLASS(gwy_3d_view_parent_class)->configure_event(widget, event);
...
static void
check_zrange_update(Gwy3DView *view)
{
    Gwy3DViewPrivate *priv = GWY_3D_VIEW_GET_PRIVATE(view);
    gdouble min, max;

    if (!view->ovlays || !view->ovlays[0])
        return;

    gwy_layer_basic_get_range(GWY_LAYER_BASIC(view->ovlays[0]), &min, &max);
    if (min != priv->zbot || max != priv->ztop) {
        priv->zbot = min;
        priv->ztop = max;
        priv->need_update_list = TRUE;
    }
                   ...
                   G_GNUC_UNUSED GdkEventExpose *event)
{
    ...
    if (priv->need_update_list) {
        gwy_3d_make_list(gwy3dview, gwy3dview->data_field, priv->mask_field, gwy3dview->ovlays);
    ...
    glViewport(0, 0, w, h);
    if (gwy3dview->setup->fmscale_visible && gwy_3d_visualisation_has_colormap(gwy3dview->setup->visualization)) {
    ...
    glScalef(gwy3dview->setup->scale, gwy3dview->setup->scale, gwy3dview->setup->scale);
    ...
    if (gwy3dview->setup->visualization == GWY_3D_VISUALIZATION_LIGHTING) {
        ...
        gwy_3d_view_rgba_dv(GL_FRONT, GL_AMBIENT, gwy_gl_material_get_ambient(material));
        gwy_3d_view_rgba_dv(GL_FRONT, GL_DIFFUSE, gwy_gl_material_get_diffuse(material));
        gwy_3d_view_rgba_dv(GL_FRONT, GL_SPECULAR, gwy_gl_material_get_specular(material));
        glMaterialf(GL_FRONT, GL_SHININESS, (GLfloat)gwy_gl_material_get_shininess(material)*128.0f);
        glPushMatrix();
                          ...
                          G_GNUC_UNUSED GdkEventMotion *event)
{
    ...
    gwy_debug("motion event: (%d, %d), shape=%d", ex, ey, gwy3dview->shape_current);
    ...
    if (mods & GDK_BUTTON1_MASK) {
        ...
        switch (gwy3dview->movement) {
            ...
            g_object_set(gwy3dview->setup,
                         "rotation-x", gwy3dview->setup->rotation_x + dx*DEG_2_RAD,
                         "rotation-y", gwy3dview->setup->rotation_y + dy*DEG_2_RAD,
                         NULL);
            break;
            ...
            g_object_set(gwy3dview->setup,
                         "light-theta", gwy3dview->setup->light_theta + dy*DEG_2_RAD,
                         "light-phi", gwy3dview->setup->light_phi + dx*DEG_2_RAD,
                         NULL);
            break;
        }
    }
 ...
 *
 * The smaller step of @pdx and @pdy is always 1.0, which mean the GL dimension will be equal to the resolution.  The
 * larger is calculated to keep the physical aspect ratio.
 **/
static void
 ...
 * gwy_3d_make_normals:
 * @dfield: A data field.
 * @normals: Array of normals to fill.  It must have the same number of elements as @dfield.
 *
                    ...
                    Gwy3DVector *normals)
{
   ...
   normals[xres * (yres - 1)].x = (norms[(xres - 1)*(yres -2)].A.x + norms[(xres - 1)*(yres -2)].B.x)/2.0f;
   normals[xres * (yres - 1)].y = (norms[xres - 2].A.y + norms[(xres - 1)*(yres -2)].B.y)/2.0f;
   normals[xres * (yres - 1)].z = (norms[xres - 2].A.z + norms[(xres - 1)*(yres -2)].B.z)/2.0f;
...
#define GWY_3D_VERTEX(i,j) \
  do { \
      ...
      glColor3d((GLfloat) *(color)/255.0, (GLfloat) *(color+1)/255.0, (GLfloat) *(color+2)/255.0); \
                 ...
                 GwyPixmapLayer** ovlays)
{
    ...
    gdouble data_min, data_max, view_min, view_max, recenter, res;
    GLfloat dx, dy;
    Gwy3DVector *normals;
    const gdouble *data;
    ...
    xres = gwy_data_field_get_xres(dfield);
    yres = gwy_data_field_get_yres(dfield);
    get_view_data_ranges(gwy3dview, &data_min, &data_max, &view_min, &view_max);
    ...
    if (gwy3dview->ovlays) {
        ...
        for (l = 0; l < gwy3dview->novlays; l++) {
            if (ovlays[l]) {
                ...
                if (lpb)
                    gdk_pixbuf_composite(lpb, pixbuf, 0, 0, xres, yres, 0, 0,
    ...
    /* The scaling should not change by hide masked and must be given by the actual data range. However, since hiding
     * makes the value range smaller we need to recenter the view. */
    glScalef(2.0/res, 2.0/res, GWY_3D_Z_TRANSFORMATION/(data_max - data_min));
    recenter = 0.5*((data_max - data_min) - (view_max - view_min));
    glTranslatef(0.0, 0.0, -view_min + recenter);
    ...
    for (j = 0; j < yres-1; j++) {
        ...
        for (i = 0; i < xres-1; i++) {
            gdouble z;
            guchar *color;
            if (gwy3dview->setup->hide_masked && mask) {
                if (mask[(yres-1 - j)*xres + i] == 0.0 && mask[(yres-2 - j)*xres + i] == 0.0 && !glon) {
                    glBegin(GL_TRIANGLE_STRIP);
                    glon = TRUE;
                    if (mask[(yres-1 - j)*xres + i - 1] > 0.0 && mask[(yres-2 - j)*xres + i - 1] > 0.0)
                        GWY_3D_VERTEX(i, j);
                ...
                if (mask[(yres-2 - j)*xres + i] == 0 && glon)
                    GWY_3D_VERTEX(i, j+1);
                if (mask[(yres-1 - j)*xres + i] > 0 || mask[(yres-2 - j)*xres + i] > 0) {
                    glEnd();
                    glon = FALSE;
                }
            }
 ...
 */
static void
gwy_3d_util_mult_matrix(const GLdouble *a, const GLdouble *b, GLdouble *mpMatrix)
{
    int i, j;

    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++)
            mpMatrix[i*4+j] = a[i*4+0]*b[0*4+j] + a[i*4+1]*b[1*4+j] + a[i*4+2]*b[2*4+j] + a[i*4+3]*b[3*4+j];
    }
}
...
/* wx,wy,wz denote the pont x,y,z will result in under the model-projection-matrix mpM and the viewport vpMatrix */
static int
...
static void
gwy_3d_draw_axes(Gwy3DView *gwy3dview, gint width, gint height)
{
    GwyDataField *field = gwy3dview->data_field;
    GLfloat dx, dy, rx, Ax, Ay, Bx, By, Cx, Cy, ticklen, xt, yt;
    gdouble data_min, data_max, view_min, view_max, recenter;
    gint xres, yres, res;
    gboolean yfirst;
    ...
    get_view_data_ranges(gwy3dview, &data_min, &data_max, &view_min, &view_max);
    gwy_3d_calculate_pixel_sizes(field, &dx, &dy);
    res = MAX(xres*dx, yres*dy);

    Ax = Ay = Bx = By = Cx = Cy = 0.0f;
    yfirst = TRUE;
    rx = fmod(gwy3dview->setup->rotation_x*RAD_2_DEG, 360.0);
    if (rx < 0.0)
        rx += 360.0;
    ...
    glScalef(2.0/res, 2.0/res, GWY_3D_Z_TRANSFORMATION/(data_max - data_min));
    recenter = 0.5*((data_max - data_min) - (view_max - view_min));  /* See gwy_3d_make_list(). */
    glTranslatef(0.0, 0.0, recenter);
    gwy_3d_view_rgba_dv(GL_FRONT, GL_AMBIENT, gwy_gl_material_get_ambient(mat_none));
    gwy_3d_view_rgba_dv(GL_FRONT, GL_DIFFUSE, gwy_gl_material_get_diffuse(mat_none));
    gwy_3d_view_rgba_dv(GL_FRONT, GL_SPECULAR, gwy_gl_material_get_specular(mat_none));
    glMaterialf(GL_FRONT, GL_SHININESS, (GLfloat)gwy_gl_material_get_shininess(mat_none)*128.0f);

    /* This is optimistic.  Hope something reasonable happens when the selected
     * line width is not actually supported. */
    glLineWidth(gwy3dview->setup->line_width);
    ...
    glVertex2f(Bx, By);
    glVertex2f(Cx, Cy);
    glVertex3f(Cx, Cy, view_max - view_min);
    glEnd();
    ...
    /* Z-axis ticks */
    glVertex3f(Cx, Cy, view_max - view_min);
    glVertex3f(Cx + xt, Cy + yt, view_max - view_min);

    glVertex3f(Cx, Cy, (view_max - view_min)/2);
    glVertex3f(Cx + xt, Cy + yt, (view_max - view_min)/2);
    glEnd();

    /* TODO: create bitmaps with labels in the beginning (possibly in init_gl) into display lists and draw here */
    /* FIXME: I was able to fix the ticks drawing but this is still terribly broken for non-square images.  (Yeti) */
    if (gwy3dview->setup->labels_visible && !ugly_hack_globally_disable_axis_drawing) {
        guint view_size;
        gint size;
        GLdouble mM[16], pM[16], mpM[16];
        GLdouble sx, sy, sz, tx, ty, tz, ux, uy, uz, vx, vy, vz, rotA, rotB;
        GLint vpM[4];
        ...
        gwy_3d_util_project((Ax+Bx)/2 - (Cx-Bx)*0.1, (Ay+By)/2 - (Cy-By)*0.1, -0.0f, mpM, vpM, &sx, &sy, &sz);
        gwy_3d_util_project((Bx+Cx)/2 - (Ax-Bx)*0.1, (By+Cy)/2 - (Ay-By)*0.1, -0.0f, mpM, vpM, &tx, &ty, &tz);
        gwy_3d_util_project(Cx - (Ax-Bx)*0.1, Cy - (Ay-By)*0.1, 0.0f, mpM, vpM, &ux, &uy, &uz);
        gwy_3d_util_project(Cx - (Ax-Bx)*0.1, Cy - (Ay-By)*0.1, view_max - view_min, mpM, vpM, &vx, &vy, &vz);
        ...
        gwy_3d_texture_text(gwy3dview, yfirst ? GWY_3D_VIEW_LABEL_Y : GWY_3D_VIEW_LABEL_X, sx, sy, sz, rotA,
                            size, GWY_3D_JUST_CENTER, GWY_3D_JUST_CENTER);

        gwy_3d_texture_text(gwy3dview, yfirst ? GWY_3D_VIEW_LABEL_X : GWY_3D_VIEW_LABEL_Y, tx, ty, tz, rotB,
                            size, GWY_3D_JUST_CENTER, GWY_3D_JUST_CENTER);

        gwy_3d_texture_text(gwy3dview, GWY_3D_VIEW_LABEL_MAX, vx, vy, vz, 0,
                            size, GWY_3D_JUST_LEFT_EDGE, GWY_3D_JUST_CENTER);

        gwy_3d_texture_text(gwy3dview, GWY_3D_VIEW_LABEL_MIN, ux, uy, uz, 0,
                            size, GWY_3D_JUST_LEFT_EDGE, GWY_3D_JUST_CENTER);
...
static void
gwy_3d_draw_light_position(Gwy3DView *gwy3dview)
{
    GwyGLMaterial *mat_none;
    GLfloat plane_z;
    gdouble data_min, data_max, view_min, view_max;
    int i;
    ...
    glPushMatrix();
    plane_z = 0.5*GWY_3D_Z_TRANSFORMATION*(view_max - view_min)/(data_max - data_min) + GWY_3D_Z_DISPLACEMENT;
                          ...
                          gint *stride)
{
    ...
    surface = cairo_image_surface_create_for_data(alpha, CAIRO_FORMAT_ARGB32, 1, 1, wstride);
    ...
    *height = py;
    ...
    surface = cairo_image_surface_create_for_data(alpha, CAIRO_FORMAT_ARGB32, px, py, wstride);
    cr = cairo_create(surface);
    cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
                  ...
                  gint *width)
{
    ...
    /* The z-axis always goes up.  So bot > top means inverted gradient. */
    if (bot > top) {
        GWY_SWAP(gdouble, bot, top);
        inverted = TRUE;
    }

    max = MAX(fabs(bot), fabs(top));
    gwy_debug("bot %g, top %g, max %g", bot, top, max);
    ...
    format = gwy_si_unit_get_format(siunit, GWY_SI_UNIT_FORMAT_VFMARKUP, max, NULL);
    ...
    mintickdist = label_height*1.5; /* mintickdist is in pixels; tickdist is in meters or whichever basic unit */
    ...
    gwy_3d_format_layout(layout, s, "%.*f %s", prec, top/format->magnitude, format->units);
    pango_layout_get_pixel_size(layout, &layw1, &layh);
    gwy_3d_format_layout(layout, s, "%.*f %s", prec, bot/format->magnitude, format->units);
    ...
    *width = fmw + 2*line_width + l + 2*zoom + tick + 2;
    ...
    if (!noticks) {
        ...
        while (x <= max) {
            ...
            cairo_stroke(cr);
            cairo_move_to(cr, tick+2*zoom, 0);
            gwy_3d_format_layout(layout, s, "%.*f", prec, x/format->magnitude);
            gwy_3d_pango_cairo_show_layout_vcentered(cr, layout);
            cairo_translate(cr, 0, -tickdist*scale);
            x += tickdist;
        }
...
gwy_3d_draw_fmscaletex(Gwy3DView *view)
{
    ...
    /* XXX: This function requires view->ovlays[0] to be present.  Is is always satisfied when we can get here?  */
    ...
    noticks = (gwy_layer_basic_get_range_type(blayer) == GWY_LAYER_BASIC_RANGE_ADAPT);

    dfield = GWY_DATA_FIELD(view->ovlays[0]->data_field);
    zunit = gwy_data_field_get_si_unit_z(dfield);
    surf = gwy_3d_fmscaletex(reduced_height, min, max, zunit,
                             fontsize, sqrt(setup->line_width), view->gradient, noticks,
                             &actual_width);
    ...
    gwy_3d_cairo_to_tex(cairo_image_surface_get_data(surf),
                        actual_width, reduced_height, cairo_image_surface_get_stride(surf));

    he = GWY_ROUND((1.0 - setup->fmscale_yalign) * (height - reduced_height));
    glTranslatef(0, he, 0);

    gwy_3d_draw_ctex(0, GWY_3D_JUST_RIGHT_EDGE, GWY_3D_JUST_BOTTOM_EDGE, actual_width, reduced_height);
    cairo_surface_destroy(surf);
 ...
 *
 * We are only able to get GLContext-unique list id's from glGenLists(), but lists with the same id's seem to
 * interfere across GLContexts.  See bug #53 for more.
 *
...
gwy_3d_view_class_make_list_pool(Gwy3DListPool *pool)
{
    ...
    while (try_size >= 1) {
        ...
        if (!glGetError()) {
            gwy_debug("Allocated a pool with %u items (%u lists)", try_size, GWY_3D_N_LISTS*try_size);
            pool->size = try_size;
            return;
        }
                         ...
                         const gchar *key)
{
    ...
    gwy_3d_view_container_connect(gwy3dview, g_quark_to_string(quark), &priv->mask_item_id,
...
gwy_3d_view_mask_field_connect(Gwy3DView *gwy3dview)
{
    ...
    if (!gwy_container_gis_object(gwy3dview->data, priv->mask_key, &priv->mask_field))
        return;

    g_object_ref(priv->mask_field);
    priv->mask_id = g_signal_connect_swapped(priv->mask_field, "data-changed",
                                             G_CALLBACK(gwy_3d_view_mask_field_changed), gwy3dview);
}
...
#else /* HAVE_GTKGLEXT */
/* Export the same set of symbols if we don't have OpenGL support for correct linking.  But let them all fail
 * run-time. */
static void
 ...
 *
 * If axis drawing is disabled, axes are never drawn.  If it is not disabled, their rendering depends on the 3D view
 * setup.
 *
 * This function is a hack and exists to work around various GL implementations that crash on pixmap drawing
 * operations.
 *
 * Since: 2.14
 **/
 ...
 * a data container with gwy_3d_view_new().  By default, it inherits properties like palette from <link
 * linkend="GwyDataView">data view</link> settings, but supports separate settings -- see
 * gwy_3d_view_set_gradient_key() et al.
 *
 * #Gwy3DView allows the user to interactively rotate, scale, z-scale the data or move lights, depending on its <link
 * linkend="Gwy3DMovement">movement state</link>. There are no controls provided for mode change, you have to provide
 * some yourself and set the movement mode with gwy_3d_view_set_movement_type().
 *
 * You have initialize GtkGLExt with gtk_gl_init_check() and then Gwyddion's OpenGL with gwy_widgets_gl_init() before
