/*
 *  $Id: gwyaxis.c 25853 2023-10-13 17:24:18Z yeti-dn $
...
static void
clear_ticks(GwyAxis *axis)
{
    GwyAxisLabeledTick *mjt;
    guint i;

    for (i = 0; i < axis->mjticks->len; i++) {
        mjt = &g_array_index(axis->mjticks, GwyAxisLabeledTick, i);
        g_string_free(mjt->ttext, TRUE);
    }
    g_array_set_size(axis->mjticks, 0);
    /* We do not have minor tick texts. */
    g_array_set_size(axis->miticks, 0);
}
...
gwy_axis_finalize(GObject *object)
{
    ...
    if (axis->magnification_string)
        g_string_free(axis->magnification_string, TRUE);
    clear_ticks(axis);
...
static gint
gwy_axis_scale(GwyAxis *axis)
{
    gint ret;

    /*never use logarithmic mode for negative numbers*/
    if (axis->reqmin < 0 && axis->is_logarithmic)
    ...
    /*find tick positions*/
    if (axis->is_logarithmic)
        gwy_axis_logscale(axis);
    else
        gwy_axis_normalscale(axis);
...
gwy_axis_set_si_unit(GwyAxis *axis, GwySIUnit *unit)
{
    ...
    gwy_si_unit_assign(axis->unit, unit);
    gwy_axis_adjust(axis, -1, -1);
}
 ...
 * @axis: An axis.
 * @xmin: Left edge of the bounding rectangle.
 * @ymin: Bottom edge of the bounding rectangle.
 ...
 *
 * This is a helper functions for gwy_graph_export_postscript() and probably
 * not useful anywhere else.
 *
 * Returns: A newly created #GString.
 **/
GString*
                       ...
                       gint width, gint height, gint fontsize)
{
    ...
    plotlabel = g_string_new(axis->label_text->str);
    units = gwy_si_unit_get_string(axis->unit, GWY_SI_UNIT_FORMAT_PLAIN);
                         ...
                         guint *nticks)
{
    ...
    for (i = 0; i < axis->mjticks->len; i++) {
        ...
        if (!axis->is_logarithmic)
            ...
            *pvalue = gwy_powi(10.0, pmji->t.value);
    }
