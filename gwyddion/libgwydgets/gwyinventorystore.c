/*
 *  $Id: gwyinventorystore.c 24989 2022-09-02 13:16:29Z yeti-dn $
...
#define GWY_IMPLEMENT_TREE_MODEL(iface_init) \
    { \
        ...
        g_type_add_interface_static(g_define_type_id, GTK_TYPE_TREE_MODEL, &gwy_tree_model_iface_info); \
    }
...
gwy_inventory_store_class_init(GwyInventoryStoreClass *klass)
{
    ...
    g_type_class_add_private(klass, sizeof(GwyInventoryStorePrivate));
...
{
    store->priv = G_TYPE_INSTANCE_GET_PRIVATE(store, GWY_TYPE_INVENTORY_STORE, GwyInventoryStorePrivate);
...
gwy_inventory_store_finalize(GObject *object)
{
    ...
    if (store->inventory) {
        ...
        g_signal_handler_disconnect(store->inventory, store->items_reordered_id);
    }
...
gwy_inventory_store_new(GwyInventory *inventory)
{
    ...
    i = g_signal_connect(inventory, "item-updated", G_CALLBACK(gwy_inventory_store_row_changed), store);
    store->item_updated_id = i;
    i = g_signal_connect(inventory, "item-inserted", G_CALLBACK(gwy_inventory_store_row_inserted), store);
    store->item_inserted_id = i;
    i = g_signal_connect(inventory, "item-deleted", G_CALLBACK(gwy_inventory_store_row_deleted), store);
    store->item_deleted_id = i;
    i = g_signal_connect(inventory, "items-reordered", G_CALLBACK(gwy_inventory_store_rows_reordered), store);
                                  ...
                                  GtkTreeIter *iter)
{
    ...
    copy = *iter;
    if (!gwy_inventory_find(store->inventory, gwy_inventory_store_check_item, &copy))
        return FALSE;
