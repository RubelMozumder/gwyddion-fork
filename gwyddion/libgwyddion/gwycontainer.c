/*
 *  $Id: gwycontainer.c 25635 2023-09-07 12:17:05Z yeti-dn $
...
G_DEFINE_TYPE_EXTENDED(GwyContainer, gwy_container, G_TYPE_OBJECT, 0,
...
gwy_container_class_init(GwyContainerClass *klass)
{
    ...
    * identifier.
    */
    container_signals[ITEM_CHANGED] = g_signal_new("item-changed",
...
{
    container->values = g_hash_table_new_full(NULL, NULL, NULL, value_destroy_func);
}
...
{
    g_return_val_if_fail(GWY_IS_CONTAINER(container), 0);
    return key && g_hash_table_lookup(container->values, GUINT_TO_POINTER(key)) != NULL;
}
 ...
 *
 * Returns: A newly allocated array with quark keys of all @container items, in no particular order.  The number of
 *          items can be obtained with gwy_container_get_n_items().  If there are no items, %NULL is returned.
 *
 * Since: 2.7
 **/
GQuark*
 ...
 *
 * Returns: A newly allocated array with string keys of all @container items, in no particular order.  The number of
 *          items can be obtained with gwy_container_get_n_items().  If there are no items, %NULL is returned.  Unlike
 *          the array the strings are owned by GLib and must not be freed.
 *
 * Since: 2.7
 **/
const gchar**
 ...
 *
 * Returns: A newly allocated array with quark keys, in no particular order. The array is zero-terminated.
 *
 * Since: 2.50
 **/
GQuark*
 ...
 *
 * Returns: A newly allocated array with string keys, in no particular order. Unlike the array the strings are owned
 ...
 *
 * Causes a warning when no such value exists, or it's of a wrong type. Use gwy_container_gis_value_of_type() to get
                                ...
                                GType type)
{
    ...
    if (!p) {
        g_warning("%s: no value for key %u (%s)", GWY_CONTAINER_TYPE_NAME, key, g_quark_to_string(key));
        return NULL;
    }
    if (type && !G_VALUE_HOLDS(p, type)) {
        g_warning("%s: trying to get %s as %s, key %u (%s)",
                  GWY_CONTAINER_TYPE_NAME, G_VALUE_TYPE_NAME(p), g_type_name(type), key, g_quark_to_string(key));
        return NULL;
    }
 ...
 * Get-if-set a generic value from a container.
 *
 * Returns: %TRUE if @v was actually updated, %FALSE when there is no such value in the container.
 **/
gboolean
 ...
 *
 * The returned object doesn't have its reference count increased, use g_object_ref() if you want to access it even
 ...
 *
 * The object eventually stored in @value doesn't have its reference count increased, use g_object_ref() if you want
                       ...
                       GwyKeyVal *values)
{
    ...
    for (i = 0; i < nvalues; i++)
        values[i].changed = gwy_container_try_set_one(container, values[i].key, values[i].value);
}
 ...
 *
 * The container takes ownership of the string, so it can't be used on static strings, use g_strdup() to duplicate
 * them first.
 **/
                         ...
                         const guchar *value)
{
    ...
    /* XXX: We MUST do this.  There is code relying on @value being consumed and kept alive, not just immediately
                        ...
                        GByteArray *buffer)
{
    ...
    buffer = gwy_serialize_object_items(buffer, GWY_CONTAINER_TYPE_NAME, sdata.i, sdata.items);
    g_free(sdata.items);
...
gwy_container_get_size(GObject *object)
{
    ...
    sdata.i = 0;
    g_hash_table_foreach(container->values, hash_serialize_func, &sdata);
    size = gwy_serialize_get_items_size(GWY_CONTAINER_TYPE_NAME, sdata.i, sdata.items);
    g_free(sdata.items);
                          ...
                          gsize *position)
{
    ...
    items = gwy_deserialize_object_hash(buffer, size, position, GWY_CONTAINER_TYPE_NAME, &nitems);
...
gwy_container_duplicate_real(GObject *object)
{
    ...
    g_hash_table_foreach(GWY_CONTAINER(object)->values, hash_duplicate_func, duplicate);
                         ...
                         GObject *copy)
{
    ...
    g_hash_table_foreach_remove(container->values, (GHRFunc)hash_remove_all_func, NULL);
 ...
 *
 * Like gwy_container_duplicate(), this method creates a deep copy, that is contained object are physically duplicated
...
duplicate_by_prefix_do(GwyContainer *container, PrefixListData *pfxlist)
{
    ...
    g_hash_table_foreach(container->values, hash_prefix_duplicate_func, pfxlist);
    duplicate->in_construction = FALSE;
                       ...
                       gboolean force)
{
    ...
    count = 0;
    for (i = 0; i < n; i++) {
        val = (GValue*)g_hash_table_lookup(source->values, GUINT_TO_POINTER(srckeys[i]));
        if (!val) {
            g_critical("Container contents changed during gwy_container_transfer().");
            break;
        }
...
gwy_container_deserialize_from_text(const gchar *text)
{
    ...
    while ((len = token_length(tok))) {
        ...
        /* object */
        else if (typelen+1 == sizeof("object") && g_str_has_prefix(type, "object")) {
            guchar *buf;
            GObject *object;
            gsize i, pos = 0;
            ...
            if (object) {
                ...
                g_warning("cannot deserialize object %.*s", (guint)namelen, name);
        }
        /* UFO */
        else {
 ...
 *
 * #GwyContainer is a general-purpose container, it can hold atomic types, strings and objects. However, objects must
 * implement the #GwySerializable interface, because the container itself is serializable.
 *
