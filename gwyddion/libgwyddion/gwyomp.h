/*
 *  $Id: gwyomp.h 25912 2023-10-23 15:40:51Z yeti-dn $
...
#define gwy_omp_thread_num() \
    (gwy_threads_are_enabled() ? omp_get_thread_num() : 0)
#define gwy_omp_chunk_start(size) \
    (gwy_threads_are_enabled() ? omp_get_thread_num()*(size)/gwy_omp_num_threads() : 0)
#define gwy_omp_chunk_end(size) \
    (gwy_threads_are_enabled() ? (omp_get_thread_num() + 1)*(size)/gwy_omp_num_threads() : (size))
#else
