/*
 *  $Id: gwyexpr.c 25991 2023-11-01 13:08:41Z yeti-dn $
...
struct _GwyExpr {
    ...
    /* String stoage like intern strings/quarks, but private to the expression. Avoids string duplication and
     * especially having to remember when to free them. */
    GHashTable *strings;
    /* Variables */
...
#define make_function_1_1(name) static void gwy_expr_##name(gdouble **s) { **s = name(**s); }
#define make_function_2_1(name) static void gwy_expr_##name(gdouble **s) { --*s; **s = name(*(*s+1), **s); }
 ...
 * @data: An array of arrays of length @n.  The arrays correspond to expression variables in gwy_expr_execute().
 *        The zeroth array can be %NULL.
                                 ...
                                 gdouble *result)
{
     ...
     * shared(call_table), whereas some forbid it. */
#ifdef _OPENMP
...
gwy_expr_stack_check_executability(GwyExpr *expr)
{
    ...
    nval = max = 0;
    for (i = 0; i < expr->in; i++) {
        if (expr->input[i].type == GWY_EXPR_CODE_CONSTANT || (gint)expr->input[i].type < 0) {
            nval++;
        }
...
gwy_expr_stack_print(GwyExpr *expr)
{
    ...
    for (i = 0; i < expr->in; i++) {
        ...
        if ((gint)code->type > 0) {
            ...
            g_print("#%u Argument %s\n", i, (gchar*)g_ptr_array_index(expr->identifiers, -(gint)code->type));
        }
        else
 ...
 *
 * This is done by selective execution, that is we go through the stack like in gwy_expr_stack_interpret(), but only
 * execute function calls on constant while copying operands and operations we cannot carry out right away.
 *
 ...
 *
 * Assumes each token is either unconverted scanner token, or it has non-empty RPN block.  In either case it frees the
 * RPN block, or strings of uncoverted scanner tokens.
 **/
static void
                           ...
                           GwyExprToken *tokens)
{
    ...
    for (t = tokens; t; t = t->next) {
        if (t->rpn_block)
            expr->reservoir = gwy_expr_token_list_concat(t->rpn_block, expr->reservoir);
    }
    expr->reservoir = gwy_expr_token_list_concat(tokens, expr->reservoir);
}

/* NB: The result is gchar because it is technically modifiable and tends to appear in gchar* variables, but the
 * caller should not really modify it. */
static gchar*
gwy_expr_intern_string(GwyExpr *expr, gchar *s)
{
    gchar *s_intern;

    if ((s_intern = g_hash_table_lookup(expr->strings, s)))
        return s_intern;

    s_intern = g_strdup(s);
    g_hash_table_insert(expr->strings, s_intern, s_intern);
                     ...
                     GError **err)
{
    ...
    while ((token = g_scanner_get_next_token(scanner))) {
        switch ((gint)token) {
            ...
            if (token == G_TOKEN_IDENTIFIER)
                t->value.v_identifier = gwy_expr_intern_string(expr, t->value.v_identifier);
            break;

            default:
            g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_INVALID_TOKEN, _("Invalid token"));
    ...
    if (!tokens) {
        g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_EMPTY, _("Empty expression"));
        return FALSE;
    }
    ...
    for (t = expr->tokens; t; ) {
        ...
        switch (t->token) {
            ...
            if (!prev || (prev->token != G_TOKEN_FLOAT
                ...
                prev = t;
                t = t->next;
                expr->tokens = gwy_expr_token_list_delete_token(expr, expr->tokens, prev);
            }
            else
                t = t->next;
            break;
            ...
            if (prev && (prev->token == G_TOKEN_FLOAT
                ...
                prev->token = '*';
                expr->tokens = gwy_expr_token_list_insert(expr->tokens, t, prev);
            }
            t = t->next;
            break;
...
gwy_expr_initialize_scanner(GwyExpr *expr)
{
    ...
    for (i = 1; i < G_N_ELEMENTS(call_table); i++) {
        if (!call_table[i].name || !g_ascii_isalpha(call_table[i].name[0]))
            continue;
        g_scanner_scope_add_symbol(expr->scanner, GWY_EXPR_SCOPE_GLOBAL, call_table[i].name, GUINT_TO_POINTER(i));
    }
 ...
 *
 * %G_TOKEN_IDENTIFIER strings are freed, they are converted to single-item RPN lists too, with type as minus index in
 * returned array of name.
 *
    ...
    for (t = expr->tokens; t; t = t->next) {
        ...
        if (expr->constants) {
            if ((quark = g_quark_try_string(t->value.v_identifier))
                && (cval = g_hash_table_lookup(expr->constants, GUINT_TO_POINTER(quark)))) {
                code = gwy_expr_token_new0(expr);
                code->token = GWY_EXPR_CODE_CONSTANT;
        ...
        for (i = 1; i < expr->identifiers->len; i++) {
            if (strcmp(t->value.v_identifier, g_ptr_array_index(expr->identifiers, i)) == 0)
                break;
        }
 ...
 * @expr: An expression.
 * @tokens: A token list.
 * @right_to_left: %TRUE to process operators right to left, %FALSE to left to right.
                             ...
                             GError **err)
{
    ...
    for (t = right_to_left ? gwy_expr_token_list_last(tokens) : tokens;
        ...
        prev->rpn_block = gwy_expr_token_list_concat(next->rpn_block, prev->rpn_block);
                             ...
                             GError **err)
{
    ...
    for (t = gwy_expr_token_list_last(tokens); t; t = t->prev) {
        ...
        for (i = 0; i < nargs; i++) {
            ...
            t->rpn_block = gwy_expr_token_list_concat(arg->rpn_block, t->rpn_block);
...
gwy_expr_transform_to_rpn_real(GwyExpr *expr,
                               ...
                               GError **err)
{
    GwyExprOpCode pow_operators[] = { GWY_EXPR_CODE_POWER, };
    GwyExprOpCode mult_operators[] = { GWY_EXPR_CODE_MULTIPLY, GWY_EXPR_CODE_DIVIDE, GWY_EXPR_CODE_MODULO, };
    GwyExprOpCode add_operators[] = { GWY_EXPR_CODE_ADD, GWY_EXPR_CODE_SUBTRACT, };
    ...
    if (tokens->token != G_TOKEN_LEFT_PAREN) {
        g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_OPENING_PARENTHESIS, _("Missing opening parenthesis"));
        goto FAIL;
    }
    ...
    if (!t) {
        g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_CLOSING_PARENTHESIS, _("Missing closing parenthesis"));
        goto FAIL;
    }
    if (!tokens) {
        g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_EMPTY_PARENTHESES, _("Empty parentheses"));
        goto FAIL;
    }
    ...
    for (t = tokens; t; t = t->next) {
        if (t->token == G_TOKEN_COMMA) {
            if (!t->next || !t->prev) {
                g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_STRAY_COMMA, _("Stray comma"));
                goto FAIL;
            }
            t = t->prev;
            tokens = gwy_expr_token_list_delete_token(expr, tokens, t->next);
    ...
    /* 2. Power operator */
    if (!(tokens = gwy_expr_transform_infix_ops(expr, tokens, TRUE, "^", pow_operators, err))) {
        g_assert(!err || *err);
        goto FAIL;
    }

    /* 3. Multiplicative operators */
    if (!(tokens = gwy_expr_transform_infix_ops(expr, tokens, FALSE, "*/%", mult_operators, err))) {
        g_assert(!err || *err);
        goto FAIL;
    }

    /* 4. Additive operators */
    if (!(tokens = gwy_expr_transform_infix_ops(expr, tokens, FALSE, "+-", add_operators, err))) {
        g_assert(!err || *err);
        goto FAIL;
    }
    ...
    for (t = tokens; t; t = t->next) {
        if (!t->rpn_block) {
            g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_GARBAGE, _("Stray symbol %d"), t->token);
            goto FAIL;
        }
    }
 ...
 *
 * @expr->tokens is destroyed by the conversion and set to %NULL, @expr->input is filled with opcodes, stack is
 * checked for executability and eventually resized.
 *
                          ...
                          GError **err)
{
    ...
    if (expr->tokens->next) {
        g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_GARBAGE, _("Trailing garbage"));
        gwy_expr_token_list_delete(expr, expr->tokens);
        expr->tokens = NULL;
        return FALSE;
    }
    ...
    if (!gwy_expr_stack_check_executability(expr)) {
        g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_NOT_EXECUTABLE, _("Stack is not executable"));
        return FALSE;
    }
...
gwy_expr_new(void)
{
    ...
    /* We could also put constants into scanner's symbol table, but then we have to tell them apart when we get some
     * symbol from scanner. */
    expr->constants = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, g_free);
    expr->strings = g_hash_table_new_full(g_str_hash, g_str_equal, NULL, g_free);
...
gwy_expr_free(GwyExpr *expr)
{
    ...
    g_hash_table_destroy(expr->strings);
 ...
 *
 * Returns: The last string passed to gwy_expr_evaluate() or gwy_expr_compile().  It is owned by @expr and must not be
 *          modified or freed.
 **/
const gchar*
 ...
 * @err: Location to store compilation or evaluation error to, or %NULL. Errors from #GwyExprError domain can occur.
 *
                  ...
                  GError **err)
{
    ...
    if (expr->identifiers->len > 1) {
        g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_UNRESOLVED_IDENTIFIERS, _("Unresolved identifiers"));
        return FALSE;
    }
                 ...
                 GError **err)
{
    ...
    if (!gwy_expr_parse(expr, err) || !gwy_expr_transform_values(expr) || !gwy_expr_transform_to_rpn(expr, err)) {
        g_assert(!err || *err);
        expr->in = 0;
        return FALSE;
    }
 ...
 * @expr: An expression evaluator.
 * @names: Location to store pointer to array of variable names to (may be %NULL to get just number of variables).
 *         The string array returned in this argument in owned by @expr and is valid only until next
 *         gwy_expr_compile(), gwy_expr_evaluate(), or gwy_expr_free() call.
 *
 ...
 *
 * The position of each variable in @names corresponds to the position of its value in @values array in
 * gwy_expr_execute() call.  Namely, the first item in the array is always reserved and do not correspond to any
 * variable.
 *
 * Returns: The length of array stored to @names.  This is the number of variables plus one (for the first reserved
 ...
 * @indices: Array to store variable positions to.  The positions are the same as in gwy_expr_execute().  Variables
 *           not present in the expression are assigned the reserved position 0.  This permits assigning safely values
 *           to all variables before execution wthout caring which variables are actually present.
 *
                           ...
                           guint *indices)
{
    ...
    for (i = 0; i < n; i++) {
        for (j = 1; j < expr->identifiers->len; j++) {
            if (gwy_strequal(names[i], (gchar*)g_ptr_array_index(expr->identifiers, j))) {
                indices[i] = j;
                requested[j] = TRUE;
                break;
            }
 ...
 * gwy_expr_execute:
 * @expr: An expression evaluator.
 * @values: Array with variable values.  Its zeroth item is always unused. Variable list can be obtained by
 *          gwy_expr_get_variables().
 *
 ...
 *
 * The result is the same as repeatedly taking one element from each array and using gwy_expr_execute() to evaluate
 ...
 *
 * Valid identifier must start with a letter, continue with alphanumeric characters (or underscores).
 *
 ...
 * @err: Location to store error to, or %NULL. Only the %GWY_EXPR_ERROR_CONSTANT_NAME error from #GwyExprError domain
 *       can occur.
 *
 * Defines a symbolic constant.
 *
 * Note the definition does not affect already compiled expression, you have to recompile it (and eventually
                         ...
                         GError **err)
{
    ...
    if (!gwy_expr_constant_name_is_valid(name)) {
        g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_CONSTANT_NAME, "Invalid constant name");
        return FALSE;
    }

    gwy_expr_initialize_scanner(expr);
    if (g_scanner_lookup_symbol(expr->scanner, name)) {
        g_set_error(err, GWY_EXPR_ERROR, GWY_EXPR_ERROR_CONSTANT_NAME, "Constant name clashes with function");
        return FALSE;
    }

    quark = g_quark_from_string(name);
    g_hash_table_insert(expr->constants, GUINT_TO_POINTER(quark), g_memdup(&value, sizeof(gdouble)));
 ...
 *
 * Note the definition removal does not affect already compiled expression, you have to recompile it (and eventually
 * re-resolve variables).
 *
 ...
 * with gwy_expr_new(), then it can be used to evaluate any number of expressions; when it's no longer needed, it
 ...
 *
 * Expression with variables have to be compiled first with gwy_expr_compile(). Then either
 * gwy_expr_resolve_variables() or gwy_expr_get_variables() can be used to obtain information what variables are
 ...
 * performed by gwy_expr_execute(). It is also possible to evaluate expression on each item of value arrays with
 * gwy_expr_vector_execute().
 *
 ...
 * from some known set, the gwy_expr_resolve_variables() should be used (if the repeated evaluation happens over items
 * of some arrays, gwy_expr_vector_execute() is more efficient and usually simplier to use too):
 * <informalexample><programlisting>
 * GwyExpr *expr;
 * GError *err = NULL;
 ...
 * the list of variables with gwy_expr_get_variables() and supply only values of variables that are actually present
 * in the expression.
 **/
 ...
 * GWY_EXPR_ERROR:
 *
 * Error domain for expression parsing and evaluation. Errors in this domain will be from the #GwyExprError
