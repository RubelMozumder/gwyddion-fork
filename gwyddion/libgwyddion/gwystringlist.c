/*
 *  $Id: gwystringlist.c 25969 2023-10-27 14:47:05Z yeti-dn $
                          ...
                          GByteArray *buffer)
{
    ...
    strings = (GPtrArray*)strlist->strings;
    {
        ...
        retval = gwy_serialize_pack_object_struct(buffer, GWY_STRING_LIST_TYPE_NAME, n, spec);
        return retval;
    }
}
...
gwy_string_list_get_size(GObject *obj)
{
    ...
    strings = (GPtrArray*)strlist->strings;
    {
        ...
        size = gwy_serialize_get_struct_size(GWY_STRING_LIST_TYPE_NAME, n, spec);
    }
                            ...
                            gsize *position)
{
    ...
    if (!gwy_serialize_unpack_object_struct(buffer, size, position, GWY_STRING_LIST_TYPE_NAME,
...
gwy_string_list_duplicate_real(GObject *object)
{
    ...
    for (i = 0; i < strings->len; i++)
        g_ptr_array_index(dupstrings, i) = g_strdup(g_ptr_array_index(strings, i));
...
gwy_string_list_clone_real(GObject *source, GObject *copy)
{
    ...
    for (i = 0; i < strings->len; i++)
        g_ptr_array_index(copystrings, i) = g_strdup(g_ptr_array_index(strings, i));
