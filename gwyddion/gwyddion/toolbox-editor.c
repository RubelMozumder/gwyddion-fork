/*
 *  $Id: toolbox-editor.c 25436 2023-06-14 15:32:55Z yeti-dn $
...
gwy_toolbox_editor(void)
{
    ...
    editor.dialogue = gtk_dialog_new_with_buttons(_("Toolbox Editor"), toolbox, GTK_DIALOG_MODAL,
    ...
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(editor.dialogue)->vbox), GTK_WIDGET(hbox), TRUE, TRUE, 0);
    ...
    g_signal_connect_swapped(editor.toolbox_model, "row-changed", G_CALLBACK(update_toolbox_sensitivity), &editor);
    g_signal_connect_swapped(editor.toolbox_model, "row-deleted", G_CALLBACK(update_toolbox_sensitivity), &editor);
    g_signal_connect_swapped(editor.toolbox_model, "row-inserted", G_CALLBACK(update_toolbox_sensitivity), &editor);
    g_signal_connect_swapped(editor.toolbox_model, "rows-reordered", G_CALLBACK(update_toolbox_sensitivity), &editor);
    ...
    if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(editor.toolbox_model), &iter))
        gtk_tree_selection_select_iter(selection, &iter);
    g_signal_connect_swapped(selection, "changed", G_CALLBACK(update_toolbox_sensitivity), &editor);
    ...
    g_signal_connect_swapped(editor.add_item, "clicked", G_CALLBACK(add_toolbox_item), &editor);

    editor.add_group = gwy_stock_like_button_new(_("_New Group"), GTK_STOCK_NEW);
    gtk_box_pack_start(vbox, editor.add_group, FALSE, FALSE, 0);
    g_signal_connect_swapped(editor.add_group, "clicked", G_CALLBACK(add_toolbox_group), &editor);

    editor.edit = gwy_stock_like_button_new(_("_Edit"), GTK_STOCK_EDIT);
    gtk_box_pack_start(vbox, editor.edit, FALSE, FALSE, 0);
    g_signal_connect_swapped(editor.edit, "clicked", G_CALLBACK(edit_item_or_group), &editor);

    editor.remove = gwy_stock_like_button_new(_("_Remove"), GTK_STOCK_REMOVE);
    gtk_box_pack_start(vbox, editor.remove, FALSE, FALSE, 0);
    g_signal_connect_swapped(editor.remove, "clicked", G_CALLBACK(remove_from_toolbox), &editor);

    editor.move_up = gwy_stock_like_button_new(_("Move _Up"), GTK_STOCK_GO_UP);
    gtk_box_pack_start(vbox, editor.move_up, FALSE, FALSE, 0);
    g_signal_connect_swapped(editor.move_up, "clicked", G_CALLBACK(move_up_in_toolbox), &editor);

    editor.move_down = gwy_stock_like_button_new(_("Move _Down"), GTK_STOCK_GO_DOWN);
    gtk_box_pack_start(vbox, editor.move_down, FALSE, FALSE, 0);
    g_signal_connect_swapped(editor.move_down, "clicked", G_CALLBACK(move_down_in_toolbox), &editor);
    ...
    editor.width = GTK_ADJUSTMENT(gtk_adjustment_new(editor.spec->width, 1, 20, 1, 2, 0));
    g_signal_connect_swapped(editor.width, "value-changed", G_CALLBACK(width_changed), &editor);
...
fill_toolbox_treestore(GtkTreeStore *store, GwyToolboxSpec *spec)
{
    ...
    for (i = 0; i < group->len; i++) {
        ...
        for (j = 0; j < item->len; j++) {
            ...
            gtk_tree_store_insert_with_values(store, &iiter, &giter, j, 0, row, -1);
        }
    }
}
...
{
    gtk_tree_model_foreach(GTK_TREE_MODEL(store), &free_toolbox_treestore_row, NULL);
...
create_function_list(ForeachFunc foreach_do, GwyAppActionType type)
{
    ...
    g_array_sort_with_data(func_names, compare_func_names, GUINT_TO_POINTER(type));
...
create_toolbox_tree_view(GwyToolboxEditor *editor)
{
    ...
    gtk_tree_view_column_set_cell_data_func(column, renderer, toolbox_cell_renderer_icon, NULL, NULL);

    renderer = gtk_cell_renderer_text_new();
    gtk_tree_view_column_pack_start(column, renderer, FALSE);
    gtk_cell_renderer_set_fixed_size(renderer, -1, height);
    gtk_tree_view_column_set_cell_data_func(column, renderer, toolbox_cell_renderer_icon_flags, NULL, NULL);
    ...
    gtk_tree_view_column_pack_start(column, renderer, FALSE);
    gtk_cell_renderer_set_fixed_size(renderer, -1, height);
    gtk_tree_view_column_set_cell_data_func(column, renderer, toolbox_cell_renderer_type, NULL, NULL);
    ...
    gtk_tree_view_column_pack_start(column, renderer, FALSE);
    gtk_cell_renderer_set_fixed_size(renderer, -1, height);
    gtk_tree_view_column_set_cell_data_func(column, renderer, toolbox_cell_renderer_name, NULL, NULL);
    ...
    gtk_tree_view_column_pack_start(column, renderer, FALSE);
    gtk_cell_renderer_set_fixed_size(renderer, -1, height);
    gtk_tree_view_column_set_cell_data_func(column, renderer, toolbox_cell_renderer_mode, NULL, NULL);
...
create_function_tree_view(GwyToolboxItemEditor *ieditor)
{
    ...
    gtk_tree_view_column_pack_start(column, renderer, FALSE);
    gtk_cell_renderer_set_fixed_size(renderer, width, height);
    gtk_tree_view_column_set_cell_data_func(column, renderer, function_cell_renderer_icon, ieditor, NULL);

    renderer = gtk_cell_renderer_text_new();
    gtk_tree_view_column_pack_start(column, renderer, FALSE);
    gtk_cell_renderer_set_fixed_size(renderer, -1, height);
    gtk_tree_view_column_set_cell_data_func(column, renderer, function_cell_renderer_name, ieditor, NULL);
}
                           ...
                           G_GNUC_UNUSED gpointer userdata)
{
    ...
    pixbuf = gtk_icon_set_render_icon(iconset, widget->style,
                                      GTK_TEXT_DIR_LTR, GTK_STATE_NORMAL, GTK_ICON_SIZE_LARGE_TOOLBAR,
                                      widget, NULL);
                           ...
                           G_GNUC_UNUSED gpointer userdata)
{
    ...
    g_object_set(renderer, "text", text ? gwy_sgettext(text) : NULL, NULL);
}
                           ...
                           G_GNUC_UNUSED gpointer userdata)
{
    ...
    if (row->type == GWY_APP_ACTION_TYPE_GROUP
        ...
        g_object_set(renderer, "text", gwy_sgettext(gwy_toolbox_mode_name(row->mode)), NULL);
    }
}
                            ...
                            gpointer user_data)
{
    ...
    type = ieditor->ispec.type;
    gtk_tree_model_get(model, iter, 0, &qname, -1);
    g_object_set(renderer, "text", gwy_toolbox_action_nice_name(type, g_quark_to_string(qname)), NULL);
}
                    ...
                    GwyToolboxGroupSpec *gspec, guint i)
{
    ...
    geditor.dialogue = gtk_dialog_new_with_buttons(_("Toolbox Group"), GTK_WINDOW(editor->dialogue), GTK_DIALOG_MODAL,
    ...
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(geditor.dialogue)->vbox), GTK_WIDGET(table), TRUE, TRUE, 0);
    row = 0;
    ...
    gtk_table_attach(table, geditor.title, 1, 2, row, row+1, GTK_EXPAND | GTK_FILL, 0, 0, 0);
    g_signal_connect_swapped(geditor.title, "changed", G_CALLBACK(group_title_changed), &geditor);
    row++;
    ...
    gtk_table_attach(table, geditor.id, 1, 2, row, row+1, GTK_EXPAND | GTK_FILL, 0, 0, 0);
    g_signal_connect_swapped(geditor.id, "changed", G_CALLBACK(group_id_changed), &geditor);

    button = gtk_button_new_with_mnemonic(_("_Suggest"));
    gtk_table_attach(table, button, 2, 3, row, row+1, GTK_FILL, 0, 0, 0);
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(suggest_group_id), &geditor);
    row++;

    button = gtk_check_button_new_with_mnemonic(_("Trans_latable title"));
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button), geditor.gspec.translatable);
    gtk_table_attach(table, button, 0, 3, row, row+1, GTK_FILL, 0, 0, 0);
    g_signal_connect_swapped(button, "toggled", G_CALLBACK(group_title_transl_changed), &geditor);
    row++;

    gtk_table_set_row_spacing(table, row-1, 8);
    geditor.message = gtk_label_new(NULL);
    gtk_table_attach(table, geditor.message, 0, 3, row, row+1, GTK_FILL, 0, 0, 0);
    row++;
    ...
    if (response == GTK_RESPONSE_OK) {
        /* XXX: The caller must ensure the corresponding GwyToolboxEditorRow is updated! */
...
group_id_changed(GwyToolboxGroupEditor *geditor, GtkEntry *entry)
{
    ...
    if (!errmessage) {
        ...
        for (i = 0; i < group->len; i++) {
            gspec = &g_array_index(group, GwyToolboxGroupSpec, i);
            if (i != geditor->i && gwy_strequal(g_quark_to_string(gspec->id), newid)) {
    ...
    gtk_dialog_set_response_sensitive(GTK_DIALOG(geditor->dialogue), GTK_RESPONSE_OK, !errmessage);
}
...
suggest_group_id(GwyToolboxGroupEditor *geditor)
{
    ...
    for (i = 0; i < name_len; i++) {
        decomp_len = g_unichar_fully_decompose(name_chars[i], TRUE, buf, G_N_ELEMENTS(buf));
...
edit_item_dialogue(GwyToolboxEditor *editor, GwyToolboxItemSpec *ispec)
{
    ...
    ieditor.dialogue = gtk_dialog_new_with_buttons(_("Toolbox Item"), GTK_WINDOW(editor->dialogue), GTK_DIALOG_MODAL,
                                                   GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
                                                   GTK_STOCK_OK, GTK_RESPONSE_OK,
                                                   NULL);
    ...
    hbox = GTK_BOX(gtk_hbox_new(FALSE, 6));
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(ieditor.dialogue)->vbox), GTK_WIDGET(hbox), TRUE, TRUE, 0);
    ...
    ieditor.type = gwy_enum_combo_box_newl(G_CALLBACK(function_type_changed), &ieditor, ieditor.ispec.type,
                                           ...
                                           _("Curve Maps"), GWY_APP_ACTION_TYPE_CMAP,
                                           _("Tool"), GWY_APP_ACTION_TYPE_TOOL,
                                           NULL);
    gtk_table_attach(table, ieditor.type, 1, 2, row, row+1, GTK_EXPAND | GTK_FILL, 0, 0, 0);
    ...
    ieditor.mode = gwy_enum_combo_box_newl(G_CALLBACK(function_mode_changed), &ieditor, ieditor.ispec.mode,
                                           _("Default"), 0,
                                           _("Interactive"), GWY_RUN_INTERACTIVE,
                                           _("Non-interactive"), GWY_RUN_IMMEDIATE,
                                           NULL);
    gtk_table_attach(table, ieditor.mode, 1, 2, row, row+1, GTK_EXPAND | GTK_FILL, 0, 0, 0);
    gtk_label_set_mnemonic_widget(GTK_LABEL(label), ieditor.mode);
    row++;

    label = ieditor.icon_source_label = gtk_label_new_with_mnemonic(_("Use _icon from:"));
    gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
    gtk_table_attach(table, label, 0, 1, row, row+1, GTK_FILL, 0, 0, 0);

    iconsource = TOOLBOX_ICON_SOURCE_DEFAULT;
    ieditor.icon_source = gwy_enum_combo_box_newl(G_CALLBACK(function_icon_source_changed), &ieditor, iconsource,
                                                  _("Default"), TOOLBOX_ICON_SOURCE_DEFAULT,
                                                  "Gwyddion", TOOLBOX_ICON_SOURCE_GWY,
                                                  "GTK+", TOOLBOX_ICON_SOURCE_GTK,
                                                  NULL);
    gtk_table_attach(table, ieditor.icon_source, 1, 2, row, row+1, GTK_EXPAND | GTK_FILL, 0, 0, 0);
    gtk_label_set_mnemonic_widget(GTK_LABEL(label), ieditor.mode);
    row++;

    create_icon_icon_view(&ieditor);
    gtk_icon_view_set_selection_mode(ieditor.icon_view, GTK_SELECTION_BROWSE);
    g_signal_connect_swapped(ieditor.icon_view, "selection-changed", G_CALLBACK(function_icon_changed), &ieditor);
    scwin = gtk_scrolled_window_new(NULL, NULL);
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scwin), GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
    gtk_container_add(GTK_CONTAINER(scwin), GTK_WIDGET(ieditor.icon_view));
    gtk_table_attach(table, scwin, 0, 2, row, row+1, GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
    row++;
    ...
    gtk_tree_selection_set_mode(selection, GTK_SELECTION_BROWSE);
    g_signal_connect_swapped(selection, "changed", G_CALLBACK(function_selection_changed), &ieditor);
...
create_item_editor_models(GwyToolboxEditor *editor)
{
    ...
    stores[GWY_APP_ACTION_TYPE_PROC] = create_function_list(gwy_process_func_foreach, GWY_APP_ACTION_TYPE_PROC);
    stores[GWY_APP_ACTION_TYPE_GRAPH] = create_function_list(gwy_graph_func_foreach, GWY_APP_ACTION_TYPE_GRAPH);
    stores[GWY_APP_ACTION_TYPE_VOLUME] = create_function_list(gwy_volume_func_foreach, GWY_APP_ACTION_TYPE_VOLUME);
    stores[GWY_APP_ACTION_TYPE_XYZ] = create_function_list(gwy_xyz_func_foreach, GWY_APP_ACTION_TYPE_XYZ);
    stores[GWY_APP_ACTION_TYPE_CMAP] = create_function_list(gwy_curve_map_func_foreach, GWY_APP_ACTION_TYPE_CMAP);
    stores[GWY_APP_ACTION_TYPE_TOOL] = create_function_list(gwy_tool_func_foreach, GWY_APP_ACTION_TYPE_TOOL);
    ...
    editor->icon_model_default = gtk_list_store_new(2, G_TYPE_UINT, GDK_TYPE_PIXBUF);
}
...
sync_remaining_tools_item(GwyToolboxEditor *editor)
{
    ...
    if ((seen_tool_placeholder && qname) || (!seen_tool_placeholder && !qname))
        return;
...
try_to_select_item_function(GwyToolboxItemEditor *ieditor, GQuark qname)
{
    ...
    do {
        ...
        if (quark == qname) {
            ...
            gtk_tree_view_scroll_to_cell(ieditor->function_view, path, NULL, TRUE, 0.5, 0.0);
...
try_to_select_item_icon(GwyToolboxItemEditor *ieditor, GQuark qname)
{
    ...
    if (!qname) {
        gwy_enum_combo_box_set_active(GTK_COMBO_BOX(ieditor->icon_source), TOOLBOX_ICON_SOURCE_DEFAULT);
    }
    else {
        ...
        gwy_enum_combo_box_set_active(GTK_COMBO_BOX(ieditor->icon_source), iconsource);
    }
    ...
    do {
        ...
        if (TOOLBOX_ICON_SOURCE_DEFAULT || quark == qname) {
            ...
            gtk_icon_view_scroll_to_path(ieditor->icon_view, path, TRUE, 0.5, 0.0);
            gtk_tree_path_free(path);
            return;
        }
    } while (gtk_tree_model_iter_next(model, &iter));
...
function_type_changed(GtkComboBox *combo, GwyToolboxItemEditor *ieditor)
{
    ...
    if (type == GWY_APP_ACTION_TYPE_PLACEHOLDER) {
        ...
        gtk_icon_view_set_model(ieditor->icon_view, GTK_TREE_MODEL(editor->icon_model_default));
...
function_icon_source_changed(GtkComboBox *combo, GwyToolboxItemEditor *ieditor)
{
    ...
    if (iconsource == TOOLBOX_ICON_SOURCE_GWY) {
        gtk_icon_view_set_model(ieditor->icon_view, GTK_TREE_MODEL(editor->icon_model_gwy));
    }
    else if (iconsource == TOOLBOX_ICON_SOURCE_GTK) {
        gtk_icon_view_set_model(ieditor->icon_view, GTK_TREE_MODEL(editor->icon_model_gtk));
    }
    else {
        gtk_list_store_clear(editor->icon_model_default);
        add_default_function_icon(editor->icon_model_default, &ieditor->ispec, GTK_WIDGET(ieditor->icon_view));
        gtk_icon_view_set_model(ieditor->icon_view, GTK_TREE_MODEL(editor->icon_model_default));
    }
...
function_selection_changed(GwyToolboxItemEditor *ieditor)
{
     ...
     * choose from. */
    if (qname) {
        modes = gwy_toolbox_action_run_modes(ieditor->ispec.type, g_quark_to_string(qname));
    }
    /* We only care if the function has both run modes.  Otherwise there is
     * nothing to choose from and we enforce the default mode. */
    if ((modes & (GWY_RUN_IMMEDIATE | GWY_RUN_INTERACTIVE)) != (GWY_RUN_IMMEDIATE | GWY_RUN_INTERACTIVE))
        modes = 0;
    ...
    iconsource = gwy_enum_combo_box_get_active(GTK_COMBO_BOX(ieditor->icon_source));
    ...
    detail = gwy_toolbox_action_detail(ieditor->ispec.type, g_quark_to_string(qname));
                          ...
                          GtkWidget *widget)
{
    ...
    stock_id = gwy_toolbox_action_stock_id(ispec->type, g_quark_to_string(ispec->function));
    if (!stock_id)
        return;
...
update_toolbox_sensitivity(GwyToolboxEditor *editor)
{
    ...
    gtk_widget_set_sensitive(editor->move_up, !tree_model_iter_is_first(model, &iter));
    gtk_widget_set_sensitive(editor->move_down, !tree_model_iter_is_last(model, &iter));
}
...
add_toolbox_item(GwyToolboxEditor *editor)
{
    ...
    row = g_slice_new0(GwyToolboxEditorRow);
    fill_toolbox_row_for_item(row, &ispec);
    gtk_tree_store_insert_with_values(editor->toolbox_model, &iter, &parent, j, 0, row, -1);
...
add_toolbox_group(GwyToolboxEditor *editor)
{
    ...
    row = g_slice_new0(GwyToolboxEditorRow);
    fill_toolbox_row_for_group(row, &gspec);
    gtk_tree_store_insert_with_values(editor->toolbox_model, &iter, NULL, i, 0, row, -1);
}
...
apply_toolbox_spec(GwyToolboxEditor *editor)
{
    ...
    /* We work on a copy.  But invoking gwy_toolbox_rebuild_to_spec() also makes the passed spec the actual primary
     * spec.  So pass a copy. */
...
reset_toolbox_to_default(GwyToolboxEditor *editor)
{
    ...
    gtk_tree_view_set_model(editor->toolbox_view, GTK_TREE_MODEL(editor->toolbox_model));
    gtk_tree_view_expand_all(editor->toolbox_view);
...
{
    /* We need to use the names directly, not symbols, because the list includes icons that may not be present in Gtk+
     * we compile Gwyddion with. */
    ...
    return create_icon_list(widget, stock_icon_names, G_N_ELEMENTS(stock_icon_names));
}
...
/* @@@ GENERATED STOCK LIST BEGIN @@@ */
    ...
    GWY_STOCK_GWYDDION,
    GWY_STOCK_HOLD_SELECTION_CLEAR,
    GWY_STOCK_HOLD_SELECTION_REPLACE,
    GWY_STOCK_HOUGH,
