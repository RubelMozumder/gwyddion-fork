/*
 *  $Id: about.c 25253 2023-02-24 16:04:30Z yeti-dn $
...
gwy_app_about(void)
{
    ...
    gtk_misc_set_padding(GTK_MISC(widget), 2, 4);
    ...
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(about)->vbox), notebook, TRUE, TRUE, 0);
...
fill_credits(GtkTextBuffer *buffer)
{
    ...
    gtk_text_buffer_insert_with_tags_by_name(buffer, &iter, "http://www.cmi.cz/", -1, "uri", NULL);
}
...
#ifdef HAVE_GTKGLEXT
    /* XXX: Variables from DLLs work strangely in Win32.  We probably get uninitialised memory when reading them.
     * Show the compiled in version, not run-time, so we show at least something sane... */
