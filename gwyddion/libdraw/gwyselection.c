/*
 *  $Id: gwyselection.c 25174 2022-12-15 16:26:37Z yeti-dn $
...
gwy_selection_class_init(GwySelectionClass *klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_OBJECT_SIZE,
 ...
 * @selection: A selection.
 * @filter: Function returning %TRUE for objects that should be kept, %FALSE for objects that should be removed.
                     ...
                     gpointer data)
{
    ...
    /* Be careful to work with non-default implementations.  Do not assume we know much about the internal structure. */
 ...
 * @selection: A selection.
 * @data: Array to store selection data to.  Selection data is an array of coordinates whose precise meaning is
 *        defined by particular selection types.  It may be %NULL.
 *
 * Gets selection data.
 *
 * Returns: The number of selected objects.  This is *not* the required size of @data, which must be at least
                                 ...
                                 gdouble *data)
{
    ...
    object_size = GWY_SELECTION_GET_CLASS(selection)->object_size;
    gwy_assign(data, (gdouble*)selection->objects->data + i*object_size, object_size);
    return TRUE;
}
                                 ...
                                 const gdouble *data)
{
    ...
    if (i > selection->n) {
        g_warning("Disontinuous selections are not supported.  Moving object to first feasible position.");
        i = MIN(selection->n, max_len-1);
    }

    selection->n = MAX(selection->n, i+1);
    gwy_assign((gdouble*)selection->objects->data + i*object_size, data, object_size);
                                ...
                                GByteArray *buffer)
{
    ...
    object_size = GWY_SELECTION_GET_CLASS(selection)->object_size;
    {
        ...
        return gwy_serialize_pack_object_struct(buffer, name, G_N_ELEMENTS(spec), spec);
    }
}
                                  ...
                                  gsize *position)
{
    ...
    if (!gwy_serialize_unpack_object_struct(buffer, size, position, typename, G_N_ELEMENTS(spec), spec)) {
        g_free(data);
        return NULL;
    }
    ...
    if (data && len) {
        if (len % object_size)
            g_warning("Selection data size not multiple of object size. Ignoring it.");
        else {
...
gwy_selection_duplicate_default(GObject *object)
{
    ...
    g_array_append_vals(duplicate->objects, selection->objects->data, selection->n*object_size);
...
gwy_selection_clone_default(GObject *source, GObject *copy)
{
    ...
    g_array_append_vals(clone->objects, selection->objects->data, selection->n*object_size);
    clone->n = selection->n;
 ...
 * available.  The method gwy_selection_set_data() with %NULL second argument is also used to determine the number of
 * selected object.
 **/

/**
 * GwySelection:
 * @objects: Array of object coordinates whose meaning is defined by each selection type (subclass).  Default
 *           #GwySelection virtual methods assume each selection object is of the same size, stored in class
 *           @object_size field at selection class init time.  The size of array (multiplied with @object_size)
 *           determines maximum number of selectable objects.
 ...
 *
 * Note that it also copies properties like #GwySelection:max-objects, which may or may not be wanted.
 *
 * Since: 2.52
 **/
