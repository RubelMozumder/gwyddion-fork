...
xrefdirs=$(echo $libs | sed 's#[^ ]\+#--extra-dir=../\0#g')
...
function fix() {
  ...
  pushd $name
  # Rename the xhtml subdirectory to the module name to prevent fixxref naming
  # everything xhtml.
  mv xhtml $name
  gtkdoc-fixxref --module=$name --module-dir=$name $xrefdirs
